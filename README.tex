% Created 2026-05-26 Tue 17:19
% Intended LaTeX compiler: pdflatex
\documentclass[11pt]{article}

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{graphicx}
\usepackage{longtable}
\usepackage{wrapfig}
\usepackage{rotating}
\usepackage[normalem]{ulem}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{capt-of}
\usepackage{hyperref}
\hypersetup{
colorlinks=true,
linkcolor=black,
filecolor=black,
urlcolor=blue,
citecolor=black,
pdfborder={0 0 0}
}
\usepackage{tocloft}
\cftsetindents{section}{0em}{3em}
\cftsetindents{subsection}{1.5em}{4em}
\cftsetindents{subsubsection}{3.5em}{5em}
\author{Cisco Ramon}
\date{\textit{{[}2026-03-14 Sat 00:35]}}
\title{ClusterSetup}
\hypersetup{
 pdfauthor={Cisco Ramon},
 pdftitle={ClusterSetup},
 pdfkeywords={},
 pdfsubject={Building a mini HPC cluster environment using Rocky Linux, VirtualBox, Slurm, MPI, and OpenMP},
 pdfcreator={},
 pdflang={English}}
\begin{document}

\maketitle
\setcounter{tocdepth}{2}
\tableofcontents

\section{Abstract}
\label{sec:orga54c2b8}

This documentation presents the complete setup and configuration of a
lightweight High Performance Computing (HPC) cluster environment using
Rocky Linux and VirtualBox. The guide covers practical implementation
of core HPC infrastructure components including shared storage, Slurm
workload management, LDAP-based centralized authentication, MPI runtime
environment, and centralized software stack management using Spack.

The cluster environment supports multinode job execution, centralized
software distribution, distributed MPI applications, and scheduler-aware
runtime execution. The setup follows real-world HPC concepts while
remaining suitable for learning, experimentation, and practical hands-on
training.
\section{Preface}
\label{sec:org79910ac}

This documentation was prepared with the assistance of AI tools for:

\begin{itemize}
\item structuring
\item formatting
\item workflow organization
\item command documentation
\item concept explanations
\end{itemize}

However, the complete HPC cluster setup, configuration, testing, and
validation were performed manually by me in a fully working VirtualBox
based cluster environment.

All commands, scripts, workflows, and configurations documented in this
guide were tested successfully during the setup process, including:

\begin{itemize}
\item cluster networking
\item shared storage
\item NFS configuration
\item Slurm workload manager
\item LDAP authentication
\item centralized user management
\item Spack software stack
\item MPI runtime environment
\item multinode execution
\end{itemize}

This guide is intended for:

\begin{itemize}
\item students
\item beginners
\item Linux learners
\item HPC enthusiasts
\item system administrators
\item researchers
\end{itemize}

who want practical hands-on experience in:

\begin{itemize}
\item HPC infrastructure setup
\item cluster administration
\item workload scheduling
\item centralized authentication
\item parallel programming
\item MPI execution
\item software stack management
\end{itemize}

The goal of this documentation is not only to provide commands, but
also to explain the practical concepts behind real HPC environments in a
simple and beginner-friendly manner.
\section{About the Author}
\label{sec:org18b6424}

Hi👋, I'm Abhishek Raj

I work in the field of High Performance Computing (HPC), Linux systems,
parallel programming, and scientific computing with experience in HPC
application optimization and distributed computing environments,
including the PARAM series of supercomputers.

My primary areas of interest include:

\begin{itemize}
\item HPC clusters and infrastructure
\item Linux systems and administration
\item MPI, OpenMP, CUDA, and OpenACC
\item scientific and research workloads
\item distributed systems
\item software optimization and performance tuning
\item automation and reproducible environments
\end{itemize}

This project was created to gain a deeper understanding of how HPC
clusters are designed and managed internally. The objective was not only
to build a working cluster environment from scratch, but also to better
understand the complete software and infrastructure stack behind
parallel computing systems, which ultimately helps in writing and
optimizing efficient HPC applications.

GitHub Profile:

\textbf{\href{https://github.com/CISSSCO/}{https://github.com/CISSSCO/}}

LinkedIn :

\textbf{\href{https://www.linkedin.com/in/abhi581b}{https://www.linkedin.com/in/abhi581b}}

Project Repository:

\textbf{\href{https://github.com/CISSSCO/hpccfs}{https://github.com/CISSSCO/hpccfs}}

Hosted Documentation:

\textbf{\href{https://cisssco.github.io/hpccfs}{https://cisssco.github.io/hpccfs}}

Source codes, scripts, configuration files, and examples used in this
guide are available in the project repository.

For queries, suggestions, or discussions:

\textbf{\href{https://ciscoramon.netlify.app/}{https://ciscoramon.netlify.app/}}

If you encounter any issue in the documentation, setup process, or
configuration steps, feel free to create an issue in the GitHub
repository.
\section{Introduction}
\label{sec:org56c0353}

High Performance Computing (HPC) clusters are distributed computing
systems designed to execute computational workloads across multiple
machines simultaneously. Instead of relying on a single system, HPC
clusters combine multiple compute nodes to provide higher computational
capacity, scalability, and parallel execution capability.

Modern HPC environments are widely used in:

\begin{itemize}
\item scientific simulations
\item engineering workloads
\item computational chemistry
\item artificial intelligence
\item machine learning
\item weather modeling
\item large-scale data analysis
\end{itemize}

A typical HPC cluster contains several important components working
together:

\begin{center}
\begin{tabular}{ll}
Component & Purpose\\
\hline
Login Node & User access and job submission\\
Compute Nodes & Execute computational workloads\\
Shared Storage & Shared user data and software\\
Scheduler & Resource allocation and job management\\
MPI Runtime & Distributed parallel execution\\
Authentication System & Centralized user management\\
\end{tabular}
\end{center}

This guide focuses on building these components step-by-step in a small
virtualized environment using:

\begin{itemize}
\item Rocky Linux
\item VirtualBox
\item Slurm
\item OpenMPI
\item LDAP
\item NFS
\item Spack
\end{itemize}

Although the setup is lightweight and designed for learning purposes,
the overall architecture closely follows concepts used in production HPC
centers and supercomputers.

Throughout this documentation, readers will gradually configure:

\begin{itemize}
\item cluster networking
\item shared storage
\item centralized authentication
\item workload scheduling
\item distributed software stacks
\item MPI-based parallel execution
\item scheduler-aware job execution
\end{itemize}

The final environment provides a fully working multinode HPC cluster
capable of running distributed parallel applications across multiple
compute nodes.
\section{Cluster Architecture}
\label{sec:org142b048}

The cluster consists of five virtual machines connected using an internal
network.

\begin{verbatim}
                 HOST MACHINE
                       |
         ssh -p XXXX localhost
                       |
               [ NAT Adapter ]
                       |
 ------------------------------------------------
 |               INTERNAL NETWORK               |
 |                  "hpcnet"                    |
 ------------------------------------------------

master      10.10.10.1
login       10.10.10.2
compute01   10.10.10.11
compute02   10.10.10.12
compute03   10.10.10.13
\end{verbatim}
\section{Node Details}
\label{sec:org44a408c}
\subsection{Node Role}
\label{sec:org472683a}
\begin{enumerate}
\item Master Node
\label{sec:org62c4106}

The master node manages the overall cluster.

Main responsibilities:

\begin{itemize}
\item Slurm controller
\item NFS server
\item LDAP server
\item Monitoring tools
\item Shared software stack
\item Cluster configuration
\end{itemize}
\item Login Node
\label{sec:org857a364}

The login node is the access point for users.

Typical tasks performed here:

\begin{itemize}
\item SSH login
\item Code compilation
\item Job submission
\item Accessing shared storage
\end{itemize}
\item Compute Nodes
\label{sec:org61f0781}

Compute nodes run the actual workloads.

These nodes are mainly used for:

\begin{itemize}
\item MPI jobs
\item OpenMP jobs
\item Batch workloads
\item Parallel execution
\end{itemize}
\end{enumerate}
\subsection{Networking Setup}
\label{sec:org6cf0739}

Two network adapters are used for each VM.
\begin{enumerate}
\item NAT Adapter
\label{sec:org40b9396}

Used for:

\begin{itemize}
\item Internet access
\item Package installation
\item SSH access from host system
\end{itemize}
\item Internal Network
\label{sec:orga770ccf}

Used for communication between cluster nodes.

Network name:

\begin{verbatim}
hpcnet
\end{verbatim}

This network is isolated and only used inside the cluster.
\end{enumerate}
\subsection{Static IP Addressing}
\label{sec:org0342b68}

Each node uses a fixed IP address.

\begin{center}
\begin{tabular}{llrr}
VM Name & Hostname & Internal IP & SSH Port\\
\hline
master & master.local & 10.10.10.1 & 2221\\
login & login.local & 10.10.10.2 & 2222\\
compute01 & compute01.local & 10.10.10.11 & 2223\\
compute02 & compute02.local & 10.10.10.12 & 2224\\
compute03 & compute03.local & 10.10.10.13 & 2225\\
\end{tabular}
\end{center}

Example SSH access:

\begin{verbatim}
ssh -p 2221 hpcuser@localhost
\end{verbatim}
\section{Resource Allocation}
\label{sec:orga8380cf}

The following configuration is enough for a lightweight HPC lab.
You can increase resources later depending on your host system.
\subsection{Master Node}
\label{sec:orgb075d7e}

\begin{center}
\begin{tabular}{ll}
Resource & Value\\
\hline
RAM & 2 GB\\
CPU & 2\\
Disk & 30 GB\\
\end{tabular}
\end{center}
\subsection{Login Node}
\label{sec:org47dd378}

\begin{center}
\begin{tabular}{ll}
Resource & Value\\
\hline
RAM & 2 GB\\
CPU & 2\\
Disk & 25 GB\\
\end{tabular}
\end{center}
\subsection{Compute Nodes}
\label{sec:org8853e72}

\begin{center}
\begin{tabular}{ll}
Resource & Value\\
\hline
RAM & 2 GB\\
CPU & 2\\
Disk & 20 GB\\
\end{tabular}
\end{center}
\section{Master Node Setup}
\label{sec:org79124d4}

This section covers the complete setup of the master node. The master node
acts as the central management node of the cluster.

Main responsibilities:

\begin{itemize}
\item Cluster management
\item Shared storage
\item Slurm controller
\item Monitoring services
\item User authentication
\item Software management
\end{itemize}
\subsection{Install VirtualBox on Manjaro}
\label{sec:org984d7dc}

I'm using Manjaro (Arch) as my base system.
\begin{enumerate}
\item Update packages
\label{sec:org9056ba8}

\begin{verbatim}
sudo pacman -Syu
\end{verbatim}
\item Install VirtualBox and host modules
\label{sec:orgeb48319}

\begin{verbatim}
sudo pacman -S virtualbox virtualbox-host-modules-arch
\end{verbatim}
\item Install VirtualBox extension pack
\label{sec:orgedf2e2b}

\begin{verbatim}
sudo pacman -S virtualbox-ext-oracle
\end{verbatim}
\item Load VirtualBox kernel module
\label{sec:org1da2e1c}

\begin{verbatim}
sudo modprobe vboxdrv
\end{verbatim}
\item Add current user to VirtualBox group
\label{sec:orgeb9f844}

\begin{verbatim}
sudo usermod -aG vboxusers $USER
\end{verbatim}
\item Reboot the system
\label{sec:orgbc0ae24}

\begin{verbatim}
sudo reboot
\end{verbatim}
\end{enumerate}
\subsection{Download Rocky Linux}
\label{sec:org3caa480}

Download Rocky Linux Minimal ISO:

\url{https://rockylinux.org/download}

Choose:

\begin{itemize}
\item x86\_64
\item Minimal ISO
\end{itemize}

The minimal installation keeps the environment lightweight and avoids
unnecessary packages.
\subsection{Create Master VM}
\label{sec:orga3fa560}

Open VirtualBox and create a new virtual machine.
\begin{enumerate}
\item VM Name
\label{sec:org0962fae}

\begin{verbatim}
master
\end{verbatim}
\item Type
\label{sec:orgdd39ade}

\begin{verbatim}
Linux
Red Hat (64-bit)
\end{verbatim}
\item Hardware
\label{sec:org0733d64}

\begin{center}
\begin{tabular}{ll}
Setting & Value\\
\hline
RAM & 2046 MB\\
CPU & 2\\
Disk & 30 GB\\
\end{tabular}
\end{center}
\item Disk Type
\label{sec:org21e41a7}

\begin{verbatim}
VDI
Dynamically Allocated
\end{verbatim}

This configuration is enough for a lightweight HPC lab environment.
\end{enumerate}
\subsection{Network Configuration}
\label{sec:org2c174fb}
\begin{enumerate}
\item Adapter 1
\label{sec:org879d9d0}

\begin{verbatim}
Attached to: NAT
\end{verbatim}
Used for:

\begin{itemize}
\item Internet access
\item Package installation
\item SSH access from host machine
\end{itemize}
\item NAT Port Forwarding
\label{sec:orgb259550}

Open:

\begin{verbatim}
Settings -> Network -> Adapter 1 -> Advanced -> Port Forwarding
\end{verbatim}

\begin{center}
\begin{tabular}{llrr}
Name & Protocol & Host Port & Guest Port\\
\hline
ssh & TCP & 2221 & 22\\
\end{tabular}
\end{center}
\item Adapter 2
\label{sec:org7969fa1}

\begin{verbatim}
Attached to: Internal Network
\end{verbatim}

Internal Network Name:

\begin{verbatim}
hpcnet
\end{verbatim}

IMPORTANT:
All nodes MUST use the same internal network name.
This network is used for communication between cluster nodes.
\end{enumerate}
\subsection{Rocky Linux Installation}
\label{sec:orgdaf223e}

Choose:

\begin{verbatim}
Minimal Install
\end{verbatim}
\begin{enumerate}
\item Network and Hostname
\label{sec:orgeabe82a}

Enable BOTH network adapters.

Set hostname:

\begin{verbatim}
master.local
\end{verbatim}
\item User Creation
\label{sec:org2f95d95}

Create an administrative user during installation.

\begin{center}
\begin{tabular}{ll}
Field & Value\\
\hline
Username & hpcadmin\\
Administrator & YES\\
\end{tabular}
\end{center}
After installation, log into the VM.
\end{enumerate}
\subsection{Initial System Verification}
\label{sec:org5c8041d}
\begin{enumerate}
\item Check Interfaces
\label{sec:orge3ea144}

\begin{verbatim}
ip addr
\end{verbatim}

Expected:

\begin{itemize}
\item NAT interface should receive:
\begin{itemize}
\item 10.0.2.15
\end{itemize}
\item Internal interface should exist but may not yet have static IP
\end{itemize}
\end{enumerate}
\subsection{Configure Static Internal Network}
\label{sec:org7381602}
\begin{enumerate}
\item Check Device Names
\label{sec:orge2355dd}

\begin{verbatim}
nmcli device status
\end{verbatim}

Expected:

\begin{center}
\begin{tabular}{ll}
Interface & Purpose\\
\hline
enp0s3 & NAT\\
enp0s8 & Internal Network\\
\end{tabular}
\end{center}
\end{enumerate}
\subsection{Configure Internal Interface}
\label{sec:org48c4804}

Launch NetworkManager TUI:

\begin{verbatim}
sudo nmtui
\end{verbatim}

Navigate:

\begin{verbatim}
Edit a connection
\end{verbatim}

Select:

\begin{verbatim}
enp0s8
\end{verbatim}

Set:

\begin{center}
\begin{tabular}{ll}
Field & Value\\
\hline
IPv4 Method & Manual\\
Address & 10.10.10.1/24\\
Gateway & blank\\
DNS & blank\\
\end{tabular}
\end{center}

DO NOT modify NAT interface.
\subsection{Restart Networking}
\label{sec:orgd4b76ce}

\begin{verbatim}
sudo systemctl restart NetworkManager
\end{verbatim}
\subsection{Verify Networking}
\label{sec:org8e49e33}

\begin{verbatim}
ip addr
\end{verbatim}

Expected:

\begin{center}
\begin{tabular}{lr}
Interface & IP\\
\hline
NAT & 10.0.2.x\\
Internal & 10.10.10.1\\
\end{tabular}
\end{center}
\subsection{SSH Access from Host}
\label{sec:org7e134aa}

From host machine:

\begin{verbatim}
ssh -p 2221 hpcadmin@localhost
\end{verbatim}
If the connection works, port forwarding is configured correctly.
\subsection{System Update and Packages}
\label{sec:org65143c5}
\begin{enumerate}
\item Update System
\label{sec:org5cd8012}

\begin{verbatim}
sudo dnf update -y
\end{verbatim}
\item Install Base Packages
\label{sec:orgcc913c3}

\begin{verbatim}
sudo dnf install -y \
vim wget curl git \
net-tools bind-utils \
htop tmux tree \
gcc gcc-c++ gcc-gfortran \
make cmake \
python3 \
openssh-server
\end{verbatim}

These packages provide:

\begin{itemize}
\item Development tools
\item Network utilities
\item Monitoring utilities
\item Basic administration tools
\end{itemize}
\end{enumerate}
\subsection{Timezone Configuration}
\label{sec:orgd03c13a}

Check timezone:

\begin{verbatim}
timedatectl
\end{verbatim}

Set timezone if needed:

\begin{verbatim}
sudo timedatectl set-timezone Asia/Kolkata
\end{verbatim}
\subsection{Configure /etc/hosts}
\label{sec:orgf79fe38}

Edit:

\begin{verbatim}
sudo vim /etc/hosts
\end{verbatim}

Add:

\begin{verbatim}
10.10.10.1   master master.local
10.10.10.2   login login.local
10.10.10.11  compute01 compute01.local
10.10.10.12  compute02 compute02.local
10.10.10.13  compute03 compute03.local
\end{verbatim}
This allows hostname-based communication between nodes.
\subsection{Disable Firewall (Temporary)}
\label{sec:orgee30e97}

For initial setup and testing:
\begin{verbatim}
sudo systemctl disable --now firewalld
\end{verbatim}
NOTE:

Firewall rules can be configured properly later after cluster setup is complete.
\subsection{SELinux Configuration}
\label{sec:org98e3577}

Check status:

\begin{verbatim}
getenforce
\end{verbatim}

Temporarily disable enforcing:

\begin{verbatim}
sudo setenforce 0
\end{verbatim}

Persist configuration:

\begin{verbatim}
sudo vim /etc/selinux/config
\end{verbatim}

Modify:

\begin{verbatim}
SELINUX=permissive
\end{verbatim}
This helps avoid permission-related issues during initial cluster setup.
\subsection{Verify Hostname}
\label{sec:org132b6d6}

\begin{verbatim}
hostnamectl
\end{verbatim}

Expected:

\begin{verbatim}
Static hostname: master.local
\end{verbatim}

If hostname is not updated(showing previous), reboot the machine.

At this point, the master node setup is complete and ready for cloning or
further cluster configuration.
\section{Cloning Nodes}
\label{sec:orgf52af60}

Purpose:

\begin{itemize}
\item avoid reinstalling all nodes manually
\item simulate real HPC image provisioning workflow
\end{itemize}
\subsection{Create Snapshot}
\label{sec:org6c7fa9d}

Power off VM:

\begin{verbatim}
sudo poweroff
\end{verbatim}

Create snapshot in VirtualBox:

\begin{verbatim}
clean-base-master
\end{verbatim}
\subsection{Clone Nodes}
\label{sec:org427c244}

Create FULL CLONES.

IMPORTANT:
Enable:

\begin{verbatim}
Reinitialize the MAC address of all network cards
\end{verbatim}
\subsection{Clone Names}
\label{sec:org9b5e940}

\begin{center}
\begin{tabular}{l}
VM Name\\
\hline
login\\
compute01\\
compute02\\
compute03\\
\end{tabular}
\end{center}
\subsection{Configure Port Forwarding}
\label{sec:orgaff49bc}
\begin{enumerate}
\item Login Node
\label{sec:org1eb8c68}

\begin{center}
\begin{tabular}{rr}
Host Port & Guest Port\\
\hline
2222 & 22\\
\end{tabular}
\end{center}
\item Compute01
\label{sec:orgdbecab1}

\begin{center}
\begin{tabular}{rr}
Host Port & Guest Port\\
\hline
2223 & 22\\
\end{tabular}
\end{center}
\item Compute02
\label{sec:org6338d67}

\begin{center}
\begin{tabular}{rr}
Host Port & Guest Port\\
\hline
2224 & 22\\
\end{tabular}
\end{center}
\item Compute03
\label{sec:org8a05559}

\begin{center}
\begin{tabular}{rr}
Host Port & Guest Port\\
\hline
2225 & 22\\
\end{tabular}
\end{center}
\end{enumerate}
\subsection{Machine ID Regeneration}
\label{sec:org6821856}

IMPORTANT:
Each cloned Linux machine must have unique machine-id.
\subsection{Remove Existing Machine ID}
\label{sec:orgda06d13}

\begin{verbatim}
sudo rm -f /etc/machine-id
\end{verbatim}
\subsection{Generate New Machine ID}
\label{sec:org15d146e}

\begin{verbatim}
sudo systemd-machine-id-setup
\end{verbatim}
\subsection{Verify Machine ID}
\label{sec:orgb614ea5}

\begin{verbatim}
cat /etc/machine-id
\end{verbatim}

Ensure every node has unique machine-id.
\subsection{Configure Hostname}
\label{sec:orgcc3afbc}
\begin{enumerate}
\item Login Node
\label{sec:org458d5f0}

Edit hostname:

\begin{verbatim}
sudo vim /etc/hostname
\end{verbatim}

Set:

\begin{verbatim}
login.local
\end{verbatim}

Apply:

\begin{verbatim}
sudo hostnamectl set-hostname login.local
\end{verbatim}
\item Compute01
\label{sec:orgf047b79}

Set hostname:

\begin{verbatim}
compute01.local
\end{verbatim}

Apply:

\begin{verbatim}
sudo hostnamectl set-hostname compute01.local
\end{verbatim}
\item Compute02
\label{sec:org6438e4d}

\begin{verbatim}
compute02.local
\end{verbatim}

\begin{verbatim}
sudo hostnamectl set-hostname compute02.local
\end{verbatim}
\item Compute03
\label{sec:org3a51d1e}

\begin{verbatim}
compute03.local
\end{verbatim}

\begin{verbatim}
sudo hostnamectl set-hostname compute03.local
\end{verbatim}
\end{enumerate}
\subsection{Configure Static Internal IPs}
\label{sec:org7fc40e9}

Launch:

\begin{verbatim}
sudo nmtui
\end{verbatim}

Edit internal adapter:

\begin{verbatim}
enp0s8
\end{verbatim}
\begin{enumerate}
\item Login Node
\label{sec:org0325b14}

\begin{verbatim}
10.10.10.2/24
\end{verbatim}
\item Compute01
\label{sec:org463caf1}

\begin{verbatim}
10.10.10.11/24
\end{verbatim}
\item Compute02
\label{sec:orgef40429}

\begin{verbatim}
10.10.10.12/24
\end{verbatim}
\item Compute03
\label{sec:orga81a57c}

\begin{verbatim}
10.10.10.13/24
\end{verbatim}
\end{enumerate}
\subsection{Restart Networking}
\label{sec:orga45abc2}

\begin{verbatim}
sudo systemctl restart NetworkManager
\end{verbatim}
\subsection{Verify Networking}
\label{sec:orgd194c61}

\begin{verbatim}
ip addr
\end{verbatim}
\subsection{Verify Routing}
\label{sec:org4563054}

\begin{verbatim}
ip route
\end{verbatim}

Expected:

\begin{verbatim}
default via 10.0.2.2 dev enp0s3
10.10.10.0/24 dev enp0s8
\end{verbatim}
\subsection{Verify Connectivity}
\label{sec:org1207405}
\begin{enumerate}
\item Ping Between Nodes
\label{sec:org5c5b954}

\begin{verbatim}
ping master
\end{verbatim}

\begin{verbatim}
ping login
\end{verbatim}

\begin{verbatim}
ping compute01
\end{verbatim}
\end{enumerate}
\subsection{SSH Verification}
\label{sec:org0460a0a}

From host:
\begin{enumerate}
\item Master
\label{sec:orgd5d0a74}

\begin{verbatim}
ssh -p 2221 hpcadmin@localhost
\end{verbatim}
\item Login
\label{sec:orga00517f}

\begin{verbatim}
ssh -p 2222 hpcadmin@localhost
\end{verbatim}
\item Compute01
\label{sec:orgea3c6ef}

\begin{verbatim}
ssh -p 2223 hpcadmin@localhost
\end{verbatim}
\item Compute02
\label{sec:org7bc0799}

\begin{verbatim}
ssh -p 2224 hpcadmin@localhost
\end{verbatim}
\item Compute03
\label{sec:org04674b8}

\begin{verbatim}
ssh -p 2225 hpcadmin@localhost
\end{verbatim}
\end{enumerate}
\section{Shared Infrastructure Services}
\label{sec:orgcd377b1}

Goal:

Transform the cluster into a realistic HPC-style shared environment.

This phase includes:

\begin{itemize}
\item passwordless SSH
\item shared home directories
\item NFS server/client configuration
\item persistent mounts
\item multi-node shared filesystem verification
\end{itemize}

By the end of this phase:

\begin{itemize}
\item all nodes share same /home
\item users can access same files from all nodes
\item cluster behaves like a real HPC environment
\end{itemize}
\subsection{Cluster Role Architecture}
\label{sec:orgcb3848f}
\begin{enumerate}
\item Master Node
\label{sec:org33382f8}

Purpose:

\begin{itemize}
\item cluster management
\item NFS server
\item future Slurm controller
\item future Munge authority
\item future LDAP server
\end{itemize}

Services:

\begin{center}
\begin{tabular}{ll}
Service & Purpose\\
\hline
nfs-server & shared storage\\
future slurmctld & scheduler controller\\
future munge & authentication\\
\end{tabular}
\end{center}
\item Login Node
\label{sec:org4105395}

Purpose:

\begin{itemize}
\item user login
\item compilation
\item job submission
\end{itemize}

Users should:
\begin{itemize}
\item SSH into login node
\item compile programs
\item submit jobs using Slurm
\end{itemize}
\item Compute Nodes
\label{sec:org1b90fb5}

Purpose:

\begin{itemize}
\item execute jobs only
\end{itemize}

Future services:

\begin{center}
\begin{tabular}{ll}
Service & Purpose\\
\hline
slurmd & compute daemon\\
munge & authentication\\
\end{tabular}
\end{center}
\end{enumerate}
\subsection{Verify User Consistency}
\label{sec:orgd861a48}

IMPORTANT:

NFS shared storage requires same UID/GID across all nodes.

Verify user identity on all nodes:

\begin{verbatim}
id hpcadmin
\end{verbatim}

Expected:

\begin{verbatim}
uid=1000(hpcadmin) gid=1000(hpcadmin)
\end{verbatim}

All nodes must match.
\subsection{Passwordless SSH Configuration}
\label{sec:org38bc907}

Purpose:

\begin{itemize}
\item MPI communication
\item Slurm execution
\item cluster automation
\item remote management
\end{itemize}

SSH trust is generated ONLY from login node.

This mimics real HPC architecture:

\begin{verbatim}
User -> Login Node -> Compute Nodes
\end{verbatim}
\subsection{Generate SSH Key on Login Node}
\label{sec:orgee0e7d8}

SSH into login node from host:

\begin{verbatim}
ssh -p 2222 hpcadmin@localhost
\end{verbatim}

Generate SSH key:

\begin{verbatim}
ssh-keygen
\end{verbatim}

Press Enter for all prompts.

Generated files:

\begin{verbatim}
~/.ssh/id_rsa
~/.ssh/id_rsa.pub
\end{verbatim}
\subsection{Copy SSH Keys to Cluster Nodes}
\label{sec:orgbf47057}
\begin{enumerate}
\item Master
\label{sec:org2d55aea}

\begin{verbatim}
ssh-copy-id hpcadmin@master
\end{verbatim}
\item Compute01
\label{sec:orgc3428f0}

\begin{verbatim}
ssh-copy-id hpcadmin@compute01
\end{verbatim}
\item Compute02
\label{sec:org8f8ba13}

\begin{verbatim}
ssh-copy-id hpcadmin@compute02
\end{verbatim}
\item Compute03
\label{sec:orgce74d7b}

\begin{verbatim}
ssh-copy-id hpcadmin@compute03
\end{verbatim}
\end{enumerate}
\subsection{Verify Passwordless SSH}
\label{sec:orgceb047c}
\begin{enumerate}
\item Test Master
\label{sec:org60ced9b}

\begin{verbatim}
ssh master hostname
\end{verbatim}

Expected:

\begin{verbatim}
master.local
\end{verbatim}
\item Test Compute01
\label{sec:orgf326b4d}

\begin{verbatim}
ssh compute01 hostname
\end{verbatim}

Expected:

\begin{verbatim}
compute01.local
\end{verbatim}
\item Test Compute02
\label{sec:orge4ff429}

\begin{verbatim}
ssh compute02 hostname
\end{verbatim}
\item Test Compute03
\label{sec:orgae21010}

\begin{verbatim}
ssh compute03 hostname
\end{verbatim}

No password prompts should appear.
\end{enumerate}
\subsection{HPC Concept — Why Passwordless SSH Matters}
\label{sec:org2af335d}

Passwordless SSH becomes essential for:

\begin{center}
\begin{tabular}{ll}
Feature & Purpose\\
\hline
MPI & remote process launch\\
Slurm & distributed execution\\
automation & orchestration\\
monitoring & remote administration\\
\end{tabular}
\end{center}
\subsection{NFS Shared Storage Setup}
\label{sec:org0259b23}

Purpose:

Provide shared home directories across all cluster nodes.

This is a core HPC architecture requirement.

Without shared storage:

\begin{itemize}
\item source code differs per node
\item binaries differ
\item scripts unavailable across nodes
\item MPI execution becomes difficult
\end{itemize}

Real HPC systems commonly use:

\begin{itemize}
\item NFS
\item Lustre
\item BeeGFS
\item GPFS
\end{itemize}

This lab begins with NFS.
\subsection{Install NFS Packages}
\label{sec:orgc0cb0da}

Install on ALL nodes:

\begin{verbatim}
sudo dnf install -y nfs-utils
\end{verbatim}
\subsection{Configure NFS Server on Master}
\label{sec:orge778bc9}

SSH into master node.
\subsection{Enable NFS Services}
\label{sec:org9046aec}

\begin{verbatim}
sudo systemctl enable --now nfs-server
\end{verbatim}

Verify:

\begin{verbatim}
systemctl status nfs-server
\end{verbatim}

Expected:

\begin{verbatim}
active (running)
\end{verbatim}
\subsection{Configure NFS Exports}
\label{sec:org5c4db99}

Edit exports file:

\begin{verbatim}
sudo vim /etc/exports
\end{verbatim}

Add:

\begin{verbatim}
/home 10.10.10.0/24(rw,sync,no_root_squash)
\end{verbatim}
\subsection{Export Option Explanation}
\label{sec:org511b7f6}

\begin{center}
\begin{tabular}{ll}
Option & Meaning\\
\hline
rw & read/write access\\
sync & synchronous writes\\
no\_root\_squash & preserve root privileges\\
\end{tabular}
\end{center}
\subsection{Apply Exports}
\label{sec:org1ed0cac}

\begin{verbatim}
sudo exportfs -rav
\end{verbatim}
\subsection{Verify Exports}
\label{sec:orgca7148d}

\begin{verbatim}
sudo exportfs -v
\end{verbatim}

Expected export:

\begin{verbatim}
/home 10.10.10.0/24(...)
\end{verbatim}
\subsection{Configure NFS Clients}
\label{sec:org7a08022}

Perform on:

\begin{itemize}
\item login
\item compute01
\item compute02
\item compute03
\end{itemize}
\subsection{Backup Existing Home Directory}
\label{sec:orgf9faf5d}

\begin{verbatim}
sudo mv /home /home.local
\end{verbatim}

Purpose:

Preserve original local home directory.
\subsection{Create New Mount Point}
\label{sec:org04bc2da}

\begin{verbatim}
sudo mkdir /home
\end{verbatim}
\subsection{Mount Shared NFS Home}
\label{sec:org66a3cd6}

\begin{verbatim}
sudo mount master:/home /home
\end{verbatim}
\subsection{Verify Mount}
\label{sec:org241771c}

\begin{verbatim}
df -h
\end{verbatim}

Expected:

\begin{verbatim}
master:/home
\end{verbatim}

mounted on:

\begin{verbatim}
/home
\end{verbatim}
\subsection{Verify Shared Filesystem}
\label{sec:org5e7a84e}

From login node:

\begin{verbatim}
touch ~/shared_test
\end{verbatim}

From compute01:

\begin{verbatim}
ls ~
\end{verbatim}

Expected:

\begin{verbatim}
shared_test
\end{verbatim}

This confirms successful shared filesystem operation.
\subsection{Configure Persistent NFS Mounts}
\label{sec:org21e3621}

Without this configuration:

\begin{itemize}
\item NFS mount disappears after reboot
\end{itemize}
\subsection{Configure /etc/fstab}
\label{sec:orgdf5c609}

On ALL client nodes:

\begin{verbatim}
sudo vim /etc/fstab
\end{verbatim}

Add:

\begin{verbatim}
master:/home   /home   nfs   defaults   0 0
\end{verbatim}
\subsection{Verify fstab Before Reboot}
\label{sec:orgc2249b7}

IMPORTANT:

Always validate fstab before rebooting.

Run:

\begin{verbatim}
sudo mount -a
\end{verbatim}

If no errors appear:
\begin{itemize}
\item fstab configuration is correct
\end{itemize}
\subsection{Reboot Validation Procedure}
\label{sec:org55306bb}

Purpose:

Validate persistent cluster infrastructure after reboot.

This is important real-world HPC administration practice.
\subsection{Reboot Order}
\label{sec:orgd7d2d83}
\begin{enumerate}
\item Reboot Master First
\label{sec:orga03d261}

\begin{verbatim}
sudo reboot
\end{verbatim}

Wait until:
\begin{itemize}
\item SSH becomes available
\item NFS server active
\end{itemize}

Verify:

\begin{verbatim}
systemctl status nfs-server
\end{verbatim}
\item Reboot Login Node
\label{sec:org760e57c}

\begin{verbatim}
sudo reboot
\end{verbatim}
\item Reboot Compute Nodes
\label{sec:org07a97fb}

\begin{verbatim}
sudo reboot
\end{verbatim}

Perform for:
\begin{itemize}
\item compute01
\item compute02
\item compute03
\end{itemize}
\end{enumerate}
\subsection{Verify Persistent Mounts After Reboot}
\label{sec:org02e2e62}

On login node:

\begin{verbatim}
df -h
\end{verbatim}

Expected:

\begin{verbatim}
master:/home
\end{verbatim}

mounted automatically.
\subsection{Verify Active NFS Mount}
\label{sec:orgd6b9e97}

\begin{verbatim}
mount | grep home
\end{verbatim}

Expected:

\begin{verbatim}
master:/home on /home type nfs
\end{verbatim}
\subsection{Verify Shared Files After Reboot}
\label{sec:orgabb6e60}

\begin{verbatim}
ls ~
\end{verbatim}

Expected:
\begin{itemize}
\item previously created shared files still visible
\end{itemize}
\section{Munge Authentication + Slurm Scheduler}
\label{sec:orgf1f5c78}

Goal:

Transform the cluster into a scheduler-managed HPC environment.

This phase includes:

\begin{itemize}
\item Munge authentication
\item Slurm controller setup
\item Compute node daemon setup
\item Scheduler configuration
\item Partition creation
\item Multi-node execution
\item Resource allocation testing
\end{itemize}

By the end of this phase:

\begin{itemize}
\item cluster nodes authenticate securely
\item Slurm manages compute resources
\item jobs execute across nodes
\item scheduler topology becomes operational
\end{itemize}
\subsection{HPC Scheduler Architecture}
\label{sec:orga5c9d76}
\begin{enumerate}
\item Master Node
\label{sec:org3df5533}

Purpose:

\begin{itemize}
\item Slurm controller
\item cluster scheduler
\item resource manager
\item Munge authority
\end{itemize}

Services:

\begin{center}
\begin{tabular}{ll}
Service & Purpose\\
\hline
munged & cluster authentication\\
slurmctld & scheduler controller\\
\end{tabular}
\end{center}
\item Login Node
\label{sec:org0f23dfd}

Purpose:

\begin{itemize}
\item user login
\item compilation
\item job submission
\end{itemize}

Users should:

\begin{itemize}
\item login here
\item compile here
\item submit jobs here
\end{itemize}
\item Compute Nodes
\label{sec:orga234d66}

Purpose:

\begin{itemize}
\item execute jobs
\end{itemize}

Services:

\begin{center}
\begin{tabular}{ll}
Service & Purpose\\
\hline
munged & authentication\\
slurmd & compute execution daemon\\
\end{tabular}
\end{center}
\end{enumerate}
\subsection{IMPORTANT Rocky Linux 10 Issue}
\label{sec:org7d2337c}

Originally the cluster was created using:

\begin{verbatim}
Rocky Linux 10
\end{verbatim}

Problem encountered:

\begin{verbatim}
No match for argument: slurm
\end{verbatim}

Reason:

\begin{itemize}
\item Rocky Linux 10 HPC ecosystem still immature
\item Slurm packages unavailable in standard repos
\item OpenHPC EL10 support incomplete
\end{itemize}

Resolution:

\begin{itemize}
\item cluster rebuilt using Rocky Linux 9 Minimal
\end{itemize}

Important HPC lesson:

Production HPC clusters usually avoid bleeding-edge enterprise releases because:

\begin{itemize}
\item scheduler ecosystems lag behind
\item HPC repos may not exist
\item compatibility becomes unstable
\end{itemize}
\subsection{Install EPEL Repository}
\label{sec:org2135ccb}

Install on ALL nodes:

\begin{verbatim}
sudo dnf install -y epel-release
\end{verbatim}
\subsection{Enable CRB Repository}
\label{sec:orgb318939}

Required on Rocky Linux 9 and 10.

Install on ALL nodes:

\begin{verbatim}
sudo crb enable
\end{verbatim}
\subsection{Install Munge + Slurm Packages}
\label{sec:org1400bde}

Install on ALL nodes:

\begin{verbatim}
sudo dnf install -y \
munge munge-libs \
slurm slurm-slurmd slurm-slurmctld
\end{verbatim}
\subsection{Verify Installed Packages}
\label{sec:org996442a}
\begin{enumerate}
\item Verify Munge
\label{sec:org171964d}

\begin{verbatim}
rpm -qa | grep munge
\end{verbatim}

Expected:

\begin{verbatim}
munge
munge-libs
\end{verbatim}
\item Verify Slurm
\label{sec:orge5b965c}

\begin{verbatim}
rpm -qa | grep slurm
\end{verbatim}

Expected:

\begin{verbatim}
slurm
slurm-slurmd
slurm-slurmctld
\end{verbatim}
\end{enumerate}
\subsection{Verify System Users}
\label{sec:orgb6ab0c0}
\begin{enumerate}
\item Verify Munge User
\label{sec:org758b5c5}

\begin{verbatim}
id munge
\end{verbatim}
\item Verify Slurm User
\label{sec:org94453d4}

\begin{verbatim}
id slurm
\end{verbatim}
\end{enumerate}
\subsection{IMPORTANT Issue Encountered — Missing slurm User}
\label{sec:org2153c69}

Problem:

\begin{verbatim}
id: 'slurm': no such user
\end{verbatim}

Reason:

Some Slurm packages on Rocky Linux do not automatically create the slurm user.

Resolution:

Create manually on ALL nodes.
\subsection{Create Slurm Group}
\label{sec:org4e36c2f}

\begin{verbatim}
sudo groupadd slurm
\end{verbatim}
\subsection{Create Slurm User}
\label{sec:orgfdbf49b}

\begin{verbatim}
sudo useradd -r -g slurm -s /sbin/nologin slurm
\end{verbatim}

Explanation:

\begin{center}
\begin{tabular}{ll}
Option & Meaning\\
\hline
-r & system account\\
-g slurm & primary group\\
-s /sbin/nologin & disable shell login\\
\end{tabular}
\end{center}
\subsection{Verify Slurm User}
\label{sec:org13810b9}

\begin{verbatim}
id slurm
\end{verbatim}

Expected:

\begin{verbatim}
uid=...
gid=...
\end{verbatim}
\subsection{IMPORTANT HPC Security Concept}
\label{sec:org8e20c9b}

Cluster daemons should not run permanently as root.

Therefore:

\begin{itemize}
\item Munge runs as munge user
\item Slurm runs as slurm user
\end{itemize}

This is standard enterprise daemon security practice.
\subsection{Configure Munge Authentication}
\label{sec:org4630901}

Purpose:

Create secure trust relationship between all cluster nodes.

IMPORTANT:

All nodes MUST use the same:

\begin{verbatim}
/etc/munge/munge.key
\end{verbatim}

This key acts as the cluster-wide shared secret.
\subsection{Generate Munge Key on Master}
\label{sec:orgfe5f43c}

ONLY on master node:

\begin{verbatim}
sudo /usr/sbin/create-munge-key
\end{verbatim}
\subsection{Verify Munge Key}
\label{sec:orgebc3dd3}

\begin{verbatim}
ls -l /etc/munge/munge.key
\end{verbatim}

Expected:

\begin{verbatim}
-rw-------
\end{verbatim}

owned by:

\begin{verbatim}
munge munge
\end{verbatim}
\subsection{IMPORTANT Mistake Encountered — Munge Key Generated on All Nodes}
\label{sec:orgb1ddcfc}

Problem:

Munge key accidentally generated independently on all nodes.

Result:

Nodes could not authenticate each other.

Important HPC lesson:

Munge works using a:

\begin{verbatim}
shared trust model
\end{verbatim}

All nodes MUST share identical munge.key.
\subsection{Correct Munge Key Distribution}
\label{sec:org98e1596}
\begin{enumerate}
\item Copy Key Temporarily
\label{sec:org24c7890}

On master:

\begin{verbatim}
sudo cp /etc/munge/munge.key /tmp/
\end{verbatim}
\item Temporary Permission Adjustment
\label{sec:org35cd46b}

\begin{verbatim}
sudo chmod 644 /tmp/munge.key
\end{verbatim}

Purpose:

Allow normal user SCP transfer.
\end{enumerate}
\subsection{IMPORTANT SSH Root Login Issue}
\label{sec:org9a8e04a}

Problem encountered:

\begin{verbatim}
Permission denied (publickey,gssapi-keyex,gssapi-with-mic,password)
\end{verbatim}

Reason:

Using:

\begin{verbatim}
sudo scp ...
\end{verbatim}

attempted root SSH login.

Rocky Linux disables direct root SSH login by default.

Resolution:

Use normal user SCP.
\subsection{Copy Munge Key to Login Node}
\label{sec:orge9e7605}

\begin{verbatim}
scp /tmp/munge.key login:/tmp/
\end{verbatim}
\subsection{Copy Munge Key to Compute01}
\label{sec:org6a14a87}

\begin{verbatim}
scp /tmp/munge.key compute01:/tmp/
\end{verbatim}
\subsection{Copy Munge Key to Compute02}
\label{sec:org45b2432}

\begin{verbatim}
scp /tmp/munge.key compute02:/tmp/
\end{verbatim}
\subsection{Copy Munge Key to Compute03}
\label{sec:org09666f3}

\begin{verbatim}
scp /tmp/munge.key compute03:/tmp/
\end{verbatim}
\subsection{Remove Temporary Key Copy}
\label{sec:orgc3e5142}

On master:

\begin{verbatim}
sudo rm -f /tmp/munge.key
\end{verbatim}
\subsection{Install Munge Key on Client Nodes}
\label{sec:orgb17f823}

Perform on:

\begin{itemize}
\item login
\item compute01
\item compute02
\item compute03
\end{itemize}
\subsection{Stop Munge Service}
\label{sec:org900caa0}

\begin{verbatim}
sudo systemctl stop munge
\end{verbatim}
\subsection{Install Key}
\label{sec:org8da6a6a}

\begin{verbatim}
sudo mv /tmp/munge.key /etc/munge/
\end{verbatim}
\subsection{Fix Ownership}
\label{sec:orge0dde17}

\begin{verbatim}
sudo chown munge:munge /etc/munge/munge.key
\end{verbatim}
\subsection{Fix Permissions}
\label{sec:org37527aa}

\begin{verbatim}
sudo chmod 400 /etc/munge/munge.key
\end{verbatim}
\subsection{Start Munge}
\label{sec:org6034b26}

On ALL nodes:

\begin{verbatim}
sudo systemctl enable --now munge
\end{verbatim}
\subsection{IMPORTANT Issue Encountered — Munge Not Running on Master}
\label{sec:orgc95adac}

Problem:

\begin{verbatim}
munge: Error: Failed to access "/var/run/munge/munge.socket.2"
\end{verbatim}

Reason:

Munge service never started on master.

Resolution:

\begin{verbatim}
sudo systemctl enable --now munge
\end{verbatim}

Important lesson:

Difference between:

\begin{center}
\begin{tabular}{ll}
State & Meaning\\
\hline
inactive & service not started\\
failed & service crashed\\
\end{tabular}
\end{center}
\subsection{Verify Munge Service}
\label{sec:org8372f96}

\begin{verbatim}
systemctl status munge
\end{verbatim}

Expected:

\begin{verbatim}
active (running)
\end{verbatim}
\subsection{Verify Munge Authentication}
\label{sec:org542a811}

From master:

\begin{verbatim}
munge -n | ssh compute01 unmunge
\end{verbatim}
\subsection{Test Compute02}
\label{sec:orgc88c3fc}

\begin{verbatim}
munge -n | ssh compute02 unmunge
\end{verbatim}
\subsection{Test Compute03}
\label{sec:orgab9ba55}

\begin{verbatim}
munge -n | ssh compute03 unmunge
\end{verbatim}
\subsection{Test Login Node}
\label{sec:org3566f51}

\begin{verbatim}
munge -n | ssh login unmunge
\end{verbatim}

Expected:

\begin{verbatim}
STATUS: Success
\end{verbatim}
\subsection{HPC Concepts Learned}
\label{sec:org9b3371f}

Munge introduced:

\begin{itemize}
\item cluster-wide authentication
\item distributed trust model
\item shared secret authentication
\item daemon security
\item inter-node trust validation
\end{itemize}
\subsection{Configure Slurm Scheduler}
\label{sec:org0b7c735}

Purpose:

Enable centralized resource scheduling.

IMPORTANT:

\begin{verbatim}
/etc/slurm/slurm.conf
\end{verbatim}

must be IDENTICAL on ALL nodes.
\subsection{IMPORTANT Issue Encountered — Default localhost Configuration}
\label{sec:org3bd46f6}

Problem:

\begin{verbatim}
sinfo
-> localhost
\end{verbatim}

Reason:

Default example slurm.conf remained active.

Resolution:

Replace configuration with custom cluster topology.

Important HPC lesson:

Incorrect topology configuration is one of the most common Slurm admin problems.
\subsection{Create Slurm Configuration}
\label{sec:org27f8ccb}

On master:

\begin{verbatim}
sudo vim /etc/slurm/slurm.conf
\end{verbatim}

Replace ENTIRE file with:

\begin{verbatim}
ClusterName=hpccluster
SlurmctldHost=master

MpiDefault=none
ProctrackType=proctrack/cgroup
ReturnToService=2

SlurmctldPidFile=/run/slurmctld.pid
SlurmdPidFile=/run/slurmd.pid

SlurmdSpoolDir=/var/spool/slurmd
StateSaveLocation=/var/spool/slurmctld

SlurmUser=slurm

SwitchType=switch/none
TaskPlugin=task/affinity

SchedulerType=sched/backfill

SelectType=select/cons_tres
SelectTypeParameters=CR_Core

AuthType=auth/munge

NodeName=compute01 CPUs=2 State=UNKNOWN
NodeName=compute02 CPUs=2 State=UNKNOWN
NodeName=compute03 CPUs=2 State=UNKNOWN

PartitionName=debug Nodes=ALL Default=YES MaxTime=INFINITE State=UP
\end{verbatim}
\subsection{Configuration Explanation}
\label{sec:org8f9315f}

\begin{center}
\begin{tabular}{ll}
Parameter & Purpose\\
\hline
ClusterName & cluster identifier\\
SlurmctldHost & controller node\\
NodeName & compute nodes\\
CPUs=2 & available CPUs\\
PartitionName & scheduler queue\\
\end{tabular}
\end{center}
\subsection{Create Slurm Runtime Directories}
\label{sec:org8091a8b}
\begin{enumerate}
\item On Master
\label{sec:org2a60bcd}

\begin{verbatim}
sudo mkdir -p /var/spool/slurmctld
\end{verbatim}

\begin{verbatim}
sudo mkdir -p /var/spool/slurmd
\end{verbatim}
\end{enumerate}
\subsection{Set Ownership}
\label{sec:org2c1576f}

\begin{verbatim}
sudo chown slurm:slurm /var/spool/slurmctld
\end{verbatim}

\begin{verbatim}
sudo chown slurm:slurm /var/spool/slurmd
\end{verbatim}
\subsection{Create slurmd Directories on Compute Nodes}
\label{sec:orgbe7b17a}
\begin{enumerate}
\item Compute01
\label{sec:orgd49f16b}

\begin{verbatim}
ssh compute01 sudo -S mkdir -p /var/spool/slurmd
\end{verbatim}
\item Compute02
\label{sec:org8c36984}

\begin{verbatim}
ssh compute02 sudo -S mkdir -p /var/spool/slurmd
\end{verbatim}
\item Compute03
\label{sec:orgb013b24}

\begin{verbatim}
ssh compute03 sudo -S mkdir -p /var/spool/slurmd
\end{verbatim}
\end{enumerate}
\subsection{IMPORTANT sudo Over SSH Issue}
\label{sec:org2e2cb1f}

Problem:

\begin{verbatim}
sudo: a terminal is required to read the password
\end{verbatim}

Resolution:

Use:

\begin{verbatim}
sudo -S
\end{verbatim}

Important Linux administration concept:

sudo over non-interactive SSH requires stdin password handling.
\subsection{Fix Ownership on Compute Nodes}
\label{sec:org4d1b09c}
\begin{enumerate}
\item Compute01
\label{sec:orga26893d}

\begin{verbatim}
ssh compute01 sudo -S chown slurm:slurm /var/spool/slurmd
\end{verbatim}
\item Compute02
\label{sec:org45f8edf}

\begin{verbatim}
ssh compute02 sudo -S chown slurm:slurm /var/spool/slurmd
\end{verbatim}
\item Compute03
\label{sec:orgf30dc7b}

\begin{verbatim}
ssh compute03 sudo -S chown slurm:slurm /var/spool/slurmd
\end{verbatim}
\end{enumerate}
\subsection{Verify Runtime Directories}
\label{sec:orga1ed69a}

\begin{verbatim}
ls -ld /var/spool/slurmctld
\end{verbatim}

Expected:

\begin{verbatim}
slurm slurm
\end{verbatim}
\subsection{Distribute slurm.conf}
\label{sec:orgfb197dc}
\begin{enumerate}
\item Login
\label{sec:orgc606672}

\begin{verbatim}
scp /etc/slurm/slurm.conf login:/tmp/
\end{verbatim}
\item Compute01
\label{sec:org38e5e53}

\begin{verbatim}
scp /etc/slurm/slurm.conf compute01:/tmp/
\end{verbatim}
\item Compute02
\label{sec:org5338fa3}

\begin{verbatim}
scp /etc/slurm/slurm.conf compute02:/tmp/
\end{verbatim}
\item Compute03
\label{sec:orgf846e5a}

\begin{verbatim}
scp /etc/slurm/slurm.conf compute03:/tmp/
\end{verbatim}
\end{enumerate}
\subsection{Install Configuration on Nodes}
\label{sec:orga549d1c}

On ALL nodes:

\begin{verbatim}
sudo mv /tmp/slurm.conf /etc/slurm/slurm.conf
\end{verbatim}
\subsection{Start Slurm Controller}
\label{sec:org109d402}

ONLY on master:

\begin{verbatim}
sudo systemctl enable --now slurmctld
\end{verbatim}
\subsection{Verify Controller}
\label{sec:org85bcb0a}

\begin{verbatim}
systemctl status slurmctld
\end{verbatim}

Expected:

\begin{verbatim}
active (running)
\end{verbatim}
\subsection{Start slurmd on Compute Nodes}
\label{sec:org223e182}

On:
\begin{itemize}
\item compute01
\item compute02
\item compute03
\end{itemize}

\begin{verbatim}
sudo systemctl enable --now slurmd
\end{verbatim}
\subsection{Verify slurmd}
\label{sec:orge49ae4a}

\begin{verbatim}
systemctl status slurmd
\end{verbatim}

Expected:

\begin{verbatim}
active (running)
\end{verbatim}
\subsection{Reconfigure Slurm}
\label{sec:orgffe7c00}

On master:

\begin{verbatim}
sudo scontrol reconfigure
\end{verbatim}
\subsection{IMPORTANT Issue Encountered — Nodes in DRAIN State}
\label{sec:orgeaa7a40}

Problem:

Nodes entered:

\begin{verbatim}
DRAIN
\end{verbatim}

after CPU topology changes.

Reason:

Slurm detected mismatch between:
\begin{itemize}
\item configured CPUs
\item detected hardware topology
\end{itemize}

Resolution:

Clear stale slurmd state.
\subsection{Stop slurmd}
\label{sec:orgc734b06}

On compute nodes:

\begin{verbatim}
sudo systemctl stop slurmd
\end{verbatim}
\subsection{Remove Old State}
\label{sec:orge8a42a1}

\begin{verbatim}
sudo rm -rf /var/spool/slurmd/*
\end{verbatim}
\subsection{Restart slurmd}
\label{sec:org9ec00ab}

\begin{verbatim}
sudo systemctl start slurmd
\end{verbatim}
\subsection{Resume Nodes}
\label{sec:orgd1563a5}

On master:

\begin{verbatim}
sudo scontrol update NodeName=compute01 State=RESUME
\end{verbatim}

\begin{verbatim}
sudo scontrol update NodeName=compute02 State=RESUME
\end{verbatim}

\begin{verbatim}
sudo scontrol update NodeName=compute03 State=RESUME
\end{verbatim}
\subsection{Verify Cluster Status}
\label{sec:org9885070}

\begin{verbatim}
sinfo
\end{verbatim}

Expected:

\begin{verbatim}
PARTITION AVAIL TIMELIMIT NODES STATE NODELIST
debug* up infinite 3 idle compute[01-03]
\end{verbatim}
\subsection{Verify Node Details}
\label{sec:org9a65ef9}

\begin{verbatim}
scontrol show nodes
\end{verbatim}

This displays:

\begin{itemize}
\item CPU information
\item memory
\item node states
\item scheduler topology
\item resource allocation
\end{itemize}
\subsection{Test Slurm Execution}
\label{sec:org9e4c1af}
\begin{enumerate}
\item Single Node Test
\label{sec:org49f0bbc}

From login node:

\begin{verbatim}
srun hostname
\end{verbatim}

Expected:

\begin{verbatim}
compute01.local
\end{verbatim}
\end{enumerate}
\subsection{Multi-node Execution}
\label{sec:org5235cbf}

\begin{verbatim}
srun -N 3 hostname
\end{verbatim}

Expected:

\begin{verbatim}
compute01.local
compute02.local
compute03.local
\end{verbatim}
\subsection{Multi-task Multi-node Execution}
\label{sec:org04f15cc}

\begin{verbatim}
srun -N 3 -n 6 hostname
\end{verbatim}

This verifies:

\begin{itemize}
\item task distribution
\item multi-node scheduling
\item CPU allocation
\end{itemize}
\subsection{Multi-core Single-node Execution}
\label{sec:orgdd5ef6f}

\begin{verbatim}
srun -N 1 -n 2 hostname
\end{verbatim}

Expected:

\begin{verbatim}
compute01.local
compute01.local
\end{verbatim}
\subsection{IMPORTANT Slurm Scheduling Concept}
\label{sec:org4316dbe}

Earlier:

\begin{verbatim}
srun -N 1 -n 2 hostname
\end{verbatim}

failed because nodes initially had:

\begin{verbatim}
CPUs=1
\end{verbatim}

After increasing VM CPUs and updating slurm.conf:

\begin{verbatim}
CPUs=2
\end{verbatim}

the scheduler successfully allocated 2 tasks on a single node.

Important HPC lesson:

Slurm strictly enforces:
\begin{itemize}
\item CPU limits
\item task allocation
\item topology constraints
\item resource consistency
\end{itemize}
\section{Shared Software Filesystem (/apps)}
\label{sec:org1befa66}

Goal:

Create a dedicated shared software filesystem for:

\begin{itemize}
\item Spack
\item compilers
\item MPI stacks
\item scientific applications
\item modulefiles
\item HPC software hierarchy
\end{itemize}

This phase transitions the cluster from:

\begin{verbatim}
basic scheduler infrastructure
\end{verbatim}

to:

\begin{verbatim}
realistic HPC software environment
\end{verbatim}
\subsection{Why Separate /apps Filesystem?}
\label{sec:org8b0e723}

Initially:

\begin{itemize}
\item each VM had a 30 GB local disk
\item /home was exported from master over NFS
\end{itemize}

Concern encountered:

\begin{verbatim}
Shared /home only had ~30 GB available.
Spack and HPC software stacks can become very large.
\end{verbatim}

Important HPC storage concept:

Real HPC clusters usually separate:

\begin{center}
\begin{tabular}{ll}
Filesystem & Purpose\\
\hline
/home & user files\\
/apps & software stack\\
/scratch & temporary runtime data\\
/archive & long-term storage\\
\end{tabular}
\end{center}

Therefore a dedicated:

\begin{verbatim}
/apps
\end{verbatim}

filesystem was created.

This is significantly closer to real HPC architecture.
\subsection{IMPORTANT Storage Architecture Concept}
\label{sec:orgafb3aea}

Each VM still has its own independent local storage.

Example:

\begin{center}
\begin{tabular}{ll}
Node & Local Disk\\
\hline
master & 30 GB\\
login & 30 GB\\
compute01 & 30 GB\\
compute02 & 30 GB\\
compute03 & 30 GB\\
\end{tabular}
\end{center}

When:

\begin{verbatim}
master:/home
\end{verbatim}

was mounted via NFS:

\begin{itemize}
\item compute node local /home became hidden
\item but local disks still exist
\end{itemize}

Only the exported filesystem becomes shared.

Therefore:

\begin{verbatim}
/apps
\end{verbatim}

was created as a dedicated shared software layer.
\subsection{Add New Virtual Disk to Master}
\label{sec:org279b99a}

A second virtual disk was attached ONLY to:

\begin{verbatim}
master
\end{verbatim}

Purpose:

\begin{itemize}
\item centralized software storage
\item shared application filesystem
\item Spack software stack
\end{itemize}

Disk size selected:

\begin{verbatim}
60 GB
\end{verbatim}

Disk type:

\begin{verbatim}
VDI (Dynamically Allocated)
\end{verbatim}
\subsection{Verify New Disk Detection}
\label{sec:org473227a}

After booting master:

\begin{verbatim}
lsblk
\end{verbatim}

Expected:

\begin{verbatim}
sda    30G
sdb    60G
\end{verbatim}

Important:

\begin{center}
\begin{tabular}{ll}
Device & Purpose\\
\hline
sda & OS disk\\
sdb & New apps disk\\
\end{tabular}
\end{center}

Only:

\begin{verbatim}
sdb
\end{verbatim}

was modified.
\subsection{Create GPT Partition Table}
\label{sec:org6d8ece8}

\begin{verbatim}
sudo fdisk /dev/sdb
\end{verbatim}

Inside fdisk:

Create GPT label:

\begin{verbatim}
g
\end{verbatim}

Create partition:

\begin{verbatim}
n
\end{verbatim}

Accept defaults.

Write changes:

\begin{verbatim}
w
\end{verbatim}
\subsection{Verify Partition}
\label{sec:orgbedce2f}

\begin{verbatim}
lsblk
\end{verbatim}

Expected:

\begin{verbatim}
sdb
└── sdb1
\end{verbatim}
\subsection{Create XFS Filesystem}
\label{sec:org1c69fa5}

\begin{verbatim}
sudo mkfs.xfs /dev/sdb1
\end{verbatim}

Why XFS?

\begin{itemize}
\item default enterprise filesystem
\item highly scalable
\item excellent for large filesystems
\item commonly used in HPC
\end{itemize}
\subsection{Create Mount Point}
\label{sec:orgfd94871}

\begin{verbatim}
sudo mkdir -p /apps
\end{verbatim}
\subsection{Get Filesystem UUID}
\label{sec:org536be65}

\begin{verbatim}
sudo blkid /dev/sdb1
\end{verbatim}

Example:

\begin{verbatim}
UUID="xxxxxxxx-xxxx-xxxx"
\end{verbatim}
\subsection{Configure Persistent Mount}
\label{sec:org595ebf5}

Edit:

\begin{verbatim}
sudo vim /etc/fstab
\end{verbatim}

Add:

\begin{verbatim}
UUID=YOUR_UUID_HERE   /apps   xfs   defaults   0 0
\end{verbatim}

Important Linux administration concept:

UUIDs are preferred over:

\begin{verbatim}
/dev/sdb1
\end{verbatim}

because device names can change after reboot.
\subsection{Mount Filesystem}
\label{sec:org92c611c}

\begin{verbatim}
sudo mount -a
\end{verbatim}
\subsection{Verify Mount}
\label{sec:org1afee73}

\begin{verbatim}
df -h
\end{verbatim}

Expected:

\begin{verbatim}
/apps
\end{verbatim}

mounted with approximately:

\begin{verbatim}
60 GB
\end{verbatim}
\subsection{Configure Ownership}
\label{sec:orgc411bc7}

\begin{verbatim}
sudo chown hpcadmin:hpcadmin /apps
\end{verbatim}
\subsection{Create Shared HPC Software Hierarchy}
\label{sec:org3b3bd8b}

\begin{verbatim}
mkdir -p /apps/{spack,modulefiles,software,sources}
\end{verbatim}

Resulting structure:

\begin{verbatim}
/apps
├── spack
├── modulefiles
├── software
└── sources
\end{verbatim}
\subsection{HPC Software Hierarchy Concept}
\label{sec:orgc2fcd19}

Purpose of directories:

\begin{center}
\begin{tabular}{ll}
Directory & Purpose\\
\hline
spack & Spack package manager\\
modulefiles & custom modulefiles\\
software & installed software\\
sources & downloaded sources\\
\end{tabular}
\end{center}

This is similar to production HPC software layouts.
\subsection{Export /apps via NFS}
\label{sec:org2ade23b}

Edit exports file:

\begin{verbatim}
sudo vim /etc/exports
\end{verbatim}

Add:

\begin{verbatim}
/apps 10.10.10.0/24(rw,sync,no_root_squash)
\end{verbatim}
\subsection{Apply NFS Exports}
\label{sec:org9c9c8cc}

\begin{verbatim}
sudo exportfs -rav
\end{verbatim}
\subsection{Verify Exports}
\label{sec:org3e475f7}

\begin{verbatim}
showmount -e localhost
\end{verbatim}

Expected:

\begin{verbatim}
/apps
/home
\end{verbatim}
\subsection{Create Mount Points on Client Nodes}
\label{sec:org8c5db25}

On:
\begin{itemize}
\item login
\item compute01
\item compute02
\item compute03
\end{itemize}

Create:

\begin{verbatim}
sudo mkdir -p /apps
\end{verbatim}
\subsection{Configure Persistent NFS Mounts}
\label{sec:orgf10933e}

Edit:

\begin{verbatim}
sudo vim /etc/fstab
\end{verbatim}

Add:

\begin{verbatim}
master:/apps   /apps   nfs   defaults,_netdev   0 0
\end{verbatim}
\subsection{Mount Shared /apps Filesystem}
\label{sec:org0ea59ad}

\begin{verbatim}
sudo mount -a
\end{verbatim}
\subsection{Verify Shared Mount}
\label{sec:org1596133}

\begin{verbatim}
df -h
\end{verbatim}

Expected:

\begin{verbatim}
master:/apps
\end{verbatim}

mounted on all nodes.
\subsection{Verify Shared Filesystem Functionality}
\label{sec:org8c2c7d3}

On master:

\begin{verbatim}
touch /apps/testfile
\end{verbatim}

On compute node:

\begin{verbatim}
ls /apps
\end{verbatim}

Expected:

\begin{verbatim}
testfile
\end{verbatim}

This verifies:

\begin{itemize}
\item shared software filesystem
\item NFS functionality
\item cluster-wide software visibility
\end{itemize}
\subsection{HPC Architecture After /apps Setup}
\label{sec:orgbd76616}

Current cluster storage architecture:

\begin{center}
\begin{tabular}{ll}
Filesystem & Purpose\\
\hline
/home & user files\\
/apps & software stack\\
local disks & local runtime storage\\
\end{tabular}
\end{center}

This resembles real HPC environments significantly more closely.
\section{Spack Software Stack + MPI Runtime Validation}
\label{sec:org39005cc}

Goal:

Create centralized HPC software stack using:

\begin{itemize}
\item Spack
\item shared software filesystem
\item shared compilers
\item shared MPI stack
\item scheduler-aware runtime execution
\end{itemize}

This phase validates:

\begin{itemize}
\item software distribution
\item multinode execution
\item Slurm integration
\item MPI runtime functionality
\end{itemize}

After completing this section you will be able to:

\begin{itemize}
\item install HPC software using Spack
\item manage multiple compiler versions
\item manage multiple MPI versions
\item load software dynamically
\item compile MPI applications
\item execute distributed jobs using Slurm
\item monitor and debug jobs
\item understand performance metrics
\end{itemize}
\subsection{IMPORTANT HPC Software Stack Concept}
\label{sec:org84e5280}

Instead of:

\begin{verbatim}
dnf install openmpi
\end{verbatim}

a centralized HPC software stack was created.

Benefits:

\begin{itemize}
\item software installed once
\item accessible cluster-wide
\item compiler hierarchy support
\item reproducible builds
\item module-based software environments
\item centralized software management
\end{itemize}

This approach is significantly closer to real HPC centers.

In production HPC environments:

\begin{itemize}
\item operating system packages are separated
\item scientific software stack is separated
\item compiler stacks are separated
\item MPI stacks are separated
\end{itemize}

This separation improves:

\begin{itemize}
\item portability
\item reproducibility
\item scalability
\item maintenance
\end{itemize}
\subsection{IMPORTANT Shared Filesystem Concept}
\label{sec:orga8eac24}

The cluster uses:

\begin{verbatim}
/apps
\end{verbatim}

as shared application storage.

Because:

\begin{verbatim}
/apps
\end{verbatim}

is mounted cluster-wide through NFS:

\begin{itemize}
\item software installed once becomes visible everywhere
\item compute nodes automatically access installed software
\item users share common software stack
\item recompilation is unnecessary on compute nodes
\end{itemize}

This is one of the MOST important HPC architecture concepts.
\subsection{Install Development Tools}
\label{sec:org0491b86}

Installed manually on ALL nodes:

\begin{verbatim}
sudo dnf groupinstall -y "Development Tools"
\end{verbatim}

Purpose:

Provides:

\begin{itemize}
\item gcc
\item g++
\item gfortran
\item make
\item linker tools
\item build utilities
\item development environment
\end{itemize}

Important note:

\begin{verbatim}
System compilers are still required even when using Spack.
\end{verbatim}

Reason:

Spack builds software from source code and therefore still requires:

\begin{itemize}
\item assembler
\item linker
\item basic compiler toolchain
\end{itemize}
\subsection{Move into Shared Application Filesystem}
\label{sec:org4e3b0c1}

Run on:

\begin{verbatim}
master
\end{verbatim}

Move into shared applications directory:

\begin{verbatim}
cd /apps
\end{verbatim}

Verify:

\begin{verbatim}
pwd
\end{verbatim}

Expected:

\begin{verbatim}
/apps
\end{verbatim}
\subsection{Clone Spack into Shared Filesystem}
\label{sec:orgb1d1d15}

Clone Spack:

\begin{verbatim}
git clone https://github.com/spack/spack.git
\end{verbatim}

Expected:

\begin{verbatim}
/apps/spack
\end{verbatim}

This creates centralized package manager installation.
\subsection{Enter Spack Directory}
\label{sec:orgb67f5bc}

\begin{verbatim}
cd spack
\end{verbatim}

Verify:

\begin{verbatim}
pwd
\end{verbatim}

Expected:

\begin{verbatim}
/apps/spack
\end{verbatim}
\subsection{Initialize Spack Environment}
\label{sec:orgd74c894}

Initialize Spack:

\begin{verbatim}
source share/spack/setup-env.sh
\end{verbatim}

Purpose:

Adds:

\begin{itemize}
\item spack command
\item shell integration
\item environment variables
\item package management environment
\end{itemize}

to current shell session.

Without initialization:

\begin{itemize}
\item spack command will not work
\item spack load will fail
\item software environments will not activate
\end{itemize}
\subsection{IMPORTANT Shell Environment Concept}
\label{sec:org27b3d66}

\begin{verbatim}
source
\end{verbatim}

does NOT execute script normally.

Instead:

\begin{verbatim}
it modifies current shell environment
\end{verbatim}

This is extremely important in:

\begin{itemize}
\item Spack
\item modules
\item MPI environments
\item compiler environments
\end{itemize}
\subsection{Make Spack Permanent}
\label{sec:org3f0558f}

To avoid sourcing manually every session:

Edit:

\begin{verbatim}
vim ~/.bashrc
\end{verbatim}

Add:

\begin{verbatim}
source /apps/spack/share/spack/setup-env.sh
\end{verbatim}

Reload shell:

\begin{verbatim}
source ~/.bashrc
\end{verbatim}

Verify:

\begin{verbatim}
spack --version
\end{verbatim}

Expected:
\begin{itemize}
\item Spack version information
\end{itemize}
\subsection{Clone Additional Spack Repository}
\label{sec:orgf2271f5}

Clone repository:

\begin{verbatim}
git clone https://github.com/spack/spack-packages.git
\end{verbatim}

Purpose:

Adds additional package definitions.
\subsection{Configure Spack Repository}
\label{sec:org12becf3}

\begin{verbatim}
spack repo set --destination "$(pwd)/spack-packages" builtin
\end{verbatim}

Purpose:

Configure additional repository support.

This allows:

\begin{itemize}
\item package metadata
\item package recipes
\item software definitions
\end{itemize}

to become visible to Spack.
\subsection{Verify Spack Functionality}
\label{sec:orgccddae1}

\begin{verbatim}
spack info gcc
\end{verbatim}

Expected:

Detailed GCC package information.

This verifies:

\begin{itemize}
\item Spack environment working
\item package repository functional
\item package metadata accessible
\end{itemize}
\subsection{IMPORTANT HPC Software Stack Concept}
\label{sec:orga6d8c40}

Spack operates as:

\begin{verbatim}
source-based HPC package manager
\end{verbatim}

Unlike:

\begin{center}
\begin{tabular}{ll}
Package Manager & Purpose\\
\hline
dnf & operating system\\
Spack & HPC software ecosystem\\
\end{tabular}
\end{center}

Production HPC systems commonly separate:

\begin{itemize}
\item OS packages
\item scientific software stacks
\end{itemize}

exactly like this setup.
\subsection{IMPORTANT Spack Build Concept}
\label{sec:org5a233e5}

When installing software using Spack:

\begin{verbatim}
software is compiled from source
\end{verbatim}

Advantages:

\begin{itemize}
\item architecture optimization
\item compiler flexibility
\item dependency control
\item reproducibility
\item variant support
\end{itemize}

Disadvantages:

\begin{itemize}
\item longer installation time
\end{itemize}
\subsection{Detect Available Compilers}
\label{sec:org3bef4ea}

Before installing software:

\begin{verbatim}
spack compiler find
\end{verbatim}

Purpose:

Detect system compilers available for building software.

Verify detected compilers:

\begin{verbatim}
spack compilers
\end{verbatim}

Expected:
\begin{itemize}
\item compiler list
\end{itemize}
\subsection{Install GCC Using Spack}
\label{sec:org902ab69}

Install GCC version:

\begin{verbatim}
spack install gcc@13.4.0
\end{verbatim}

Purpose:

Create dedicated HPC compiler stack.

This may take considerable time because:

\begin{itemize}
\item GCC itself is compiled from source
\item dependencies are built
\item optimization occurs
\end{itemize}
\subsection{IMPORTANT Multiple Compiler Version Concept}
\label{sec:orgc928bd5}

Production HPC systems frequently maintain multiple compiler versions simultaneously.

Example:

\begin{verbatim}
gcc@11
gcc@12
gcc@13
intel
nvhpc
clang
\end{verbatim}

Reasons:

\begin{itemize}
\item application compatibility
\item compiler ABI differences
\item reproducibility
\item benchmark consistency
\end{itemize}
\subsection{Verify Installed Software}
\label{sec:org08d7368}

List installed packages:

\begin{verbatim}
spack find
\end{verbatim}

Expected example:

\begin{verbatim}
==> 1 installed package
gcc@13.4.0
\end{verbatim}
\subsection{Load Installed GCC Compiler}
\label{sec:orga44e938}

\begin{verbatim}
spack load gcc@13.4.0
\end{verbatim}

Purpose:

Activate compiler environment.

Verify compiler path:

\begin{verbatim}
which gcc
\end{verbatim}

Expected:
\begin{itemize}
\item Spack GCC path
\end{itemize}

Verify version:

\begin{verbatim}
gcc --version
\end{verbatim}

Expected:
\begin{itemize}
\item GCC 13.4.0
\end{itemize}
\subsection{IMPORTANT Runtime Environment Concept}
\label{sec:org7b33dcd}

When software is loaded:

\begin{itemize}
\item PATH changes
\item LD\_LIBRARY\_PATH changes
\item MANPATH changes
\end{itemize}

This allows applications to locate:

\begin{itemize}
\item executables
\item shared libraries
\item runtime dependencies
\end{itemize}

automatically.
\subsection{Register Newly Installed Compiler}
\label{sec:org559f641}

After installing GCC:

\begin{verbatim}
spack compiler find
\end{verbatim}

Purpose:

Allow Spack to use newly installed GCC for future package builds.

Verify:

\begin{verbatim}
spack compilers
\end{verbatim}

Expected:
\begin{itemize}
\item new GCC compiler entry
\end{itemize}
\subsection{Install OpenMPI Using Spack}
\label{sec:org2baa15b}

Install MPI runtime:

\begin{verbatim}
spack install openmpi
\end{verbatim}

Purpose:

Create shared MPI runtime stack.

This installation includes:

\begin{itemize}
\item MPI runtime
\item MPI compiler wrappers
\item MPI libraries
\item distributed runtime environment
\end{itemize}
\subsection{IMPORTANT MPI Compiler Dependency Concept}
\label{sec:org550054b}

MPI stacks are tightly coupled with compiler toolchains.

Example:

\begin{verbatim}
openmpi built using gcc@13
\end{verbatim}

This is important because:

\begin{itemize}
\item ABI compatibility matters
\item runtime compatibility matters
\item mixed compiler environments may fail
\end{itemize}
\subsection{Verify Installed Software Tree}
\label{sec:orgdb4f11c}

Show dependency tree:

\begin{verbatim}
spack find -d
\end{verbatim}

Purpose:

Displays:

\begin{itemize}
\item dependency hierarchy
\item compiler hierarchy
\item runtime dependencies
\end{itemize}

Example:

\begin{verbatim}
openmpi
   ^gcc@13.4.0
\end{verbatim}
\subsection{IMPORTANT Spack Spec Concept}
\label{sec:orge70c0d0}

Every Spack installation contains:

\begin{itemize}
\item package version
\item compiler information
\item dependency tree
\item unique installation hash
\end{itemize}

Example:

\begin{verbatim}
openmpi@5.0.7%gcc@13.4.0
\end{verbatim}

Meaning:

\begin{center}
\begin{tabular}{ll}
Component & Meaning\\
\hline
openmpi & package\\
5.0.7 & package version\\
gcc@13.4.0 & compiler used\\
\end{tabular}
\end{center}
\subsection{IMPORTANT Hash-Based Software Loading}
\label{sec:orgd80eccc}

Sometimes multiple builds of same version exist because of:

\begin{itemize}
\item different compilers
\item different variants
\item different dependencies
\end{itemize}

Spack assigns:

\begin{verbatim}
unique hashes
\end{verbatim}

View hashes:

\begin{verbatim}
spack find -l
\end{verbatim}

Expected:

\begin{verbatim}
abc1234 gcc@13.4.0
xyz5678 openmpi@5.0.7
\end{verbatim}

Load using hash:

\begin{verbatim}
spack load /xyz5678
\end{verbatim}

This is extremely common in production HPC systems.
\subsection{Load OpenMPI Environment}
\label{sec:orgeb278a0}

\begin{verbatim}
spack load openmpi
\end{verbatim}

Verify:

\begin{verbatim}
which mpicc
\end{verbatim}

Expected:
\begin{itemize}
\item Spack OpenMPI wrapper path
\end{itemize}

Verify MPI runtime:

\begin{verbatim}
mpirun --version
\end{verbatim}

Expected:
\begin{itemize}
\item OpenMPI version information
\end{itemize}
\subsection{IMPORTANT MPI Wrapper Concept}
\label{sec:org77c5862}

\begin{verbatim}
mpicc
\end{verbatim}

is NOT a compiler itself.

It is a wrapper that automatically injects:

\begin{itemize}
\item MPI headers
\item MPI libraries
\item runtime linker flags
\end{itemize}

around GCC.

This is a critical HPC build concept.
\subsection{Verify Loaded Software}
\label{sec:org67a40c8}

Display loaded packages:

\begin{verbatim}
spack find --loaded
\end{verbatim}

Unload OpenMPI:

\begin{verbatim}
spack unload openmpi
\end{verbatim}

Unload all software:

\begin{verbatim}
spack unload --all
\end{verbatim}

Verify:

\begin{verbatim}
spack find --loaded
\end{verbatim}

Expected:
\begin{itemize}
\item no loaded packages
\end{itemize}
\subsection{Create MPI Performance Validation Program}
\label{sec:orgbbc937b}

Create source file:

\begin{verbatim}
vim mpi_sum.c
\end{verbatim}

Purpose:

\begin{itemize}
\item validate MPI runtime
\item validate multinode execution
\item validate scheduler-aware execution
\item compare serial vs parallel runtime
\item demonstrate reduction operations
\item calculate speedup
\item calculate efficiency
\end{itemize}

Paste:

\begin{verbatim}
#include <mpi.h>
#include <stdio.h>
#include <stdlib.h>

#define N 1000000000

double serial_sum()
{
    double sum = 0.0;

    for(long long int i = 0; i < N; i++)
    {
        sum += i;
    }

    return sum;
}

int main(int argc, char *argv[])
{
    int rank, size;

    long long int start_index;
    long long int end_index;
    long long int chunk;

    double local_sum = 0.0;
    double global_sum = 0.0;

    double serial_start;
    double serial_end;

    double parallel_start;
    double parallel_end;

    MPI_Init(&argc, &argv);

    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);

    if(rank == 0)
    {
        serial_start = MPI_Wtime();

        double serial_result = serial_sum();

        serial_end = MPI_Wtime();

        printf("\n");
        printf("========== SERIAL COMPUTATION ==========\n");
        printf("Sum = %.0f\n", serial_result);
        printf("Time taken = %f seconds\n",
               serial_end - serial_start);
    }

    MPI_Barrier(MPI_COMM_WORLD);

    chunk = N / size;

    start_index = rank * chunk;

    if(rank == size - 1)
        end_index = N;
    else
        end_index = start_index + chunk;

    parallel_start = MPI_Wtime();

    for(long long int i = start_index;
        i < end_index;
        i++)
    {
        local_sum += i;
    }

    MPI_Reduce(&local_sum,
               &global_sum,
               1,
               MPI_DOUBLE,
               MPI_SUM,
               0,
               MPI_COMM_WORLD);

    parallel_end = MPI_Wtime();

    if(rank == 0)
    {
        double parallel_time =
            parallel_end - parallel_start;

        double serial_time =
            serial_end - serial_start;

        printf("\n");
        printf("========== PARALLEL COMPUTATION ==========\n");
        printf("Processes used = %d\n", size);
        printf("Sum = %.0f\n", global_sum);
        printf("Time taken = %f seconds\n",
               parallel_time);

        printf("\n");
        printf("========== PERFORMANCE ==========\n");
        printf("Speedup = %f\n",
               serial_time / parallel_time);

        printf("Efficiency = %f\n",
               (serial_time / parallel_time) / size);
    }

    MPI_Finalize();

    return 0;
}
\end{verbatim}
\subsection{IMPORTANT MPI Reduction Concept}
\label{sec:orgdd6ca9d}

This program uses:

\begin{verbatim}
MPI_Reduce()
\end{verbatim}

Purpose:

Combine partial sums from all MPI ranks into:

\begin{verbatim}
single global result
\end{verbatim}

Flow:

\begin{verbatim}
local sums
    ↓
MPI_Reduce
    ↓
global sum
\end{verbatim}

This is one of the MOST important MPI collective communication operations.
\subsection{IMPORTANT Parallel Execution Concept}
\label{sec:org743fa77}

Serial execution:

\begin{verbatim}
single process computes entire workload
\end{verbatim}

Parallel execution:

\begin{verbatim}
multiple MPI ranks divide workload
\end{verbatim}

Example:

\begin{center}
\begin{tabular}{ll}
Rank & Work\\
\hline
rank0 & 0 --> 25\%\\
rank1 & 25\% --> 50\%\\
rank2 & 50\% --> 75\%\\
rank3 & 75\% --> 100\%\\
\end{tabular}
\end{center}

Advantages:

\begin{itemize}
\item reduced execution time
\item distributed workload
\item scalable computation
\end{itemize}
\subsection{Load Software Before Compilation}
\label{sec:org51356ea}

Initialize Spack:

\begin{verbatim}
source /apps/spack/share/spack/setup-env.sh
\end{verbatim}

Load compiler:

\begin{verbatim}
spack load gcc@13.4.0
\end{verbatim}

Load MPI runtime:

\begin{verbatim}
spack load openmpi
\end{verbatim}

Verify:

\begin{verbatim}
which mpicc
\end{verbatim}

Expected:
\begin{itemize}
\item MPI compiler wrapper path
\end{itemize}
\subsection{Compile MPI Program}
\label{sec:org633a8f4}

Compile:

\begin{verbatim}
mpicc mpi_sum.c -o mpi_sum
\end{verbatim}

Verify executable:

\begin{verbatim}
ls
\end{verbatim}

Expected:

\begin{verbatim}
mpi_sum
\end{verbatim}
\subsection{IMPORTANT MPI Compilation Concept}
\label{sec:orgad35786}

MPI programs are compiled using:

\begin{verbatim}
mpicc
\end{verbatim}

instead of:

\begin{verbatim}
gcc
\end{verbatim}

because MPI wrapper automatically injects:

\begin{itemize}
\item MPI headers
\item MPI libraries
\item linker flags
\end{itemize}
\subsection{Interactive MPI Runtime Validation}
\label{sec:orgeb019a3}

Run interactively:

\begin{verbatim}
mpirun -np 4 ./mpi_sum
\end{verbatim}

Expected:

\begin{verbatim}
========== SERIAL COMPUTATION ==========
...

========== PARALLEL COMPUTATION ==========
...

========== PERFORMANCE ==========
...
\end{verbatim}

This validates:

\begin{itemize}
\item MPI runtime
\item MPI communication
\item reduction operations
\item distributed execution
\end{itemize}
\subsection{IMPORTANT Shared Filesystem Advantage}
\label{sec:orga329c99}

Because:

\begin{itemize}
\item source code
\item executable
\item software stack
\end{itemize}

exist on shared filesystem:

\begin{verbatim}
/apps
or
/home
\end{verbatim}

compute nodes automatically access:

\begin{itemize}
\item compiled binaries
\item runtime libraries
\item MPI stack
\end{itemize}

without recompilation.

This is one of the major HPC architecture advantages.
\subsection{Create Slurm Batch Script}
\label{sec:orgd689085}

Create batch script:

\begin{verbatim}
vim submit.sh
\end{verbatim}

Paste:

\begin{verbatim}
#!/bin/bash
#SBATCH -N 3
#SBATCH --ntasks-per-node=1
#SBATCH --job-name=test
#SBATCH --output=%J.out
#SBATCH --error=%J.err
#SBATCH --time=01:00:00
##SBATCH --nodelist=compute03,compute01

source /apps/spack/share/spack/setup-env.sh

spack load openmpi

mpirun -np $SLURM_NTASKS ./$1
\end{verbatim}

Make executable:

\begin{verbatim}
chmod +x submit.sh
\end{verbatim}
\subsection{IMPORTANT Slurm Script Explanation}
\label{sec:org6b9f258}

\begin{center}
\begin{tabular}{ll}
Line & Purpose\\
\hline
\#SBATCH -N 3 & request 3 nodes\\
--ntasks-per-node=1 & one MPI rank per node\\
--job-name=test & job name\\
--output=\%J.out & stdout file\\
--error=\%J.err & stderr file\\
--time=01:00:00 & walltime limit\\
\end{tabular}
\end{center}
\subsection{IMPORTANT Runtime Environment Section}
\label{sec:orgde7e8b7}

Inside batch script:

\begin{verbatim}
source /apps/spack/share/spack/setup-env.sh
\end{verbatim}

initializes Spack environment.

\begin{verbatim}
spack load openmpi
\end{verbatim}

loads MPI runtime stack.

Without these:

\begin{itemize}
\item mpirun may fail
\item MPI libraries may not load
\item runtime linker errors may occur
\end{itemize}
\subsection{IMPORTANT Slurm Runtime Variable}
\label{sec:orge57e648}

\begin{verbatim}
$SLURM_NTASKS
\end{verbatim}

contains:

\begin{verbatim}
total MPI process count allocated by Slurm
\end{verbatim}

Example:

\begin{center}
\begin{tabular}{rrr}
Nodes & Tasks per node & Total\\
\hline
3 & 1 & 3\\
\end{tabular}
\end{center}

Therefore:

\begin{verbatim}
mpirun -np $SLURM_NTASKS
\end{verbatim}

automatically launches correct number of MPI ranks.
\subsection{Submit MPI Job}
\label{sec:org2e7b708}

Submit job:

\begin{verbatim}
sbatch submit.sh mpi_sum
\end{verbatim}

Expected:

\begin{verbatim}
Submitted batch job 101
\end{verbatim}

This validates:

\begin{itemize}
\item Slurm scheduler
\item distributed allocation
\item multinode MPI execution
\item scheduler-aware runtime launch
\end{itemize}
\subsection{Monitor Jobs}
\label{sec:org16be97d}

Check user jobs:

\begin{verbatim}
squeue --me
\end{verbatim}

Expected:

\begin{verbatim}
JOBID PARTITION NAME USER ST TIME NODES NODELIST(REASON)
\end{verbatim}
\subsection{IMPORTANT Job States}
\label{sec:org1109b4b}

\begin{center}
\begin{tabular}{ll}
State & Meaning\\
\hline
PD & pending\\
R & running\\
CG & completing\\
CD & completed\\
F & failed\\
\end{tabular}
\end{center}
\subsection{View Detailed Job Information}
\label{sec:org1bd33a2}

Show detailed job info:

\begin{verbatim}
scontrol show job <jobid>
\end{verbatim}

Example:

\begin{verbatim}
scontrol show job 101
\end{verbatim}

Displays:

\begin{itemize}
\item allocated nodes
\item CPUs
\item runtime
\item scheduling information
\item execution directory
\item output locations
\end{itemize}
\subsection{Check Cluster Status}
\label{sec:orgf2c76b1}

Check cluster nodes:

\begin{verbatim}
sinfo
\end{verbatim}

Expected:

\begin{verbatim}
debug* up infinite 3 idle compute[01-03]
\end{verbatim}
\subsection{IMPORTANT Node States}
\label{sec:orgf5a36c0}

\begin{center}
\begin{tabular}{ll}
State & Meaning\\
\hline
idle & available\\
alloc & allocated\\
mix & partially used\\
drain & unavailable\\
down & offline\\
\end{tabular}
\end{center}
\subsection{Check Output Files}
\label{sec:org6690567}

After completion:

\begin{verbatim}
ls
\end{verbatim}

Expected:

\begin{verbatim}
101.out
101.err
\end{verbatim}

View output:

\begin{verbatim}
cat 101.out
\end{verbatim}

Expected:

\begin{itemize}
\item serial runtime
\item parallel runtime
\item speedup
\item efficiency
\end{itemize}
\subsection{IMPORTANT HPC Performance Metrics}
\label{sec:orga0df434}

The program calculates:

\begin{center}
\begin{tabular}{ll}
Metric & Meaning\\
\hline
Speedup & serial\_time / parallel\_time\\
Efficiency & speedup / processes\\
\end{tabular}
\end{center}

Ideal values:

\begin{center}
\begin{tabular}{ll}
Metric & Ideal\\
\hline
Speedup & near process count\\
Efficiency & near 1.0\\
\end{tabular}
\end{center}

Real systems experience:

\begin{itemize}
\item communication overhead
\item synchronization cost
\item MPI latency
\item reduction overhead
\end{itemize}
\subsection{Cancel Jobs}
\label{sec:orge80f1fc}

Cancel job:

\begin{verbatim}
scancel <jobid>
\end{verbatim}

Example:

\begin{verbatim}
scancel 101
\end{verbatim}

Cancel all user jobs:

\begin{verbatim}
scancel -u $USER
\end{verbatim}
\subsection{Interactive Slurm Execution}
\label{sec:orgfe309b6}

Launch interactive shell:

\begin{verbatim}
srun --pty bash
\end{verbatim}

Distributed hostname test:

\begin{verbatim}
srun -N 3 -n 3 hostname
\end{verbatim}

Expected:
\begin{itemize}
\item hostnames from allocated nodes
\end{itemize}

This validates:

\begin{itemize}
\item scheduler allocation
\item distributed execution
\item runtime communication
\end{itemize}
\subsection{IMPORTANT HPC Workflow}
\label{sec:org18c32e2}

Typical HPC workflow:

\begin{verbatim}
Load software
    ↓
Compile application
    ↓
Prepare Slurm script
    ↓
Submit job
    ↓
Monitor execution
    ↓
Analyze outputs
\end{verbatim}
\subsection{IMPORTANT Production HPC Concept}
\label{sec:org1598014}

Production HPC systems usually separate:

\begin{itemize}
\item operating system packages
\item scientific software stack
\item compiler stack
\item MPI runtime stack
\item scheduler integration
\end{itemize}

exactly like this setup.

This architecture provides:

\begin{itemize}
\item reproducibility
\item scalability
\item centralized administration
\item software consistency
\item runtime portability
\end{itemize}
\section{Centralized Identity Management using OpenLDAP + nslcd}
\label{sec:orgab76256}

Goal:

Transform the cluster from:

\begin{verbatim}
local-user-only infrastructure
\end{verbatim}

into:

\begin{verbatim}
centralized multi-user HPC environment
\end{verbatim}

using:

\begin{itemize}
\item OpenLDAP
\item nslcd
\item NSS
\item PAM
\end{itemize}

This phase introduces centralized user management for the HPC cluster.

After completion:

\begin{itemize}
\item users are created once centrally
\item login node recognizes LDAP users
\item compute nodes recognize LDAP users
\item centralized authentication works cluster-wide
\item automatic home directory creation works
\item Slurm can later use centralized identities
\end{itemize}
\subsection{Important Architecture}
\label{sec:org220e2d3}

Cluster roles:

\begin{center}
\begin{tabular}{ll}
Node & Role\\
\hline
master & LDAP server\\
login & LDAP client\\
compute01 & LDAP client\\
compute02 & LDAP client\\
compute03 & LDAP client\\
\end{tabular}
\end{center}

Important concept:

\begin{verbatim}
Only master runs LDAP SERVER (slapd).

Login and compute nodes run LDAP CLIENTS (nslcd).
\end{verbatim}
\subsection{Important Linux Authentication Concepts}
\label{sec:org28dad75}

Before starting, understand the layers.

\begin{center}
\begin{tabular}{ll}
Component & Purpose\\
\hline
LDAP & centralized user database\\
NSS & user/group lookup\\
PAM & authentication\\
nslcd & bridge between Linux and LDAP\\
\end{tabular}
\end{center}

Authentication flow:

\begin{verbatim}
SSH Login
   ↓
PAM
   ↓
nslcd
   ↓
LDAP server
\end{verbatim}
\subsection{Install OpenLDAP Server on Master Node}
\label{sec:org235206e}

Run ONLY on:

\begin{verbatim}
master
\end{verbatim}

Install LDAP packages:

\begin{verbatim}
sudo dnf install -y \
openldap \
openldap-servers \
openldap-clients
\end{verbatim}

Package explanation:

\begin{center}
\begin{tabular}{ll}
Package & Purpose\\
\hline
openldap & LDAP utilities\\
openldap-servers & slapd server daemon\\
openldap-clients & ldapsearch, ldapadd etc\\
\end{tabular}
\end{center}
\subsection{Enable LDAP Server}
\label{sec:org50c75c1}

Run ONLY on:

\begin{verbatim}
master
\end{verbatim}

Enable and start slapd:

\begin{verbatim}
sudo systemctl enable --now slapd
\end{verbatim}

Verify:

\begin{verbatim}
systemctl status slapd
\end{verbatim}

Expected:

\begin{verbatim}
active (running)
\end{verbatim}
\subsection{Generate LDAP Admin Password Hash}
\label{sec:orge9c2a41}

Run ONLY on:

\begin{verbatim}
master
\end{verbatim}

Generate secure LDAP admin password hash:

\begin{verbatim}
slappasswd
\end{verbatim}

Enter desired password.

Example:

\begin{verbatim}
cluster123
\end{verbatim}

Example output:

\begin{verbatim}
{SSHA}xxxxxxxxxxxxxxxx
\end{verbatim}

Important:

\begin{verbatim}
Copy this hash carefully.
\end{verbatim}

It will be used in LDAP database configuration.
\subsection{Configure LDAP Database}
\label{sec:orge98de61}

Run ONLY on:

\begin{verbatim}
master
\end{verbatim}

Create LDAP database configuration file:

\begin{verbatim}
vim ~/db.ldif
\end{verbatim}

Paste:

\begin{verbatim}
dn: olcDatabase={2}mdb,cn=config
changetype: modify
replace: olcSuffix
olcSuffix: dc=hpc,dc=local

dn: olcDatabase={2}mdb,cn=config
changetype: modify
replace: olcRootDN
olcRootDN: cn=Manager,dc=hpc,dc=local

dn: olcDatabase={2}mdb,cn=config
changetype: modify
replace: olcRootPW
olcRootPW: PASTE_HASH_HERE
\end{verbatim}

Replace:

\begin{verbatim}
PASTE_HASH_HERE
\end{verbatim}

with the hash generated using:

\begin{verbatim}
slappasswd
\end{verbatim}
\subsection{Important LDAP Concepts}
\label{sec:orgdc90fc8}

LDAP uses a hierarchical tree called:

\begin{verbatim}
DIT -- Directory Information Tree
\end{verbatim}

Example structure:

\begin{verbatim}
dc=hpc,dc=local
 ├── ou=People
 └── ou=Groups
\end{verbatim}

Meaning of terms:

\begin{center}
\begin{tabular}{ll}
Term & Meaning\\
\hline
dc & domain component\\
ou & organizational unit\\
cn & common name\\
\end{tabular}
\end{center}
\subsection{Apply LDAP Database Configuration}
\label{sec:org0314200}

Run ONLY on:

\begin{verbatim}
master
\end{verbatim}

Apply configuration:

\begin{verbatim}
sudo ldapmodify -Y EXTERNAL -H ldapi:/// \
-f ~/db.ldif
\end{verbatim}

Expected:

\begin{verbatim}
modifying entry
\end{verbatim}
\subsection{Import LDAP Schemas}
\label{sec:org65e2cbf}

Schemas define:

\begin{itemize}
\item user attributes
\item group attributes
\item UNIX account structure
\end{itemize}

Run ONLY on:

\begin{verbatim}
master
\end{verbatim}

Import cosine schema:

\begin{verbatim}
sudo ldapadd -Y EXTERNAL -H ldapi:/// \
-f /etc/openldap/schema/cosine.ldif
\end{verbatim}

Import nis schema:

\begin{verbatim}
sudo ldapadd -Y EXTERNAL -H ldapi:/// \
-f /etc/openldap/schema/nis.ldif
\end{verbatim}

Import inetorgperson schema:

\begin{verbatim}
sudo ldapadd -Y EXTERNAL -H ldapi:/// \
-f /etc/openldap/schema/inetorgperson.ldif
\end{verbatim}

Schema explanation:

\begin{center}
\begin{tabular}{ll}
Schema & Purpose\\
\hline
cosine & basic directory attributes\\
nis & UNIX/Linux account support\\
inetorgperson & user/person identity support\\
\end{tabular}
\end{center}
\subsection{Create LDAP Base Tree}
\label{sec:org92826f1}

Run ONLY on:

\begin{verbatim}
master
\end{verbatim}

Create base LDAP tree:

\begin{verbatim}
vim ~/base.ldif
\end{verbatim}

Paste:

\begin{verbatim}
dn: dc=hpc,dc=local
objectClass: top
objectClass: dcObject
objectClass: organization
o: HPC Cluster
dc: hpc

dn: ou=People,dc=hpc,dc=local
objectClass: organizationalUnit
ou: People

dn: ou=Groups,dc=hpc,dc=local
objectClass: organizationalUnit
ou: Groups
\end{verbatim}

Apply tree:

\begin{verbatim}
ldapadd -x \
-D "cn=Manager,dc=hpc,dc=local" \
-W \
-f ~/base.ldif
\end{verbatim}

Enter LDAP admin password.
\subsection{Verify LDAP Tree}
\label{sec:orgadfd027}

Run ONLY on:

\begin{verbatim}
master
\end{verbatim}

Verify tree:

\begin{verbatim}
ldapsearch -x -b "dc=hpc,dc=local"
\end{verbatim}

Expected:

\begin{itemize}
\item dc=hpc,dc=local
\item ou=People
\item ou=Groups
\end{itemize}
\subsection{Create LDAP Group}
\label{sec:orgc4d19b8}

Run ONLY on:

\begin{verbatim}
master
\end{verbatim}

Create group file:

\begin{verbatim}
vim ~/group.ldif
\end{verbatim}

Paste:

\begin{verbatim}
dn: cn=hpcusers,ou=Groups,dc=hpc,dc=local
objectClass: posixGroup
cn: hpcusers
gidNumber: 10000
\end{verbatim}

Add group:

\begin{verbatim}
ldapadd -x \
-D "cn=Manager,dc=hpc,dc=local" \
-W \
-f ~/group.ldif
\end{verbatim}
\subsection{Create LDAP User Password Hash}
\label{sec:orga9896ab}

Run ONLY on:

\begin{verbatim}
master
\end{verbatim}

Generate password hash:

\begin{verbatim}
slappasswd
\end{verbatim}

Example password:

\begin{verbatim}
test123
\end{verbatim}

Copy generated hash carefully.
\subsection{Create LDAP User}
\label{sec:orgf036c84}

Run ONLY on:

\begin{verbatim}
master
\end{verbatim}

Create user file:

\begin{verbatim}
vim ~/user.ldif
\end{verbatim}

Paste:

\begin{verbatim}
dn: uid=testUser,ou=People,dc=hpc,dc=local
objectClass: inetOrgPerson
objectClass: posixAccount
objectClass: shadowAccount
cn: Test User
sn: User
uid: testUser
uidNumber: 10001
gidNumber: 10000
homeDirectory: /home/testUser
loginShell: /bin/bash
userPassword: PASTE_HASH_HERE
\end{verbatim}

Replace:

\begin{verbatim}
PASTE_HASH_HERE
\end{verbatim}

with actual hash from:

\begin{verbatim}
slappasswd
\end{verbatim}

Add user:

\begin{verbatim}
ldapadd -x \
-D "cn=Manager,dc=hpc,dc=local" \
-W \
-f ~/user.ldif
\end{verbatim}
\subsection{Important LDAP User Attributes}
\label{sec:org837bfc9}

\begin{center}
\begin{tabular}{ll}
Attribute & Purpose\\
\hline
uid & username\\
uidNumber & UNIX UID\\
gidNumber & primary group\\
homeDirectory & user home\\
loginShell & login shell\\
cn & common name\\
sn & surname\\
\end{tabular}
\end{center}
\subsection{Verify LDAP User}
\label{sec:orga6f1ce9}

Run ONLY on:

\begin{verbatim}
master
\end{verbatim}

Verify user:

\begin{verbatim}
ldapsearch -x \
-b "dc=hpc,dc=local" uid=testUser
\end{verbatim}

Expected:
\begin{itemize}
\item LDAP user entry
\end{itemize}
\subsection{Configure LDAP Client on Login Node}
\label{sec:org5808567}

Now begin client-side integration.

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

Install LDAP client packages:

\begin{verbatim}
sudo dnf install -y \
openldap-clients \
nss-pam-ldapd
\end{verbatim}
\subsection{Configure nslcd}
\label{sec:orgedd0221}

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

Create configuration:

\begin{verbatim}
sudo vim /etc/nslcd.conf
\end{verbatim}

Paste:

\begin{verbatim}
uid nslcd
gid ldap

uri ldap://master
base dc=hpc,dc=local
\end{verbatim}

Explanation:

\begin{center}
\begin{tabular}{ll}
Parameter & Purpose\\
\hline
uri & LDAP server\\
base & LDAP search root\\
\end{tabular}
\end{center}

Secure configuration:

\begin{verbatim}
sudo chmod 600 /etc/nslcd.conf
\end{verbatim}
\subsection{Enable nslcd}
\label{sec:orgc3b4508}

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

Enable service:

\begin{verbatim}
sudo systemctl enable --now nslcd
\end{verbatim}

Verify:

\begin{verbatim}
systemctl status nslcd
\end{verbatim}

Expected:

\begin{verbatim}
active (running)
\end{verbatim}
\subsection{Verify LDAP Connectivity}
\label{sec:org3d5fc06}

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

Test LDAP communication:

\begin{verbatim}
ldapsearch -x \
-H ldap://master \
-b "dc=hpc,dc=local" uid=testUser
\end{verbatim}

Expected:
\begin{itemize}
\item LDAP user entry
\end{itemize}

This verifies:
\begin{itemize}
\item login node can communicate with master LDAP server
\end{itemize}
\subsection{Configure NSS}
\label{sec:org76651af}

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

Edit:

\begin{verbatim}
sudo vim /etc/nsswitch.conf
\end{verbatim}

Modify lines:

\begin{verbatim}
passwd: files ldap
group: files ldap
shadow: files ldap
\end{verbatim}

Important:

\begin{verbatim}
This tells Linux:
check local files first, then LDAP.
\end{verbatim}
\subsection{Restart nslcd}
\label{sec:orgbbcae1f}

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

Restart service:

\begin{verbatim}
sudo systemctl restart nslcd
\end{verbatim}
\subsection{Verify LDAP User Resolution}
\label{sec:org95637ff}

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

Test centralized user lookup:

\begin{verbatim}
getent passwd testUser
\end{verbatim}

Expected:

\begin{verbatim}
testUser:x:10001:10000:Test User:/home/testUser:/bin/bash
\end{verbatim}

Important concept:

\begin{verbatim}
This proves Linux itself recognizes LDAP users.
\end{verbatim}

This is one of the most important LDAP verification steps.
\subsection{NOTE (Do this for master too)}
\label{sec:org38a30f9}
Now we must configure: \textbf{master} with EXACT same setup (master should be able to recognise the user).
\subsection{Install Automatic Home Directory Support}
\label{sec:orgd422fb5}

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

Install package:

\begin{verbatim}
sudo dnf install -y oddjob-mkhomedir
\end{verbatim}

Enable service:

\begin{verbatim}
sudo systemctl enable --now oddjobd
\end{verbatim}

Purpose:

\begin{verbatim}
Automatically creates user home directory
during first successful login.
\end{verbatim}
\subsection{Configure PAM Authentication}
\label{sec:org1672221}

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

Edit:

\begin{verbatim}
sudo vim /etc/pam.d/system-auth
\end{verbatim}

Add LDAP lines in appropriate sections.

Add in auth section:

\begin{verbatim}
auth        sufficient    pam_ldap.so use_first_pass
\end{verbatim}

Add in account section:

\begin{verbatim}
account     sufficient    pam_ldap.so
\end{verbatim}

Add in password section:

\begin{verbatim}
password    sufficient    pam_ldap.so use_authtok
\end{verbatim}

Add in session section:

\begin{verbatim}
session     optional      pam_ldap.so
session     optional      pam_mkhomedir.so
\end{verbatim}

Important:

\begin{verbatim}
Do NOT remove pam_unix.so lines.
\end{verbatim}
\subsection{Configure Second PAM File}
\label{sec:org13656da}

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

Edit:

\begin{verbatim}
sudo vim /etc/pam.d/password-auth
\end{verbatim}

Add SAME LDAP PAM lines.

Reason:

\begin{verbatim}
Different login methods use different PAM stacks.
\end{verbatim}
\subsection{Restart Services}
\label{sec:orgb47c123}

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

Restart services:

\begin{verbatim}
sudo systemctl restart nslcd
sudo systemctl restart oddjobd
\end{verbatim}
\subsection{Verify LDAP Authentication}
\label{sec:org9592e35}

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

Test login:

\begin{verbatim}
su - testUser
\end{verbatim}

Enter LDAP password.

Expected:

\begin{verbatim}
Creating directory '/home/testUser'
\end{verbatim}

Successful login indicates:

\begin{itemize}
\item LDAP authentication working
\item PAM integration working
\item automatic home directory creation working
\end{itemize}
\subsection{Verify User Home}
\label{sec:org6d094eb}

Run ONLY on:

\begin{verbatim}
login
\end{verbatim}

After login:

\begin{verbatim}
pwd
\end{verbatim}

Expected:

\begin{verbatim}
/home/testUser
\end{verbatim}
\begin{enumerate}
\item NOW WE MUST REPLICATE TO COMPUTE NODES
\label{sec:orgf72aa5e}

Currently:

\begin{verbatim}
login node works fully
\end{verbatim}

Now we configure:

\begin{itemize}
\item compute01
\item compute02
\item compute03
\end{itemize}

with EXACT same client setup.
\end{enumerate}
\subsection{Troubleshooting}
\label{sec:orga67c5fc}
\begin{enumerate}
\item Important Slurm + LDAP Troubleshooting
\label{sec:orga905b74}

This section explains one of the MOST common HPC setup issues.
\item Symptom
\label{sec:org8c13839}

LDAP users:

\begin{itemize}
\item can SSH login
\item can use su
\item appear in getent passwd
\end{itemize}

BUT:

\begin{verbatim}
sbatch submit.sh
\end{verbatim}

results in:

\begin{itemize}
\item jobs stuck in pending
\item BeginTime state
\item scheduler instability
\item sinfo hanging
\item Slurm credential errors
\item slurmctld crashes
\end{itemize}
\item Root Cause
\label{sec:org324beb4}

Slurm internally works using:

\begin{itemize}
\item UID
\item GID
\item NSS identity resolution
\end{itemize}

When launching jobs, Slurm performs internal user identity lookups.

If master node cannot resolve LDAP users correctly through:

\begin{itemize}
\item NSS
\item nslcd
\item LDAP
\end{itemize}

then:

\begin{itemize}
\item Slurm credential generation fails
\item scheduler may crash
\item jobs cannot launch
\end{itemize}

Common error:

\begin{verbatim}
_fill_cred_gids: getpwuid failed for uid=10001
\end{verbatim}

Meaning:

\begin{verbatim}
Slurm cannot resolve LDAP user identity correctly.
\end{verbatim}
\item Important Verification Commands
\label{sec:org81661f3}

Run on:

\begin{verbatim}
master
login
compute nodes
\end{verbatim}

Verify LDAP resolution:

\begin{verbatim}
getent passwd testUser
id testUser
\end{verbatim}

Expected:

\begin{verbatim}
uid=10001(testUser) gid=10000(hpcusers)
\end{verbatim}

These commands MUST work correctly on ALL cluster nodes.
\item Important Master Node Verification
\label{sec:orge18a5da}

The master node MUST also resolve LDAP users correctly.

Verify:

\begin{verbatim}
getent passwd testUser
\end{verbatim}

If master cannot resolve LDAP users:

\begin{itemize}
\item slurmctld may fail
\item scheduler may crash
\item jobs remain pending
\end{itemize}

Verify nslcd configuration on master:

\begin{verbatim}
sudo vim /etc/nslcd.conf
\end{verbatim}

Example:

\begin{verbatim}
uid nslcd
gid ldap

uri ldap://master
base dc=hpc,dc=local
\end{verbatim}

Verify NSS configuration:

\begin{verbatim}
sudo vim /etc/nsswitch.conf
\end{verbatim}

Ensure:

\begin{verbatim}
passwd: files ldap
group: files ldap
shadow: files ldap
\end{verbatim}

Restart LDAP client:

\begin{verbatim}
sudo systemctl restart nslcd
\end{verbatim}
\item Important Slurm Architecture
\label{sec:orgae70540}

Correct architecture:

\begin{center}
\begin{tabular}{ll}
Node & Service\\
\hline
master & slurmctld\\
login & login services only\\
compute01 & slurmd\\
compute02 & slurmd\\
compute03 & slurmd\\
\end{tabular}
\end{center}

IMPORTANT:

\begin{verbatim}
Login node should NOT run slurmd.
\end{verbatim}

Because login node is NOT configured as a compute node in:

\begin{verbatim}
/etc/slurm/slurm.conf
\end{verbatim}

Attempting to start slurmd on login gives:

\begin{verbatim}
slurmd: fatal: Unable to determine this slurmd's NodeName
\end{verbatim}

This is NORMAL and expected.

Correct action on login node:

\begin{verbatim}
sudo systemctl stop slurmd
sudo systemctl disable slurmd
\end{verbatim}

Expected afterward:

\begin{verbatim}
inactive (dead)
\end{verbatim}

This is CORRECT for login nodes.
\item Important Slurm Recovery Commands
\label{sec:orgaf5b91b}

Whenever:

\begin{itemize}
\item LDAP users are added
\item NSS configuration changes
\item PAM configuration changes
\item nslcd configuration changes
\end{itemize}

restart Slurm services.

Restart controller on master:

\begin{verbatim}
sudo systemctl restart slurmctld
\end{verbatim}

Restart compute daemons on compute nodes:

\begin{verbatim}
sudo systemctl restart slurmd
\end{verbatim}

Restart LDAP client:

\begin{verbatim}
sudo systemctl restart nslcd
\end{verbatim}

Verify cluster:

\begin{verbatim}
sinfo
\end{verbatim}

Expected:

\begin{verbatim}
debug* up infinite 3 idle compute[01-03]
\end{verbatim}
\item Important Scheduler Debugging
\label{sec:orgb20639c}

If:

\begin{verbatim}
sinfo
\end{verbatim}

hangs indefinitely:

Possible cause:

\begin{verbatim}
slurmctld crashed
\end{verbatim}

Verify:

\begin{verbatim}
systemctl status slurmctld
\end{verbatim}

or:

\begin{verbatim}
journalctl -u slurmctld -n 50
\end{verbatim}
\end{enumerate}
\subsection{Current Cluster State After LDAP Setup}
\label{sec:org7884d49}

Cluster now contains:

\begin{center}
\begin{tabular}{ll}
Component & Status\\
\hline
OpenLDAP server & working\\
centralized users & working\\
centralized authentication & working\\
nslcd integration & working\\
NSS integration & working\\
PAM authentication & working\\
automatic home creation & working\\
\end{tabular}
\end{center}
\subsection{Important Debugging Commands}
\label{sec:org9706bb9}

Verify LDAP tree:

\begin{verbatim}
ldapsearch -x -b "dc=hpc,dc=local"
\end{verbatim}

Verify LDAP user:

\begin{verbatim}
ldapsearch -x -b "dc=hpc,dc=local" uid=testUser
\end{verbatim}

Verify LDAP authentication:

\begin{verbatim}
ldapwhoami -x \
-D "uid=testUser,ou=People,dc=hpc,dc=local" \
-W \
-H ldap://master
\end{verbatim}

Verify Linux user resolution:

\begin{verbatim}
getent passwd testUser
\end{verbatim}

Verify nslcd service:

\begin{verbatim}
systemctl status nslcd
\end{verbatim}
\subsection{Important Architecture Understanding}
\label{sec:orgcb3989b}

Master node:
\begin{itemize}
\item provides centralized identity database
\end{itemize}

Login/compute nodes:
\begin{itemize}
\item authenticate users
\item resolve LDAP users
\item create user sessions
\end{itemize}

This separation is fundamental in distributed authentication systems.
\section{Automated LDAP User Creation Script}
\label{sec:orgdc59757}

Managing users manually using individual LDIF files becomes difficult
as the number of users increases.

To simplify user management, an automation script can be used to:

\begin{itemize}
\item create LDAP group
\item create LDAP user
\item generate password automatically
\item generate password hash automatically
\item add entries into LDAP
\end{itemize}

This script automates complete LDAP user creation.

IMPORTANT:

\begin{verbatim}
The script creates:

- private group for user
- LDAP user entry
- hashed LDAP password

automatically.
\end{verbatim}
\subsection{Script Location}
\label{sec:org7fe57fd}

Example:

\begin{verbatim}
cd ~/ldap-scripts
ls
\end{verbatim}

Expected:

\begin{verbatim}
createUser.sh
\end{verbatim}
\subsection{User Creation Script}
\label{sec:orgbcdbf9d}

Create file:

\begin{verbatim}
vim createUser.sh
\end{verbatim}

Paste the complete script exactly as shown below:

\begin{verbatim}
#!/bin/bash 

LDAP_ADMIN="cn=Manager,dc=hpc,dc=local"
BASE_DN="dc=hpc,dc=local"

USERNAME=$1
UIDNUM=$2
GIDNUM=$3

if [ $# -ne 3 ]; then
    echo "Usage: $0 <username> <uid> <gid>"
    exit 1
fi

# Check if user already exists
USER_EXISTS=$(ldapsearch -x \
-b "ou=People,${BASE_DN}" "(uid=${USERNAME})" dn \
| grep "^dn:")

if [ ! -z "$USER_EXISTS" ]; then
    echo "ERROR: User ${USERNAME} already exists!"
    exit 1
fi

# Check if group already exists
GROUP_EXISTS=$(ldapsearch -x \
-b "ou=Groups,${BASE_DN}" "(cn=${USERNAME})" dn \
| grep "^dn:")

if [ ! -z "$GROUP_EXISTS" ]; then
    echo "ERROR: Group ${USERNAME} already exists!"
    exit 1
fi

PASSWORD="${USERNAME}@@123"

PASSWORD_HASH=$(slappasswd -s "$PASSWORD")

echo
echo "=================================="
echo "Creating LDAP User"
echo "=================================="
echo "Username : ${USERNAME}"
echo "UID      : ${UIDNUM}"
echo "GID      : ${GIDNUM}"
echo "=================================="

# Create private group LDIF
cat > /tmp/${USERNAME}_group.ldif <<EOF
dn: cn=${USERNAME},ou=Groups,${BASE_DN}
objectClass: posixGroup
cn: ${USERNAME}
gidNumber: ${GIDNUM}
EOF

# Create user LDIF
cat > /tmp/${USERNAME}.ldif <<EOF
dn: uid=${USERNAME},ou=People,${BASE_DN}
objectClass: inetOrgPerson
objectClass: posixAccount
objectClass: shadowAccount
cn: ${USERNAME}
sn: ${USERNAME}
uid: ${USERNAME}
uidNumber: ${UIDNUM}
gidNumber: ${GIDNUM}
homeDirectory: /home/${USERNAME}
loginShell: /bin/bash
userPassword: ${PASSWORD_HASH}
EOF

echo
echo "Adding LDAP group..."

ldapadd -x \
-D "${LDAP_ADMIN}" \
-W \
-f /tmp/${USERNAME}_group.ldif

if [ $? -ne 0 ]; then
    echo "ERROR: Failed to create LDAP group!"
    exit 1
fi

echo
echo "Adding LDAP user..."

ldapadd -x \
-D "${LDAP_ADMIN}" \
-W \
-f /tmp/${USERNAME}.ldif

if [ $? -ne 0 ]; then
    echo "ERROR: Failed to create LDAP user!"
    exit 1
fi

echo
echo "=================================="
echo "LDAP User Created Successfully"
echo "=================================="
echo "Username : ${USERNAME}"
echo "Password : ${PASSWORD}"
echo "UID      : ${UIDNUM}"
echo "GID      : ${GIDNUM}"
echo "=================================="
\end{verbatim}
\subsection{Make Script Executable}
\label{sec:orga6dbdfd}

Before using the script:

\begin{verbatim}
chmod +x createUser.sh
\end{verbatim}
\subsection{Understanding Script Arguments}
\label{sec:org477ffc2}

The script requires THREE arguments:

\begin{center}
\begin{tabular}{ll}
Argument & Meaning\\
\hline
username & LDAP username\\
uid & UNIX UID\\
gid & UNIX GID\\
\end{tabular}
\end{center}

Syntax:

\begin{verbatim}
./createUser.sh <username> <uid> <gid>
\end{verbatim}

Example:

\begin{verbatim}
./createUser.sh student01 11001 11001
\end{verbatim}

Explanation:

\begin{center}
\begin{tabular}{rl}
Value & Meaning\\
\hline
student01 & username\\
11001 & UID\\
11001 & GID\\
\end{tabular}
\end{center}

IMPORTANT:

\begin{verbatim}
The script creates a private group automatically.

Therefore username and group name become identical.
\end{verbatim}

Example:

\begin{center}
\begin{tabular}{ll}
Entity & Created Automatically\\
\hline
User & student01\\
Group & student01\\
Home Directory & /home/student01\\
\end{tabular}
\end{center}
\subsection{Default Password Format}
\label{sec:org32a3696}

The script automatically creates password using:

\begin{verbatim}
username@@123
\end{verbatim}

Example:

\begin{center}
\begin{tabular}{ll}
Username & Password\\
\hline
student01 & student01@@123\\
student02 & student02@@123\\
\end{tabular}
\end{center}

IMPORTANT:

\begin{verbatim}
Users should change password after first login.
\end{verbatim}
\subsection{What the Script Automatically Does}
\label{sec:orgb016872}

The script performs following operations automatically:

\begin{center}
\begin{tabular}{ll}
Operation & Description\\
\hline
Check existing user & avoids duplicate users\\
Check existing group & avoids duplicate groups\\
Generate password hash & secure LDAP password\\
Create LDAP group LDIF & automatic\\
Create LDAP user LDIF & automatic\\
Add LDAP group & automatic\\
Add LDAP user & automatic\\
\end{tabular}
\end{center}
\subsection{Example User Creation}
\label{sec:orge27791d}

Example:

\begin{verbatim}
./createUser.sh student02 11002 11002
\end{verbatim}

Expected output:

\begin{verbatim}
==================================
Creating LDAP User
==================================
Username : student02
UID      : 11002
GID      : 11002
==================================
\end{verbatim}

LDAP admin password will be requested.

After successful creation:

\begin{verbatim}
==================================
LDAP User Created Successfully
==================================
Username : student02
Password : student02@@123
UID      : 11002
GID      : 11002
==================================
\end{verbatim}
\subsection{Verify User Creation}
\label{sec:org3ddeece}

Verify user using:

\begin{verbatim}
getent passwd student02
\end{verbatim}

or:

\begin{verbatim}
id student02
\end{verbatim}

Expected:

\begin{verbatim}
uid=11002(student02)
\end{verbatim}

Verify LDAP entry:

\begin{verbatim}
ldapsearch -x \
-b "dc=hpc,dc=local" uid=student02
\end{verbatim}
\subsection{Verify Login}
\label{sec:org8c44f50}

Test login:

\begin{verbatim}
su - student02
\end{verbatim}

or:

\begin{verbatim}
ssh student02@login
\end{verbatim}

Expected:

\begin{verbatim}
successful login
\end{verbatim}
\subsection{Important UID/GID Notes}
\label{sec:orgfa1f31c}

IMPORTANT:

\begin{verbatim}
UID and GID values must remain unique cluster-wide.
\end{verbatim}

Avoid:
\begin{itemize}
\item duplicate UID
\item duplicate GID
\end{itemize}

because duplicate IDs can create:

\begin{itemize}
\item ownership conflicts
\item wrong usernames
\item incorrect file ownership
\item Slurm identity problems
\end{itemize}

Example of BAD configuration:

\begin{center}
\begin{tabular}{lr}
User & UID\\
\hline
student01 & 11001\\
student02 & 11001\\
\end{tabular}
\end{center}

This can cause:

\begin{verbatim}
whoami returning wrong username
\end{verbatim}

Always use unique UID/GID values.
\subsection{Important HPC Identity Concept}
\label{sec:org0078b44}

In HPC clusters:

\begin{itemize}
\item NFS
\item Slurm
\item MPI
\item LDAP
\item SSH
\end{itemize}

all depend on:

\begin{verbatim}
consistent unique UID/GID mappings
\end{verbatim}

across the entire cluster.

This is one of the MOST important concepts in centralized HPC authentication.
\section{Automated LDAP User Deletion Script}
\label{sec:orgcea540f}

Managing users manually using:

\begin{itemize}
\item ldapdelete
\item group deletion
\item home directory cleanup
\end{itemize}

can become repetitive and error-prone.

To simplify cleanup operations, an automated LDAP deletion script can be used.

This script automates:

\begin{itemize}
\item LDAP user deletion
\item optional LDAP group deletion
\item optional home directory deletion
\end{itemize}

IMPORTANT:

\begin{verbatim}
The script provides interactive cleanup options
for safer user management.
\end{verbatim}
\subsection{Script Location}
\label{sec:org9095d4e}

Example:

\begin{verbatim}
cd ~/ldap-scripts
ls
\end{verbatim}

Expected:

\begin{verbatim}
deleteUser.sh
\end{verbatim}
\subsection{User Deletion Script}
\label{sec:org27f4c1c}

Create file:

\begin{verbatim}
vim deleteUser.sh
\end{verbatim}

Paste the complete script exactly as shown below:

\begin{verbatim}
#!/bin/bash 

LDAP_ADMIN="cn=Manager,dc=hpc,dc=local"
BASE_DN="dc=hpc,dc=local"

USERNAME=$1

if [ $# -ne 1 ]; then
    echo "Usage: $0 <username>"
    exit 1
fi

echo
echo "=================================="
echo "Deleting LDAP User"
echo "=================================="
echo "Username : ${USERNAME}"
echo "=================================="

echo
echo "Deleting LDAP user entry..."

ldapdelete -x \
-D "${LDAP_ADMIN}" \
-W \
"uid=${USERNAME},ou=People,${BASE_DN}"

if [ $? -ne 0 ]; then
    echo "ERROR: Failed to delete LDAP user!"
    exit 1
fi

echo
echo "LDAP user deleted successfully."

# Ask about deleting group
echo
read -p "Delete LDAP group '${USERNAME}'? (y/n): " DELETE_GROUP

case $DELETE_GROUP in
    y|Y)

        ldapdelete -x \
        -D "${LDAP_ADMIN}" \
        -W \
        "cn=${USERNAME},ou=Groups,${BASE_DN}"

        if [ $? -eq 0 ]; then
            echo "LDAP group deleted successfully."
        else
            echo "WARNING: Failed to delete LDAP group."
        fi
        ;;

    n|N)
        echo "Skipping LDAP group deletion."
        ;;

    *)
        echo "Invalid option. Skipping group deletion."
        ;;
esac

# Ask about deleting home directory
echo
read -p "Delete home directory '/home/${USERNAME}'? (y/n): " DELETE_HOME

case $DELETE_HOME in
    y|Y)

        if [ -d "/home/${USERNAME}" ]; then
            sudo rm -rf "/home/${USERNAME}"

            if [ $? -eq 0 ]; then
                echo "Home directory deleted successfully."
            else
                echo "WARNING: Failed to delete home directory."
            fi
        else
            echo "Home directory does not exist."
        fi
        ;;

    n|N)
        echo "Skipping home directory deletion."
        ;;

    *)
        echo "Invalid option. Skipping home deletion."
        ;;
esac

echo
echo "=================================="
echo "User cleanup completed"
echo "=================================="
\end{verbatim}
\subsection{Make Script Executable}
\label{sec:org7a40f40}

Before using the script:

\begin{verbatim}
chmod +x deleteUser.sh
\end{verbatim}
\subsection{Understanding Script Arguments}
\label{sec:org8ad224c}

The script requires ONE argument:

\begin{center}
\begin{tabular}{ll}
Argument & Meaning\\
\hline
username & LDAP username\\
\end{tabular}
\end{center}

Syntax:

\begin{verbatim}
./deleteUser.sh <username>
\end{verbatim}

Example:

\begin{verbatim}
./deleteUser.sh student02
\end{verbatim}
\subsection{What the Script Automatically Does}
\label{sec:orgc5b264a}

The script performs following operations automatically:

\begin{center}
\begin{tabular}{ll}
Operation & Description\\
\hline
Delete LDAP user & removes LDAP account\\
Optional group deletion & removes LDAP group if requested\\
Optional home deletion & removes user home if requested\\
Interactive confirmation & prevents accidental cleanup\\
\end{tabular}
\end{center}
\subsection{Example User Deletion}
\label{sec:orge286187}

Example:

\begin{verbatim}
./deleteUser.sh student02
\end{verbatim}

Expected output:

\begin{verbatim}
==================================
Deleting LDAP User
==================================
Username : student02
==================================
\end{verbatim}

LDAP admin password will be requested.

After successful deletion:

\begin{verbatim}
LDAP user deleted successfully.
\end{verbatim}

The script then asks:

\begin{verbatim}
Delete LDAP group 'student02'? (y/n):
\end{verbatim}

If:

\begin{verbatim}
y
\end{verbatim}

is selected, corresponding LDAP group will also be removed.

Next prompt:

\begin{verbatim}
Delete home directory '/home/student02'? (y/n):
\end{verbatim}

If:

\begin{verbatim}
y
\end{verbatim}

is selected, user home directory will also be deleted.
\subsection{Important Group Deletion Notes}
\label{sec:org660ee3c}

IMPORTANT:

\begin{verbatim}
Do NOT delete shared groups accidentally.
\end{verbatim}

Example:

\begin{verbatim}
hpcusers
\end{verbatim}

may contain multiple users.

Deleting shared groups can create:

\begin{itemize}
\item broken group mappings
\item permission problems
\item Slurm identity issues
\end{itemize}

The script therefore asks confirmation before deleting groups.
\subsection{Important Home Directory Notes}
\label{sec:org0416084}

Deleting home directories permanently removes:

\begin{itemize}
\item source codes
\item job scripts
\item outputs
\item datasets
\item SSH keys
\item configuration files
\end{itemize}

IMPORTANT:

\begin{verbatim}
Home directory deletion is irreversible.
\end{verbatim}

Always verify before deleting.
\subsection{Verify User Removal}
\label{sec:org0e1a791}

Verify deletion:

\begin{verbatim}
getent passwd student02
\end{verbatim}

Expected:

\begin{verbatim}
no output
\end{verbatim}

Verify LDAP entry removal:

\begin{verbatim}
ldapsearch -x \
-b "dc=hpc,dc=local" uid=student02
\end{verbatim}

Expected:

\begin{verbatim}
no entries returned
\end{verbatim}

Verify login failure:

\begin{verbatim}
su - student02
\end{verbatim}

Expected:

\begin{verbatim}
user does not exist
\end{verbatim}
\subsection{Important HPC User Management Concept}
\label{sec:org4e2e896}

In centralized HPC environments:

\begin{itemize}
\item user lifecycle management
\item account cleanup
\item UID/GID consistency
\item group management
\end{itemize}

are critical administrative tasks.

Improper deletion can create:

\begin{itemize}
\item orphaned files
\item broken permissions
\item Slurm scheduling issues
\item incorrect ownership mappings
\end{itemize}

Therefore automated and controlled cleanup procedures are highly important in production HPC systems.
\section{Automated LDAP Password Change Script}
\label{sec:orgac347dd}

Managing passwords manually using LDIF files becomes inconvenient
in centralized LDAP environments.

To simplify password management, an automated password change script
can be used.

This script automates:

\begin{itemize}
\item LDAP password update
\item password hash generation
\item secure password input
\item LDAP password modification
\end{itemize}

IMPORTANT:

\begin{verbatim}
Because authentication is centralized through LDAP,
password changes automatically work cluster-wide.
\end{verbatim}

After changing password:

\begin{itemize}
\item login node recognizes new password
\item compute nodes recognize new password
\item master recognizes new password
\end{itemize}

without changing anything individually on nodes.
\subsection{Script Location}
\label{sec:org656ef66}

Example:

\begin{verbatim}
cd ~/ldap-scripts
ls
\end{verbatim}

Expected:

\begin{verbatim}
changePassword.sh
\end{verbatim}
\subsection{Password Change Script}
\label{sec:org8268eeb}

Create file:

\begin{verbatim}
vim changePassword.sh
\end{verbatim}

Paste the complete script exactly as shown below:

\begin{verbatim}
#!/bin/bash

LDAP_ADMIN="cn=Manager,dc=hpc,dc=local"
BASE_DN="dc=hpc,dc=local"

USERNAME=$1

if [ $# -ne 1 ]; then
    echo "Usage: $0 <username>"
    exit 1
fi

# Check if user exists
USER_EXISTS=$(ldapsearch -x \
-b "ou=People,${BASE_DN}" "(uid=${USERNAME})" dn \
| grep "^dn:")

if [ -z "$USER_EXISTS" ]; then
    echo "ERROR: User ${USERNAME} does not exist!"
    exit 1
fi

echo
echo "=================================="
echo "LDAP Password Change"
echo "=================================="
echo "Username : ${USERNAME}"
echo "=================================="

# Take password securely
echo
read -s -p "Enter new password: " PASSWORD1
echo

read -s -p "Confirm new password: " PASSWORD2
echo

# Check password match
if [ "$PASSWORD1" != "$PASSWORD2" ]; then
    echo
    echo "ERROR: Passwords do not match!"
    exit 1
fi

# Generate LDAP password hash
PASSWORD_HASH=$(slappasswd -s "$PASSWORD1")

# Create LDIF file
cat > /tmp/${USERNAME}_password.ldif <<EOF
dn: uid=${USERNAME},ou=People,${BASE_DN}
changetype: modify
replace: userPassword
userPassword: ${PASSWORD_HASH}
EOF

echo
echo "Updating LDAP password..."

ldapmodify -x \
-D "${LDAP_ADMIN}" \
-W \
-f /tmp/${USERNAME}_password.ldif

if [ $? -ne 0 ]; then
    echo
    echo "ERROR: Failed to change password!"
    exit 1
fi

echo
echo "=================================="
echo "Password updated successfully"
echo "=================================="
echo "Username : ${USERNAME}"
echo "=================================="
\end{verbatim}
\subsection{Make Script Executable}
\label{sec:org6b20c28}

Before using the script:

\begin{verbatim}
chmod +x changePassword.sh
\end{verbatim}
\subsection{Understanding Script Arguments}
\label{sec:orgd2c9c9c}

The script requires ONE argument:

\begin{center}
\begin{tabular}{ll}
Argument & Meaning\\
\hline
username & LDAP username\\
\end{tabular}
\end{center}

Syntax:

\begin{verbatim}
./changePassword.sh <username>
\end{verbatim}

Example:

\begin{verbatim}
./changePassword.sh student01
\end{verbatim}
\subsection{What the Script Automatically Does}
\label{sec:orgaa84716}

The script performs following operations automatically:

\begin{center}
\begin{tabular}{ll}
Operation & Description\\
\hline
Verify LDAP user exists & prevents invalid modifications\\
Secure password input & password hidden during typing\\
Password confirmation & avoids typing mistakes\\
Generate password hash & secure LDAP password storage\\
Modify LDAP password & centralized password update\\
\end{tabular}
\end{center}
\subsection{Example Password Change}
\label{sec:orgbcf0c19}

Example:

\begin{verbatim}
./changePassword.sh student01
\end{verbatim}

Expected output:

\begin{verbatim}
==================================
LDAP Password Change
==================================
Username : student01
==================================
\end{verbatim}

The script asks:

\begin{verbatim}
Enter new password:
\end{verbatim}

Then:

\begin{verbatim}
Confirm new password:
\end{verbatim}

LDAP admin password will also be requested.

After successful update:

\begin{verbatim}
==================================
Password updated successfully
==================================
Username : student01
==================================
\end{verbatim}
\subsection{Verify Password Change}
\label{sec:orgdf1e495}

Test login using updated password:

\begin{verbatim}
su - student01
\end{verbatim}

or:

\begin{verbatim}
ssh student01@login
\end{verbatim}

Expected:

\begin{verbatim}
successful login using new password
\end{verbatim}

Because LDAP authentication is centralized, the updated password works automatically on:

\begin{itemize}
\item login node
\item compute nodes
\item master node
\end{itemize}
\subsection{Important Password Management Notes}
\label{sec:orgc61b724}

IMPORTANT:

\begin{verbatim}
LDAP stores password hashes, not plain-text passwords.
\end{verbatim}

The script automatically generates secure LDAP-compatible password hashes using:

\begin{verbatim}
slappasswd
\end{verbatim}

This improves security by avoiding direct plain-text password storage.
\section{Advanced Slurm Configuration Preparation}
\label{sec:org50b76a7}

This phase marks the transition from a:

\begin{verbatim}
basic educational Slurm setup
\end{verbatim}

to a more:

\begin{verbatim}
resource-aware and production-like HPC scheduler environment
\end{verbatim}

At this stage, the cluster was already fully functional and stable.

Existing working components:

\begin{itemize}
\item Slurm scheduler
\item sbatch job submission
\item MPI execution
\item multinode MPI jobs
\item LDAP centralized authentication
\item shared filesystem
\item shared Spack software stack
\end{itemize}

The goal of this phase was to carefully introduce advanced scheduler
features WITHOUT destabilizing runtime execution.
\subsection{IMPORTANT APPROACH}
\label{sec:org4b1df97}

Advanced Slurm administration should always be performed incrementally.

Instead of enabling many advanced features simultaneously, only ONE
feature was introduced and validated at a time.

This approach helps:

\begin{itemize}
\item isolate issues properly
\item simplify debugging
\item understand scheduler behavior
\item maintain cluster stability
\end{itemize}

This is how production HPC administrators commonly manage scheduler
configuration changes.
\subsection{Advanced Slurm Roadmap}
\label{sec:org947ab70}

The cluster was upgraded gradually in the following order:

\begin{center}
\begin{tabular}{rll}
Phase & Feature & Risk Level\\
\hline
1 & Resource Accounting & Safe\\
2 & Memory-Aware Scheduling & Safe\\
3 & CPU Binding / Affinity & Safe\\
4 & Job Limits & Safe\\
5 & QoS & Medium\\
6 & Accounts \& Fairshare & Medium\\
7 & Reservations & Medium\\
8 & Advanced Scheduling Policies & Medium\\
9 & cgroups/task/cgroup & Advanced\\
10 & Monitoring \& Metrics & Advanced\\
\end{tabular}
\end{center}
\begin{enumerate}
\item IMPORTANT NOTE
\label{sec:org6211354}

At this stage:

\begin{verbatim}
TaskPlugin=task/cgroup
\end{verbatim}

was intentionally NOT enabled.

Reason:

Strict cgroup enforcement can introduce runtime instability,
especially on:

\begin{itemize}
\item Rocky Linux 9
\item cgroup v2 systems
\item educational clusters
\end{itemize}

Instead, the cluster first focused on:

\begin{itemize}
\item stable scheduling
\item resource accounting
\item affinity scheduling
\item scheduler-aware resource tracking
\end{itemize}

before moving toward strict kernel-level enforcement.
\end{enumerate}
\subsection{Resource-Aware Scheduling}
\label{sec:orge88e5f1}

Goal:

Teach Slurm to understand:

\begin{itemize}
\item CPU resources
\item memory resources
\item consumable resources
\item schedulable resources
\end{itemize}

without enabling strict runtime enforcement.

This creates a much safer and more stable intermediate scheduler state.
\subsection{Understanding Current Cluster Resource State}
\label{sec:orgda7bdd5}

Initial scheduler behavior:

\begin{itemize}
\item node-aware scheduling
\item CPU-aware scheduling
\end{itemize}

BUT:

Slurm did NOT properly understand actual node memory capacity.

This can be verified using:

\begin{verbatim}
scontrol show node compute01
\end{verbatim}

Without:

\begin{verbatim}
RealMemory
\end{verbatim}

configured, Slurm cannot properly perform memory-aware scheduling.
\subsection{Understanding Actual Node Memory}
\label{sec:org5233b2a}

Actual node memory was checked using:

\begin{verbatim}
ssh compute01 free -m
ssh compute02 free -m
ssh compute03 free -m
\end{verbatim}

Observed memory:

\begin{verbatim}
~1700 MB
\end{verbatim}

IMPORTANT:

Production HPC clusters NEVER allocate 100\% physical memory to jobs.

Reason:

The operating system itself requires memory for:

\begin{itemize}
\item kernel
\item SSH sessions
\item Slurm daemons
\item filesystem cache
\item system services
\item background tasks
\end{itemize}

A portion of memory must always remain reserved for stable node
operation.
\subsection{Configuring RealMemory}
\label{sec:orgf22b350}

Node configuration was updated inside:

\begin{verbatim}
sudo vim /etc/slurm/slurm.conf
\end{verbatim}

Original configuration:

\begin{verbatim}
NodeName=compute01 CPUs=2 State=UNKNOWN
NodeName=compute02 CPUs=2 State=UNKNOWN
NodeName=compute03 CPUs=2 State=UNKNOWN
\end{verbatim}

Updated configuration:

\begin{verbatim}
NodeName=compute01 CPUs=2 RealMemory=1200 State=UNKNOWN
NodeName=compute02 CPUs=2 RealMemory=1200 State=UNKNOWN
NodeName=compute03 CPUs=2 RealMemory=1200 State=UNKNOWN
\end{verbatim}
\subsection{RealMemory Concept}
\label{sec:orge37732e}

\begin{verbatim}
RealMemory=1200
\end{verbatim}

does NOT represent total physical memory.

Instead, it defines:

\begin{verbatim}
schedulable memory available to jobs
\end{verbatim}

Example:

\begin{center}
\begin{tabular}{ll}
Actual Physical Memory & Configured Schedulable Memory\\
\hline
\textasciitilde{}1700 MB & 1200 MB\\
\end{tabular}
\end{center}

This reserved memory helps maintain:

\begin{itemize}
\item system stability
\item scheduler reliability
\item daemon responsiveness
\item SSH availability
\end{itemize}

during heavy workloads.
\subsection{Resource-Aware Scheduling Concept}
\label{sec:orgae25f9b}

After configuring:

\begin{verbatim}
RealMemory
\end{verbatim}

Slurm now understands:

\begin{itemize}
\item CPU resources
\item schedulable memory
\item consumable resources
\end{itemize}

instead of only allocating entire nodes.

This is a major scheduler upgrade.

The scheduler transitioned from:

\begin{verbatim}
simple node allocation
\end{verbatim}

to:

\begin{verbatim}
resource-aware scheduling
\end{verbatim}
\subsection{Scheduler Configuration Enhancements}
\label{sec:orgbe700c6}

The following important scheduler options were enabled:

\begin{verbatim}
SchedulerType=sched/backfill

SelectType=select/cons_tres
SelectTypeParameters=CR_Core

TaskPlugin=task/affinity
\end{verbatim}
\subsection{Configuration Explanation}
\label{sec:orga92de16}

\begin{enumerate}
\item SchedulerType=sched/backfill
\label{sec:orgfd5b887}

Enables Slurm backfill scheduler.

Purpose:

Allows smaller jobs to execute while larger jobs wait for resources.

Benefits:

\begin{itemize}
\item improves cluster utilization
\item reduces idle resources
\item increases scheduling efficiency
\end{itemize}

This is commonly used in production HPC systems.
\item SelectType=select/cons\_tres
\label{sec:org9b3e120}

Enables:

\begin{verbatim}
consumable resource scheduling
\end{verbatim}

This allows Slurm to track:

\begin{itemize}
\item CPUs
\item memory
\item schedulable resources
\end{itemize}

instead of only allocating complete nodes.
\item SelectTypeParameters=CR\_Core
\label{sec:org5f256ee}

Enables:

\begin{verbatim}
core-aware scheduling
\end{verbatim}

Resources are now allocated at CPU-core granularity.

This is extremely important for:

\begin{itemize}
\item MPI workloads
\item OpenMP workloads
\item hybrid parallel applications
\end{itemize}
\item TaskPlugin=task/affinity
\label{sec:orgf5bae6d}

Enables CPU affinity support.

Purpose:

Allows Slurm to:

\begin{itemize}
\item pin tasks to CPUs
\item improve cache locality
\item optimize task placement
\item stabilize parallel performance
\end{itemize}

This is one of the most important performance-related scheduler
features in HPC environments.
\end{enumerate}
\subsection{Distributing Updated Slurm Configuration}
\label{sec:org382fedd}

Updated configuration was distributed cluster-wide.

Copy configuration:

\begin{verbatim}
scp /etc/slurm/slurm.conf login:/tmp/
scp /etc/slurm/slurm.conf compute01:/tmp/
scp /etc/slurm/slurm.conf compute02:/tmp/
scp /etc/slurm/slurm.conf compute03:/tmp/
\end{verbatim}

Move configuration into place:

\begin{verbatim}
sudo mv /tmp/slurm.conf /etc/slurm/
\end{verbatim}

Compute nodes:

\begin{verbatim}
ssh compute01 sudo -S mv /tmp/slurm.conf /etc/slurm/
ssh compute02 sudo -S mv /tmp/slurm.conf /etc/slurm/
ssh compute03 sudo -S mv /tmp/slurm.conf /etc/slurm/
\end{verbatim}
\subsection{Restarting Slurm Services}
\label{sec:org15a4385}

Restart controller on master node:

\begin{verbatim}
sudo systemctl restart slurmctld
\end{verbatim}

Restart compute daemons:

\begin{verbatim}
ssh compute01 sudo -S systemctl restart slurmd
ssh compute02 sudo -S systemctl restart slurmd
ssh compute03 sudo -S systemctl restart slurmd
\end{verbatim}

Purpose:

Reload updated scheduler configuration across all nodes.
\subsection{Verifying Cluster Health}
\label{sec:orgd318d28}

Verify node state:

\begin{verbatim}
sinfo
\end{verbatim}

Expected:

\begin{verbatim}
compute[01-03] idle
\end{verbatim}

Verify scheduler memory recognition:

\begin{verbatim}
scontrol show node compute01
\end{verbatim}

Expected:

\begin{verbatim}
RealMemory=1200
\end{verbatim}

This validates:

\begin{itemize}
\item scheduler configuration loaded correctly
\item memory-aware scheduling active
\item stable cluster operation
\end{itemize}
\subsection{Difference Between Scheduling and Enforcement}
\label{sec:org7814bc0}

At this stage:

\begin{verbatim}
memory-aware scheduling
\end{verbatim}

was enabled.

BUT:

\begin{verbatim}
strict kernel-level memory enforcement
\end{verbatim}

was NOT enabled.

This distinction is extremely important.

Current behavior:

\begin{verbatim}
#SBATCH --mem=500M
\end{verbatim}

does:

\begin{itemize}
\item reserve memory in scheduler accounting
\item affect scheduling decisions
\item prevent scheduler over-allocation
\end{itemize}

BUT does NOT:

\begin{itemize}
\item hard-limit memory usage
\item kill memory-overusing jobs
\item enforce kernel-level restrictions
\end{itemize}

because:

\begin{verbatim}
TaskPlugin=task/cgroup
\end{verbatim}

was intentionally NOT enabled.
\subsection{Why task/cgroup Was Postponed}
\label{sec:org16bf535}

Earlier experiments with:

\begin{verbatim}
TaskPlugin=task/cgroup
\end{verbatim}

introduced runtime instability.

Observed issues included:

\begin{itemize}
\item batch runtime failures
\item node drain states
\item unstable job execution
\item scheduler inconsistencies
\end{itemize}

Reason:

Modern Rocky Linux 9 systems use:

\begin{verbatim}
cgroup v2
\end{verbatim}

which often requires significantly more careful Slurm configuration and
kernel-level tuning.

Instead of introducing unstable runtime enforcement early, the cluster
was intentionally kept in a stable intermediate state using:

\begin{verbatim}
TaskPlugin=task/affinity
\end{verbatim}

This approach provides:

\begin{itemize}
\item stable runtime execution
\item CPU affinity support
\item resource-aware scheduling
\item simplified administration
\end{itemize}

without destabilizing scheduler behavior.
\subsection{Process Tracking Note}
\label{sec:org3276f1c}

The cluster still uses:

\begin{verbatim}
ProctrackType=proctrack/cgroup
\end{verbatim}

This is generally safe because:

\begin{itemize}
\item lightweight process tracking only
\item no strict runtime enforcement
\item no aggressive kernel-level isolation
\end{itemize}

This differs significantly from:

\begin{verbatim}
TaskPlugin=task/cgroup
\end{verbatim}

which performs actual resource enforcement and isolation.

Keeping:

\begin{verbatim}
proctrack/cgroup
\end{verbatim}

while avoiding:

\begin{verbatim}
task/cgroup
\end{verbatim}

provides a much safer and more stable educational HPC environment.
\subsection{Verifying Stable Job Execution}
\label{sec:org3e584ee}

After enabling resource-aware scheduling:

\begin{itemize}
\item sbatch jobs remained stable
\item MPI jobs executed correctly
\item multinode execution continued working
\item no node drain issues occurred
\item no runtime instability observed
\end{itemize}

This confirmed that the scheduler upgrade was successful.
\subsection{Milestone Achieved}
\label{sec:orgb7b6c0b}

The cluster successfully evolved from:

\begin{verbatim}
basic educational scheduler
\end{verbatim}

into:

\begin{verbatim}
resource-aware production-like scheduler
\end{verbatim}

while maintaining:

\begin{itemize}
\item stable runtime execution
\item multinode MPI support
\item scheduler reliability
\item simplified administration
\end{itemize}

This forms the foundation required for future advanced features such as:

\begin{itemize}
\item CPU affinity and task binding
\item QoS
\item fairshare
\item accounting
\item scheduling policies
\item advanced resource management
\end{itemize}
\section{Advanced Slurm Configuration}
\label{sec:org5b51f68}

This section extends the Slurm setup from a basic scheduler into a more
resource-aware and production-like HPC workload manager.

At this stage, the cluster already had:

\begin{itemize}
\item working Slurm scheduler
\item multinode job execution
\item MPI runtime support
\item centralized LDAP authentication
\item shared filesystem
\item shared Spack software stack
\end{itemize}

The goal of this phase was to gradually introduce advanced scheduler
features without destabilizing the cluster runtime environment.

The approach followed:

\begin{itemize}
\item incremental configuration
\item verification after every change
\item understanding scheduler behavior
\item avoiding unsafe configurations initially
\end{itemize}
\subsection{IMPORTANT APPROACH}
\label{sec:org452122a}

Advanced Slurm administration should always be performed incrementally.

Instead of enabling multiple advanced features simultaneously, only one
feature was introduced and tested at a time.

This helps isolate issues and understand scheduler behavior properly.
\subsection{Current Cluster State Before Advanced Configuration}
\label{sec:orgaec8fe9}

Cluster components already working:

\begin{center}
\begin{tabular}{ll}
Component & Status\\
\hline
Slurm scheduler & Working\\
sbatch & Working\\
MPI jobs & Working\\
Multinode jobs & Working\\
LDAP users & Working\\
Shared filesystem & Working\\
Spack software stack & Working\\
\end{tabular}
\end{center}

This provided a stable foundation before introducing advanced scheduler
features.
\subsection{Resource-Aware Scheduling}
\label{sec:org76fa42b}

Goal:

Enable Slurm to understand and schedule cluster resources more accurately.

This phase introduced:

\begin{itemize}
\item memory-aware scheduling
\item consumable resource scheduling
\item core-aware scheduling
\item scheduler backfill support
\end{itemize}

without enabling strict kernel-level resource enforcement.
\begin{enumerate}
\item IMPORTANT CONCEPT
\label{sec:orgd29771a}

At this stage:

\begin{verbatim}
memory-aware scheduling
\end{verbatim}

was enabled.

NOT:

\begin{verbatim}
strict memory enforcement
\end{verbatim}

This distinction is extremely important.

The scheduler now understands memory resources and schedules jobs
accordingly, but Linux is still responsible for actual memory allocation.

No cgroup-based hard memory limits were enabled yet.
\item Existing Stable Slurm Configuration
\label{sec:orgf34bd18}

Current working configuration:

\begin{verbatim}
ClusterName=hpccluster
SlurmctldHost=master

MpiDefault=none
ProctrackType=proctrack/cgroup
ReturnToService=2

SlurmctldPidFile=/run/slurmctld.pid
SlurmdPidFile=/run/slurmd.pid

SlurmdSpoolDir=/var/spool/slurmd
StateSaveLocation=/var/spool/slurmctld

SlurmUser=slurm

SwitchType=switch/none
TaskPlugin=task/affinity

SchedulerType=sched/backfill

SelectType=select/cons_tres
SelectTypeParameters=CR_Core

AuthType=auth/munge

NodeName=compute01 CPUs=2 RealMemory=1200 State=UNKNOWN
NodeName=compute02 CPUs=2 RealMemory=1200 State=UNKNOWN
NodeName=compute03 CPUs=2 RealMemory=1200 State=UNKNOWN

PartitionName=debug Nodes=ALL Default=YES MaxTime=INFINITE State=UP
\end{verbatim}
\item Explanation of Important Parameters
\label{sec:org7ee4001}

\begin{enumerate}
\item SchedulerType=sched/backfill
\label{sec:orgc79773d}

Enables Slurm backfill scheduler.

Purpose:

Allows smaller jobs to run while larger jobs wait for resources.

Benefits:

\begin{itemize}
\item improves cluster utilization
\item reduces idle time
\item increases scheduling efficiency
\end{itemize}

Production HPC systems commonly use backfill scheduling.
\item SelectType=select/cons\_tres
\label{sec:org82cdfb3}

Enables:

\begin{verbatim}
consumable resource scheduling
\end{verbatim}

This changes Slurm from:

\begin{verbatim}
simple node allocation
\end{verbatim}

to:

\begin{verbatim}
resource-aware allocation
\end{verbatim}

Slurm now tracks:

\begin{itemize}
\item CPUs
\item memory
\item consumable resources
\end{itemize}

instead of only allocating entire nodes.
\item SelectTypeParameters=CR\_Core
\label{sec:org6b8b81a}

Enables:

\begin{verbatim}
core-aware scheduling
\end{verbatim}

Resources are now scheduled at CPU core granularity.

This is extremely important for:

\begin{itemize}
\item MPI workloads
\item OpenMP workloads
\item hybrid parallel jobs
\end{itemize}
\item TaskPlugin=task/affinity
\label{sec:orge8a87a4}

Enables CPU affinity support.

Purpose:

Allows Slurm to:

\begin{itemize}
\item pin tasks to CPUs
\item control process placement
\item improve cache locality
\item optimize parallel performance
\end{itemize}

This is one of the most important performance-related scheduler features.
\item RealMemory=1200
\label{sec:org70aa2d5}

Defines schedulable memory for each node.

Important:

This value should NOT equal total physical memory.

Example:

Node actual memory:

\begin{verbatim}
1700 MB
\end{verbatim}

Configured schedulable memory:

\begin{verbatim}
1200 MB
\end{verbatim}

Reason:

The operating system itself requires memory for:

\begin{itemize}
\item kernel
\item filesystem cache
\item SSH sessions
\item Slurm daemons
\item background services
\end{itemize}

Production clusters always reserve memory for system operation.
\end{enumerate}
\item IMPORTANT MEMORY SCHEDULING CONCEPT
\label{sec:org2a63945}

At this stage:

\begin{verbatim}
#SBATCH --mem=500M
\end{verbatim}

does:

\begin{itemize}
\item reserve memory in scheduler accounting
\item affect scheduling decisions
\item prevent scheduler over-allocation
\end{itemize}

BUT does NOT:

\begin{itemize}
\item hard-limit memory usage
\item kill memory-overusing jobs
\item enforce kernel-level restrictions
\end{itemize}

because:

\begin{verbatim}
TaskPlugin=task/cgroup
\end{verbatim}

was intentionally NOT enabled yet.

This avoids runtime instability while still enabling resource-aware
scheduling.
\end{enumerate}
\subsection{CPU Affinity and Task Binding}
\label{sec:org5cadd9d}

Goal:

Understand and validate:

\begin{itemize}
\item CPU affinity
\item process placement
\item task binding
\item scheduler-controlled CPU allocation
\end{itemize}

This phase connects directly with:

\begin{itemize}
\item parallel performance
\item cache locality
\item MPI optimization
\item OpenMP optimization
\end{itemize}
\begin{enumerate}
\item IMPORTANT CONCEPT
\label{sec:orgfda6dbb}

Without CPU affinity:

\begin{itemize}
\item processes may migrate between CPUs
\item cache locality decreases
\item performance becomes inconsistent
\end{itemize}

With affinity:

\begin{itemize}
\item processes stay pinned to CPUs
\item cache locality improves
\item scheduler behavior becomes deterministic
\end{itemize}

This is extremely important in production HPC systems.
\end{enumerate}
\subsection{Understanding CPU Topology}
\label{sec:orgc3bdb3e}

CPU topology information was inspected using:

\begin{verbatim}
ssh compute01 lscpu
\end{verbatim}

Important fields:

\begin{center}
\begin{tabular}{ll}
Field & Meaning\\
\hline
CPU(s) & total logical CPUs\\
Core(s) per socket & physical cores\\
Socket(s) & CPU packages\\
Thread(s) per core & SMT/Hyperthreading\\
NUMA node(s) & memory locality domains\\
\end{tabular}
\end{center}

This information becomes important for:

\begin{itemize}
\item task placement
\item thread pinning
\item NUMA optimization
\item cache locality
\end{itemize}
\subsection{CPU Affinity Validation Program}
\label{sec:orgbe7d931}

Program used to validate CPU placement:

\begin{verbatim}
#define _GNU_SOURCE

#include <stdio.h>
#include <sched.h>
#include <unistd.h>

int main() {

    int cpu = sched_getcpu();

    printf("Process running on CPU: %d\n", cpu);

    sleep(20);

    return 0;
}
\end{verbatim}

Compile:

\begin{verbatim}
gcc affinity.c -o affinity
\end{verbatim}

Purpose:

The program asks Linux:

\begin{verbatim}
Which CPU core am I running on?
\end{verbatim}

This helps visualize scheduler-controlled CPU placement.
\subsection{Distributed CPU Placement Test}
\label{sec:org00b7e12}

Slurm batch script:

\begin{verbatim}
#!/bin/bash
#SBATCH -N 2
#SBATCH --ntasks-per-node=1
#SBATCH --job-name=affinity
#SBATCH --output=%j.out
#SBATCH --error=%j.err
#SBATCH --time=00:01:00

srun ./affinity
\end{verbatim}

Submit:

\begin{verbatim}
sbatch affinity.sh
\end{verbatim}

Observed behavior:

\begin{verbatim}
Process running on CPU: 0
Process running on CPU: 0
\end{verbatim}

IMPORTANT:

CPU numbering is local to each node.

Meaning:

\begin{itemize}
\item compute01 has CPU0
\item compute02 also has CPU0
\end{itemize}

Each node maintains independent CPU numbering.

This is an important distributed-system concept.
\subsection{Multiple Task Placement Test}
\label{sec:orga14b198}

Modified batch script:

\begin{verbatim}
#!/bin/bash
#SBATCH -N 2
#SBATCH --ntasks-per-node=2
#SBATCH --job-name=affinity
#SBATCH --output=%j.out
#SBATCH --error=%j.err
#SBATCH --time=00:01:00

srun ./affinity
\end{verbatim}

Observed behavior:

\begin{verbatim}
Process running on CPU: 0
Process running on CPU: 1
Process running on CPU: 1
Process running on CPU: 0
\end{verbatim}

This validates:

\begin{itemize}
\item task distribution across cores
\item scheduler-controlled placement
\item multi-core resource allocation
\end{itemize}

Each node distributed tasks across available CPUs.
\subsection{Explicit CPU Binding}
\label{sec:orge59c6a6}

Explicit CPU binding script:

\begin{verbatim}
#!/bin/bash
#SBATCH -N 1
#SBATCH --ntasks=2
#SBATCH --cpus-per-task=1
#SBATCH --job-name=bindtest
#SBATCH --output=%j.out
#SBATCH --error=%j.err
#SBATCH --time=00:01:00

srun --cpu-bind=cores ./affinity
\end{verbatim}

Submit:

\begin{verbatim}
sbatch bind.sh
\end{verbatim}

Observed behavior:

\begin{verbatim}
Process running on CPU: 0
Process running on CPU: 1
\end{verbatim}

This validates:

\begin{itemize}
\item explicit CPU pinning
\item deterministic CPU placement
\item Slurm affinity control
\item Linux scheduler cooperation
\end{itemize}
\begin{enumerate}
\item IMPORTANT CONCEPT
\label{sec:org30d2f41}

Without:

\begin{verbatim}
--cpu-bind=cores
\end{verbatim}

Linux may migrate processes between CPUs.

With explicit binding:

\begin{itemize}
\item processes stay pinned
\item cache locality improves
\item performance stabilizes
\end{itemize}

This becomes extremely important for:

\begin{itemize}
\item MPI scaling
\item OpenMP scaling
\item hybrid MPI/OpenMP execution
\item NUMA systems
\end{itemize}
\end{enumerate}
\subsection{Slurm Accounting Database Setup}
\label{sec:orgb86cfc4}

Goal:

Transform Slurm from:

\begin{verbatim}
basic scheduler
\end{verbatim}

into:

\begin{verbatim}
persistent accounting-aware scheduler
\end{verbatim}

This phase introduced:

\begin{itemize}
\item MariaDB backend
\item Slurm accounting daemon
\item persistent job history
\item resource usage tracking
\item scheduler accounting infrastructure
\end{itemize}
\subsection{Install MariaDB}
\label{sec:org1c74106}

Performed ONLY on master node.

Install packages:

\begin{verbatim}
sudo dnf install -y mariadb-server mariadb
\end{verbatim}

Enable and start service:

\begin{verbatim}
sudo systemctl enable --now mariadb
\end{verbatim}

Verify:

\begin{verbatim}
sudo systemctl status mariadb
\end{verbatim}

Expected:

\begin{verbatim}
active (running)
\end{verbatim}
\subsection{Secure MariaDB}
\label{sec:org5828630}

Run:

\begin{verbatim}
sudo mysql_secure_installation
\end{verbatim}

Recommended configuration:

\begin{center}
\begin{tabular}{ll}
Question & Recommended Answer\\
\hline
Switch to unix\_socket authentication & no\\
Change root password & optional\\
Remove anonymous users & yes\\
Disallow remote root login & yes\\
Remove test database & yes\\
Reload privileges & yes\\
\end{tabular}
\end{center}
\subsection{Create Slurm Accounting Database}
\label{sec:orgb07d469}

Enter MariaDB:

\begin{verbatim}
sudo mysql -u root -p
\end{verbatim}

Create database:

\begin{verbatim}
CREATE DATABASE slurm_acct_db;
\end{verbatim}

Create Slurm database user:

\begin{verbatim}
CREATE USER 'slurm'@'localhost' IDENTIFIED BY 'slurmdbpass';
\end{verbatim}

Grant permissions:

\begin{verbatim}
GRANT ALL PRIVILEGES ON slurm_acct_db.* TO 'slurm'@'localhost';
\end{verbatim}

Apply changes:

\begin{verbatim}
FLUSH PRIVILEGES;
\end{verbatim}

Exit:

\begin{verbatim}
EXIT;
\end{verbatim}
\subsection{Install Slurm Accounting Daemon}
\label{sec:org1ff5927}

Install:

\begin{verbatim}
sudo dnf install -y slurm-slurmdbd
\end{verbatim}

Purpose:

Installs:

\begin{verbatim}
slurmdbd
\end{verbatim}

which connects:

\begin{itemize}
\item Slurm scheduler
\item accounting database
\item resource tracking infrastructure
\end{itemize}
\subsection{Configure slurmdbd}
\label{sec:org08c60da}

Create configuration:

\begin{verbatim}
sudo vim /etc/slurm/slurmdbd.conf
\end{verbatim}

Configuration:

\begin{verbatim}
AuthType=auth/munge

DbdHost=master
DbdPort=6819

SlurmUser=slurm

DebugLevel=info
LogFile=/var/log/slurmdbd.log
PidFile=/run/slurmdbd.pid

StorageType=accounting_storage/mysql
StorageHost=localhost
StoragePort=3306
StorageUser=slurm
StoragePass=slurmdbpass
StorageLoc=slurm_acct_db
\end{verbatim}
\begin{enumerate}
\item IMPORTANT SECURITY STEP
\label{sec:orgf11106e}

Protect configuration file:

\begin{verbatim}
sudo chown slurm:slurm /etc/slurm/slurmdbd.conf
sudo chmod 600 /etc/slurm/slurmdbd.conf
\end{verbatim}

Reason:

Database credentials are stored inside the file.

Production systems always restrict access to this configuration.
\end{enumerate}
\subsection{Connect Slurm With Accounting System}
\label{sec:org00761b1}

Modify:

\begin{verbatim}
sudo vim /etc/slurm/slurm.conf
\end{verbatim}

Add:

\begin{verbatim}
AccountingStorageType=accounting_storage/slurmdbd
AccountingStorageHost=master
AccountingStoragePort=6819
JobAcctGatherType=jobacct_gather/linux
\end{verbatim}

Purpose:

Enable:

\begin{itemize}
\item persistent job accounting
\item resource tracking
\item historical scheduler data
\end{itemize}
\subsection{Restart Slurm Services}
\label{sec:org803b5d7}

Restart accounting daemon on master:

\begin{verbatim}
sudo systemctl enable --now slurmdbd
\end{verbatim}

Verify:

\begin{verbatim}
sudo systemctl status slurmdbd
\end{verbatim}

Expected:

\begin{verbatim}
active (running)
\end{verbatim}

Restart Slurm controller:

\begin{verbatim}
sudo systemctl restart slurmctld
\end{verbatim}

Restart compute daemons:

\begin{verbatim}
ssh compute01 sudo -S systemctl restart slurmd
ssh compute02 sudo -S systemctl restart slurmd
ssh compute03 sudo -S systemctl restart slurmd
\end{verbatim}
\subsection{Verify Accounting Infrastructure}
\label{sec:org836f678}

Check cluster:

\begin{verbatim}
sinfo
\end{verbatim}

Expected:

\begin{verbatim}
compute[01-03] idle
\end{verbatim}

Check accounting cluster registration:

\begin{verbatim}
sacctmgr list cluster
\end{verbatim}

Expected:

\begin{verbatim}
hpccluster
\end{verbatim}
\subsection{Verify Persistent Job Accounting}
\label{sec:orgb9e198c}

Submit MPI job:

\begin{verbatim}
sbatch submit.sh a.out
\end{verbatim}

Check scheduler queue:

\begin{verbatim}
squeue --me
\end{verbatim}

Check accounting history:

\begin{verbatim}
sacct
\end{verbatim}

Observed output:

\begin{verbatim}
JobID           JobName  Partition    AllocCPUS      State
22                 test      debug             4  COMPLETED
22.batch          batch                       2  COMPLETED
22.0              prted                       2  COMPLETED
\end{verbatim}

This validates:

\begin{itemize}
\item persistent accounting
\item resource tracking
\item MPI runtime tracking
\item scheduler database integration
\end{itemize}
\subsection{IMPORTANT MPI Accounting Observation}
\label{sec:org42fdb82}

Accounting now tracks internal execution hierarchy:

\begin{center}
\begin{tabular}{rl}
Entry & Meaning\\
\hline
22 & main job\\
22.batch & batch execution step\\
22.0 & MPI runtime launcher\\
\end{tabular}
\end{center}

This provides deeper visibility into scheduler behavior and runtime
execution structure.
\subsection{Current Cluster Capability}
\label{sec:org4b7b281}

The cluster now supports:

\begin{center}
\begin{tabular}{ll}
Feature & Status\\
\hline
Multinode scheduling & Working\\
MPI execution & Working\\
LDAP authentication & Working\\
Shared filesystem & Working\\
Spack software stack & Working\\
Resource-aware scheduling & Working\\
Memory-aware scheduling & Working\\
CPU affinity & Working\\
Explicit CPU binding & Working\\
Slurm accounting database & Working\\
Persistent job history & Working\\
Resource tracking & Working\\
\end{tabular}
\end{center}

The cluster has now evolved from a basic educational setup into a much
more realistic HPC infrastructure environment.
\section{QoS + User Limits + Scheduling Policies}
\label{sec:orgeabf07d}

This phase extends the Slurm scheduler from:

\begin{verbatim}
resource-aware scheduling
\end{verbatim}

into:

\begin{verbatim}
policy-aware scheduling
\end{verbatim}

At this stage, the cluster already supported:

\begin{itemize}
\item multinode scheduling
\item MPI execution
\item LDAP authentication
\item shared filesystem
\item shared Spack software stack
\item resource-aware scheduling
\item CPU affinity
\item persistent Slurm accounting
\end{itemize}

The goal of this phase was to introduce:

\begin{itemize}
\item user policies
\item scheduler limits
\item QoS enforcement
\item runtime restrictions
\item fair resource sharing
\end{itemize}

without introducing kernel-level runtime instability.
\subsection{IMPORTANT CONCEPT}
\label{sec:org10e1299}

Before this phase:

Slurm could:

\begin{itemize}
\item schedule resources
\item track jobs
\item track users
\item record accounting information
\end{itemize}

BUT it could NOT yet:

\begin{itemize}
\item enforce user policies
\item restrict CPU usage
\item limit runtime
\item prevent resource abuse
\end{itemize}

This phase enables:

\begin{verbatim}
policy-aware scheduling
\end{verbatim}

similar to real production HPC systems.
\subsection{IMPORTANT Slurm Scheduling Hierarchy}
\label{sec:orga606759}

Slurm scheduling hierarchy:

\begin{verbatim}
Cluster
 └── Account
      └── User
           └── QoS
\end{verbatim}

Production HPC systems commonly organize users using:

\begin{itemize}
\item projects
\item research groups
\item departments
\item labs
\end{itemize}

called:

\begin{verbatim}
Accounts
\end{verbatim}

Users are then associated with:

\begin{itemize}
\item accounts
\item QoS policies
\item scheduler limits
\end{itemize}
\subsection{Existing Stable Accounting Infrastructure}
\label{sec:org041ce05}

Before enabling QoS enforcement, the following components were already
working:

\begin{center}
\begin{tabular}{ll}
Component & Status\\
\hline
MariaDB & Working\\
slurmdbd & Working\\
sacct & Working\\
Persistent accounting & Working\\
Job history tracking & Working\\
Resource tracking & Working\\
\end{tabular}
\end{center}

This provided the required foundation for policy enforcement.
\subsection{Understanding QoS}
\label{sec:org7de303f}

QoS stands for:

\begin{verbatim}
Quality of Service
\end{verbatim}

QoS controls:

\begin{itemize}
\item maximum CPUs
\item maximum jobs
\item walltime limits
\item scheduler priorities
\item resource restrictions
\item policy enforcement
\end{itemize}

Production HPC clusters heavily rely on QoS to maintain fairness and
prevent abuse.
\subsection{IMPORTANT QoS Concept}
\label{sec:org7a5e25b}

QoS does NOT directly enforce kernel-level runtime isolation.

Instead, it performs:

\begin{verbatim}
scheduler-side policy enforcement
\end{verbatim}

Meaning:

Slurm scheduler decides:

\begin{itemize}
\item whether jobs may start
\item whether jobs remain pending
\item whether limits are exceeded
\end{itemize}

WITHOUT requiring:

\begin{verbatim}
task/cgroup
\end{verbatim}

or strict kernel enforcement.

This keeps the cluster significantly more stable while still enabling
realistic scheduler policies.
\subsection{Verify Cluster Registration}
\label{sec:org0fb6d2e}

Before creating accounts and policies, verify cluster registration.

Run on:

\begin{itemize}
\item login node
\item or master node
\end{itemize}

\begin{verbatim}
sacctmgr list cluster
\end{verbatim}

Expected:

\begin{verbatim}
hpccluster
\end{verbatim}

Purpose:

Verifies:

\begin{itemize}
\item accounting database connectivity
\item slurmdbd functionality
\item cluster accounting registration
\end{itemize}
\subsection{Create Slurm Account}
\label{sec:org1be98ef}

Accounts represent:

\begin{itemize}
\item research groups
\item projects
\item departments
\item user groups
\end{itemize}

Create a general account.

Run on master node:

\begin{verbatim}
sudo sacctmgr add account general
\end{verbatim}

Verify:

\begin{verbatim}
sacctmgr list account
\end{verbatim}

Expected:

\begin{verbatim}
general
\end{verbatim}
\subsection{IMPORTANT Account Concept}
\label{sec:org09168cf}

Linux users and Slurm accounts are NOT the same thing.

Linux users provide:

\begin{verbatim}
authentication identity
\end{verbatim}

Slurm accounts provide:

\begin{verbatim}
scheduler policy grouping
\end{verbatim}

This distinction is extremely important in production HPC systems.
\subsection{Add Users to Slurm Accounting}
\label{sec:org9b6f7da}

LDAP users already existed in Linux.

However, Slurm accounting ALSO requires user associations.

Example user:

\begin{verbatim}
rishi
\end{verbatim}

Run on master node:

\begin{verbatim}
sudo sacctmgr add user rishi account=general
\end{verbatim}

Add additional users if needed:

\begin{verbatim}
sudo sacctmgr add user hpcadmin account=general
\end{verbatim}

Verify associations:

\begin{verbatim}
sacctmgr list user
\end{verbatim}

Expected:

Users associated with:

\begin{verbatim}
general
\end{verbatim}
\subsection{Create QoS Policy}
\label{sec:orgf736e7d}

Create QoS policy named:

\begin{verbatim}
normal
\end{verbatim}

Run on master node:

\begin{verbatim}
sudo sacctmgr add qos normal
\end{verbatim}

Verify:

\begin{verbatim}
sacctmgr list qos
\end{verbatim}

Expected:

\begin{verbatim}
normal
\end{verbatim}
\subsection{Configure QoS Limits}
\label{sec:org577e857}

The following limits were configured.
\begin{enumerate}
\item Maximum Concurrent CPUs
\label{sec:orge54a3cf}

Run:

\begin{verbatim}
sudo sacctmgr modify qos normal set MaxTRESPerUser=cpu=4
\end{verbatim}

Purpose:

Limit each user to:

\begin{verbatim}
maximum 4 CPUs simultaneously
\end{verbatim}

IMPORTANT:

This controls:

\begin{verbatim}
active running resource usage
\end{verbatim}

NOT:

\begin{itemize}
\item total submissions
\item queued jobs
\item single-job submission ability
\end{itemize}
\item Maximum Concurrent Running Jobs
\label{sec:org58980f2}

Run:

\begin{verbatim}
sudo sacctmgr modify qos normal set MaxJobsPerUser=2
\end{verbatim}

Purpose:

Limit users to:

\begin{verbatim}
maximum 2 running jobs
\end{verbatim}

Additional jobs remain pending.
\item Maximum Walltime
\label{sec:orgc067e55}

Run:

\begin{verbatim}
sudo sacctmgr modify qos normal set MaxWall=01:00:00
\end{verbatim}

Purpose:

Restrict maximum job runtime to:

\begin{verbatim}
1 hour
\end{verbatim}

Jobs requesting larger runtime remain pending.
\end{enumerate}
\subsection{Attach QoS to Account}
\label{sec:orgb55e32f}

Connect QoS policy to account.

Run:

\begin{verbatim}
sudo sacctmgr modify account general set qos=normal
\end{verbatim}

Purpose:

Users associated with:

\begin{verbatim}
general
\end{verbatim}

now inherit:

\begin{verbatim}
normal
\end{verbatim}

QoS policies.
\subsection{Verify Associations}
\label{sec:orgce5d691}

Verify:

\begin{itemize}
\item user associations
\item account associations
\item QoS assignments
\end{itemize}

Run:

\begin{verbatim}
sacctmgr show assoc
\end{verbatim}

Purpose:

Displays:

\begin{itemize}
\item users
\item accounts
\item partitions
\item qos assignments
\end{itemize}
\subsection{IMPORTANT ISSUE FACED}
\label{sec:orgcac201d}

Initially, QoS appeared to exist correctly:

\begin{itemize}
\item QoS visible
\item associations visible
\item accounting working
\item job tracking working
\end{itemize}

BUT:

\begin{verbatim}
policy enforcement was NOT working
\end{verbatim}

Jobs exceeding configured CPU limits still executed successfully.

Example:

\begin{verbatim}
MaxTRESPerUser=cpu=4
\end{verbatim}

BUT:

\begin{verbatim}
6 CPU jobs still executed
\end{verbatim}

This revealed an extremely important missing scheduler configuration.
\subsection{Root Cause of QoS Enforcement Failure}
\label{sec:org4e20689}

The following line was missing from:

\begin{verbatim}
/etc/slurm/slurm.conf
\end{verbatim}

Missing configuration:

\begin{verbatim}
AccountingStorageEnforce=limits,qos
\end{verbatim}

Without this line:

\begin{itemize}
\item accounting works
\item sacct works
\item qos appears correctly
\item associations appear correctly
\end{itemize}

BUT:

\begin{verbatim}
scheduler policies are NOT enforced
\end{verbatim}

This is one of the most common beginner Slurm configuration mistakes.
\subsection{Enable QoS Enforcement}
\label{sec:org3215925}

Edit configuration on master node:

\begin{verbatim}
sudo vim /etc/slurm/slurm.conf
\end{verbatim}

Add:

\begin{verbatim}
AccountingStorageEnforce=limits,qos
\end{verbatim}

Recommended placement:

Near existing accounting configuration:

\begin{verbatim}
AccountingStorageType=accounting_storage/slurmdbd
AccountingStorageHost=master
AccountingStoragePort=6819
JobAcctGatherType=jobacct_gather/linux
\end{verbatim}
\subsection{IMPORTANT Enforcement Concept}
\label{sec:org2a82226}

This line enables actual scheduler-side enforcement for:

\begin{itemize}
\item qos limits
\item account limits
\item user policies
\item scheduler restrictions
\end{itemize}

Without:

\begin{verbatim}
AccountingStorageEnforce
\end{verbatim}

QoS becomes informational only.
\subsection{Distribute Updated Configuration}
\label{sec:orgdc2bfdc}

Copy updated configuration from master node:

\begin{verbatim}
scp /etc/slurm/slurm.conf login:/tmp/
scp /etc/slurm/slurm.conf compute01:/tmp/
scp /etc/slurm/slurm.conf compute02:/tmp/
scp /etc/slurm/slurm.conf compute03:/tmp/
\end{verbatim}

Move configuration into place on login node:

\begin{verbatim}
sudo mv /tmp/slurm.conf /etc/slurm/
\end{verbatim}

Move configuration into place on compute nodes:

\begin{verbatim}
ssh compute01 sudo -S mv /tmp/slurm.conf /etc/slurm/
ssh compute02 sudo -S mv /tmp/slurm.conf /etc/slurm/
ssh compute03 sudo -S mv /tmp/slurm.conf /etc/slurm/
\end{verbatim}
\subsection{Restart Slurm Services}
\label{sec:orgc112298}

Restart controller on master node:

\begin{verbatim}
sudo systemctl restart slurmctld
\end{verbatim}

Restart compute daemons:

\begin{verbatim}
ssh compute01 sudo -S systemctl restart slurmd
ssh compute02 sudo -S systemctl restart slurmd
ssh compute03 sudo -S systemctl restart slurmd
\end{verbatim}

Purpose:

Reload updated scheduler policy configuration.
\subsection{Verify Cluster Health}
\label{sec:org73cd967}

Run:

\begin{verbatim}
sinfo
\end{verbatim}

Expected:

\begin{verbatim}
compute[01-03] idle
\end{verbatim}

This validates:

\begin{itemize}
\item scheduler stability
\item successful configuration reload
\item healthy compute nodes
\end{itemize}
\subsection{Verify QoS Enforcement}
\label{sec:orgc503848}

\begin{enumerate}
\item CPU Limit Validation
\label{sec:orgcad1a05}

Job requesting:

\begin{verbatim}
6 CPUs
\end{verbatim}

using:

\begin{verbatim}
#SBATCH -N 3
#SBATCH --ntasks-per-node=2
#SBATCH --ntasks=6
\end{verbatim}

was submitted.

Observed scheduler behavior:

\begin{verbatim}
PENDING
Reason=QOSMaxCpuPerUserLimit
\end{verbatim}

This validates:

\begin{itemize}
\item active QoS enforcement
\item CPU limit enforcement
\item scheduler-side policy restriction
\end{itemize}
\item IMPORTANT CPU QoS Concept
\label{sec:org75291ea}

\begin{verbatim}
MaxTRESPerUser=cpu=4
\end{verbatim}

means:

\begin{verbatim}
maximum ACTIVE running CPUs simultaneously
\end{verbatim}

NOT:

\begin{itemize}
\item maximum job submissions
\item maximum queued jobs
\item maximum CPUs requested in scripts
\end{itemize}

This distinction is extremely important.
\end{enumerate}
\subsection{Verify Valid CPU Allocation}
\label{sec:org95f47e3}

Job requesting:

\begin{verbatim}
4 CPUs
\end{verbatim}

using:

\begin{verbatim}
#SBATCH -N 2
#SBATCH --ntasks-per-node=2
\end{verbatim}

executed successfully.

Observed output:

\begin{verbatim}
SLURM_NTASKS=4
SLURM_JOB_NUM_NODES=2
SLURM_CPUS_ON_NODE=2
\end{verbatim}

This validates:

\begin{itemize}
\item multinode allocation
\item scheduler variable propagation
\item QoS-aware MPI execution
\item proper CPU accounting
\end{itemize}
\subsection{IMPORTANT MPI + QoS Integration}
\label{sec:org855cf58}

MPI jobs remained fully functional while QoS policies were enforced.

This validates successful integration between:

\begin{itemize}
\item Slurm scheduler
\item QoS policies
\item multinode scheduling
\item MPI runtime
\item resource accounting
\end{itemize}
\subsection{Verify Walltime Enforcement}
\label{sec:org9bfd1eb}

Job script modified:

\begin{verbatim}
#SBATCH --time=02:00:00
\end{verbatim}

Observed scheduler behavior:

\begin{verbatim}
PENDING
Reason=QOSMaxWallDurationPerJobLimit
\end{verbatim}

This validates:

\begin{itemize}
\item runtime limit enforcement
\item scheduler-side walltime policy control
\end{itemize}
\subsection{IMPORTANT Pending Queue Concept}
\label{sec:org5fa7138}

QoS-restricted jobs are usually:

\begin{verbatim}
queued and pending
\end{verbatim}

NOT:

\begin{verbatim}
immediately rejected
\end{verbatim}

This is standard production HPC scheduler behavior.

Scheduler explains pending reason using:

\begin{verbatim}
squeue --me
\end{verbatim}

or:

\begin{verbatim}
scontrol show job <jobid>
\end{verbatim}

This greatly improves scheduler transparency.
\subsection{Inspect QoS Configuration}
\label{sec:org66f9e3e}

View configured QoS settings:

\begin{verbatim}
sacctmgr show qos normal
\end{verbatim}

Observed configuration:

\begin{verbatim}
MaxWall=01:00:00
MaxTRESPU=cpu=4
MaxJobsPU=2
\end{verbatim}

This validates:

\begin{itemize}
\item correct QoS policy configuration
\item scheduler policy persistence
\item accounting database integration
\end{itemize}
\subsection{IMPORTANT Scheduler Layers}
\label{sec:org204a02e}

The cluster now supports multiple scheduler layers simultaneously.

\begin{center}
\begin{tabular}{ll}
Layer & Purpose\\
\hline
Resource scheduling & CPU and memory allocation\\
Affinity scheduling & CPU placement and pinning\\
Accounting & Persistent job/resource tracking\\
Policy enforcement & User and QoS restrictions\\
\end{tabular}
\end{center}

This architecture is significantly closer to real production HPC
clusters.
\subsection{Current Cluster Capability}
\label{sec:orgcd653c3}

The cluster now supports:

\begin{center}
\begin{tabular}{ll}
Feature & Status\\
\hline
Multinode scheduling & Working\\
MPI execution & Working\\
LDAP authentication & Working\\
Shared filesystem & Working\\
Spack software stack & Working\\
Resource-aware scheduling & Working\\
CPU affinity & Working\\
Persistent accounting & Working\\
QoS enforcement & Working\\
User limits & Working\\
CPU policy enforcement & Working\\
Walltime policy enforcement & Working\\
Multi-user scheduling policies & Working\\
\end{tabular}
\end{center}

The cluster has now evolved into a significantly more realistic
multi-user HPC scheduling environment.
\section{Validation and Verification of Advanced Slurm Configuration and QoS}
\label{sec:org0835044}

This section validates all advanced Slurm scheduler features configured
during the setup process.

The purpose of validation is to ensure:

\begin{itemize}
\item scheduler stability
\item correct resource tracking
\item proper QoS enforcement
\item multinode execution
\item MPI integration
\item accounting consistency
\item policy-aware scheduling behavior
\end{itemize}

All validation steps were tested practically on the configured cluster.
\subsection{Cluster Validation State}
\label{sec:orgaaa5cd3}

At the time of validation, the cluster already supported:

\begin{center}
\begin{tabular}{ll}
Feature & Status\\
\hline
Slurm scheduler & Working\\
Multinode scheduling & Working\\
MPI execution & Working\\
LDAP authentication & Working\\
Shared filesystem & Working\\
Spack software stack & Working\\
Resource-aware scheduling & Working\\
CPU affinity & Working\\
Accounting database & Working\\
QoS enforcement & Working\\
\end{tabular}
\end{center}

This validation phase ensures all components function together correctly.
\subsection{Verify Cluster Health}
\label{sec:org2bf81f0}

Run on login node:

\begin{verbatim}
sinfo
\end{verbatim}

Expected:

\begin{verbatim}
PARTITION AVAIL  TIMELIMIT  NODES  STATE NODELIST
debug*       up   infinite      3   idle compute[01-03]
\end{verbatim}

Purpose:

Verifies:

\begin{itemize}
\item scheduler communication
\item node availability
\item slurmd daemon health
\item cluster operational state
\end{itemize}

IMPORTANT:

Nodes should NOT show:

\begin{itemize}
\item drain
\item down
\item fail
\item unknown
\end{itemize}

during normal operation.
\subsection{Verify Node Resource Recognition}
\label{sec:orgf000133}

Run:

\begin{verbatim}
scontrol show node compute01
\end{verbatim}

Important fields:

\begin{verbatim}
CPUTot=2
RealMemory=1200
CfgTRES=cpu=2,mem=1200M
\end{verbatim}

Purpose:

Validates:

\begin{itemize}
\item CPU detection
\item memory-aware scheduling
\item TRES resource tracking
\item scheduler resource recognition
\end{itemize}

This confirms Slurm now understands:

\begin{itemize}
\item schedulable CPUs
\item schedulable memory
\item consumable resources
\end{itemize}

instead of only node allocation.
\subsection{Verify Accounting Infrastructure}
\label{sec:org291ff87}

Run:

\begin{verbatim}
sudo systemctl status mariadb
\end{verbatim}

Expected:

\begin{verbatim}
active (running)
\end{verbatim}

Purpose:

Validates accounting database backend.

Verify accounting daemon:

\begin{verbatim}
sudo systemctl status slurmdbd
\end{verbatim}

Expected:

\begin{verbatim}
active (running)
\end{verbatim}

Purpose:

Validates:

\begin{itemize}
\item Slurm database daemon
\item scheduler/accounting communication
\item persistent accounting infrastructure
\end{itemize}
\subsection{Verify Cluster Accounting Registration}
\label{sec:org96099da}

Run:

\begin{verbatim}
sacctmgr list cluster
\end{verbatim}

Expected:

\begin{verbatim}
hpccluster
\end{verbatim}

Purpose:

Verifies:

\begin{itemize}
\item cluster registration
\item accounting database connectivity
\item slurmdbd integration
\end{itemize}
\subsection{Verify Accounts and User Associations}
\label{sec:orgeb6c40e}

Run:

\begin{verbatim}
sacctmgr show assoc
\end{verbatim}

Expected:

\begin{verbatim}
Cluster    Account    User    Partition    QOS
hpccluster general    rishi               normal
\end{verbatim}

Purpose:

Validates:

\begin{itemize}
\item Slurm account creation
\item user associations
\item QoS assignments
\item scheduler policy hierarchy
\end{itemize}
\subsection{Verify QoS Configuration}
\label{sec:org38f2858}

Run:

\begin{verbatim}
sacctmgr show qos normal
\end{verbatim}

Expected:

\begin{verbatim}
MaxWall=01:00:00
MaxTRESPU=cpu=4
MaxJobsPU=2
\end{verbatim}

Purpose:

Validates:

\begin{itemize}
\item QoS creation
\item CPU limits
\item runtime limits
\item job count limits
\end{itemize}
\subsection{IMPORTANT Validation Issue Faced}
\label{sec:org179ea5e}

Initially:

\begin{itemize}
\item QoS existed
\item associations existed
\item accounting worked
\item jobs tracked correctly
\end{itemize}

BUT:

\begin{verbatim}
scheduler policies were NOT enforced
\end{verbatim}

Jobs exceeding configured CPU limits still executed successfully.

Example:

\begin{verbatim}
MaxTRESPU=cpu=4
\end{verbatim}

BUT:

\begin{verbatim}
6 CPU jobs still executed
\end{verbatim}

This revealed a missing scheduler enforcement configuration.
\subsection{Root Cause Analysis}
\label{sec:orgbb73400}

The following configuration was missing:

\begin{verbatim}
AccountingStorageEnforce=limits,qos
\end{verbatim}

inside:

\begin{verbatim}
/etc/slurm/slurm.conf
\end{verbatim}

Without this line:

\begin{itemize}
\item accounting works
\item sacct works
\item qos visible
\item associations visible
\end{itemize}

BUT:

\begin{verbatim}
scheduler-side policy enforcement remains disabled
\end{verbatim}

This is one of the most common Slurm QoS configuration mistakes.
\subsection{Fix Applied}
\label{sec:org319c170}

Added to:

\begin{verbatim}
/etc/slurm/slurm.conf
\end{verbatim}

\begin{verbatim}
AccountingStorageEnforce=limits,qos
\end{verbatim}

Then:

\begin{itemize}
\item distributed configuration to all nodes
\item restarted slurmctld
\item restarted slurmd daemons
\end{itemize}

This enabled actual policy enforcement.
\subsection{MPI Validation Program}
\label{sec:org4613400}

The following MPI-based parallel array summation program was used for
validation.

Program characteristics:

\begin{itemize}
\item MPI parallel execution
\item multinode communication
\item distributed workload
\item scheduler-aware execution
\item CPU resource consumption
\end{itemize}

Compilation:

\begin{verbatim}
mpicc arrSum.c -o a.out
\end{verbatim}

Purpose:

Generate realistic MPI workloads for scheduler validation.
\subsection{Validation Job Script}
\label{sec:org87174fb}

Validation batch script:

\begin{verbatim}
#!/bin/bash
#SBATCH -N 2
#SBATCH --ntasks-per-node=2
#SBATCH --job-name=test
#SBATCH --output=%J.out
#SBATCH --error=%J.err
#SBATCH --time=00:10:00

source /apps/spack/share/spack/setup-env.sh
spack load openmpi

echo "SLURM_NTASKS=$SLURM_NTASKS"
echo "SLURM_JOB_NUM_NODES=$SLURM_JOB_NUM_NODES"
echo "SLURM_CPUS_ON_NODE=$SLURM_CPUS_ON_NODE"

mpirun -np $SLURM_NTASKS ./a.out
\end{verbatim}

Purpose:

Validates:

\begin{itemize}
\item multinode scheduling
\item MPI integration
\item scheduler environment variables
\item CPU allocation
\item QoS-aware execution
\end{itemize}
\subsection{Verify Successful MPI Execution}
\label{sec:org7a71b51}

Submit job:

\begin{verbatim}
sbatch submit.sh
\end{verbatim}

Monitor queue:

\begin{verbatim}
squeue --me
\end{verbatim}

Expected:

\begin{verbatim}
RUNNING
\end{verbatim}

Purpose:

Verifies:

\begin{itemize}
\item scheduler functionality
\item multinode allocation
\item successful MPI execution
\end{itemize}
\subsection{Verify Scheduler Environment Variables}
\label{sec:org4422860}

Observed output:

\begin{verbatim}
SLURM_NTASKS=4
SLURM_JOB_NUM_NODES=2
SLURM_CPUS_ON_NODE=2
\end{verbatim}

This validates:

\begin{itemize}
\item correct CPU allocation
\item correct task allocation
\item proper multinode scheduling
\item scheduler environment propagation
\end{itemize}
\subsection{Verify CPU QoS Enforcement}
\label{sec:org9a8d28c}

Modified job script:

\begin{verbatim}
#SBATCH -N 3
#SBATCH --ntasks-per-node=2
#SBATCH --ntasks=6
\end{verbatim}

Requested resources:

\begin{verbatim}
6 CPUs
\end{verbatim}

Configured QoS limit:

\begin{verbatim}
MaxTRESPU=cpu=4
\end{verbatim}

Observed scheduler behavior:

\begin{verbatim}
PENDING
Reason=QOSMaxCpuPerUserLimit
\end{verbatim}

Purpose:

Validates:

\begin{itemize}
\item CPU policy enforcement
\item active QoS restriction
\item scheduler-side resource control
\end{itemize}
\subsection{IMPORTANT CPU QoS Concept}
\label{sec:org5e460f2}

\begin{verbatim}
MaxTRESPU=cpu=4
\end{verbatim}

means:

\begin{verbatim}
maximum ACTIVE running CPUs simultaneously
\end{verbatim}

NOT:

\begin{itemize}
\item maximum submitted jobs
\item maximum queued jobs
\item maximum CPUs requested in scripts
\end{itemize}

This distinction is extremely important.
\subsection{Verify Walltime Enforcement}
\label{sec:orgb11275c}

Modified batch script:

\begin{verbatim}
#SBATCH --time=02:00:00
\end{verbatim}

Configured QoS limit:

\begin{verbatim}
MaxWall=01:00:00
\end{verbatim}

Observed scheduler behavior:

\begin{verbatim}
PENDING
Reason=QOSMaxWallDurationPerJobLimit
\end{verbatim}

Purpose:

Validates:

\begin{itemize}
\item runtime limit enforcement
\item walltime policy enforcement
\item scheduler-side runtime restriction
\end{itemize}
\subsection{Verify Job Count Enforcement}
\label{sec:org03e8668}

Configured:

\begin{verbatim}
MaxJobsPU=2
\end{verbatim}

Submitted multiple jobs:

\begin{verbatim}
sbatch submit.sh
sbatch submit.sh
sbatch submit.sh
\end{verbatim}

Observed behavior:

\begin{itemize}
\item first jobs execute
\item additional jobs remain pending
\end{itemize}

Purpose:

Validates:

\begin{itemize}
\item concurrent job restriction
\item scheduler fairness policy
\item user job limit enforcement
\end{itemize}
\subsection{Verify Accounting Persistence}
\label{sec:orgcf601ae}

Run:

\begin{verbatim}
sacct
\end{verbatim}

Observed output:

\begin{verbatim}
JobID           JobName  Partition    AllocCPUS      State
22                 test      debug             4  COMPLETED
22.batch          batch                       2  COMPLETED
22.0              prted                       2  COMPLETED
\end{verbatim}

Purpose:

Validates:

\begin{itemize}
\item persistent accounting
\item MPI runtime tracking
\item historical scheduler data
\item job execution tracking
\end{itemize}
\subsection{IMPORTANT MPI Accounting Observation}
\label{sec:org00ee071}

Accounting tracks internal MPI execution hierarchy:

\begin{center}
\begin{tabular}{rl}
Entry & Meaning\\
\hline
22 & main scheduler job\\
22.batch & batch execution step\\
22.0 & MPI runtime launcher\\
\end{tabular}
\end{center}

This provides deeper scheduler visibility similar to production HPC
environments.
\subsection{Final Validation State}
\label{sec:orgf584be8}

After validation, the cluster successfully supported:

\begin{center}
\begin{tabular}{ll}
Feature & Status\\
\hline
Multinode scheduling & Working\\
MPI execution & Working\\
LDAP authentication & Working\\
Shared filesystem & Working\\
Spack software stack & Working\\
Resource-aware scheduling & Working\\
CPU affinity & Working\\
Accounting database & Working\\
Persistent accounting & Working\\
QoS enforcement & Working\\
CPU policy enforcement & Working\\
Runtime policy enforcement & Working\\
User job limits & Working\\
Scheduler-side policy control & Working\\
\end{tabular}
\end{center}
\subsection{Validation Conclusion}
\label{sec:org8c1a3e9}

The cluster successfully evolved from:

\begin{verbatim}
basic educational scheduler
\end{verbatim}

into:

\begin{verbatim}
policy-aware production-like HPC infrastructure
\end{verbatim}

while maintaining:

\begin{itemize}
\item scheduler stability
\item multinode MPI execution
\item resource-aware scheduling
\item persistent accounting
\item user policy enforcement
\item simplified administration
\end{itemize}

This now closely resembles the operational structure used in real HPC
centers and supercomputing environments.
\section{Fairshare Scheduling and Multifactor Priority Scheduling}
\label{sec:org3a170a8}

This phase upgrades the Slurm scheduler from:

\begin{verbatim}
policy-aware scheduling
\end{verbatim}

into:

\begin{verbatim}
fairness-aware scheduling
\end{verbatim}

At this stage, the cluster already supported:

\begin{itemize}
\item multinode scheduling
\item MPI execution
\item resource-aware scheduling
\item QoS enforcement
\item persistent accounting
\item CPU-aware scheduling
\item memory-aware scheduling
\item user policy enforcement
\end{itemize}

The goal of this phase was to introduce:

\begin{itemize}
\item fair resource sharing
\item historical usage balancing
\item multifactor scheduling
\item scheduler priorities
\item usage-aware scheduling behavior
\end{itemize}

similar to real production HPC environments.
\subsection{IMPORTANT Fairshare Concept}
\label{sec:org298bcae}

Before enabling fairshare:

Scheduler decisions were mostly based on:

\begin{itemize}
\item available resources
\item QoS limits
\item job submission order
\end{itemize}

Meaning:

\begin{verbatim}
first user to submit jobs usually gains resources first
\end{verbatim}

This can create unfair long-term resource usage.

Fairshare introduces:

\begin{verbatim}
historical usage-aware scheduling
\end{verbatim}

Meaning:

Users consuming large amounts of resources gradually lose scheduler
priority while lighter users gain scheduling preference.

This creates:

\begin{itemize}
\item balanced cluster usage
\item long-term fairness
\item anti-resource-hoarding behavior
\item improved scheduler fairness
\end{itemize}

Production supercomputers heavily rely on fairshare scheduling.
\subsection{IMPORTANT Difference Between QoS and Fairshare}
\label{sec:org20bac77}

QoS controls:

\begin{verbatim}
hard scheduling limits
\end{verbatim}

Examples:

\begin{itemize}
\item maximum CPUs
\item maximum jobs
\item maximum runtime
\end{itemize}

Fairshare controls:

\begin{verbatim}
dynamic scheduling priority balancing
\end{verbatim}

Meaning:

Jobs are NOT strictly blocked.

Instead:

\begin{verbatim}
scheduler priority changes over time
\end{verbatim}

depending on:

\begin{itemize}
\item historical resource usage
\item waiting time
\item scheduler fairness policies
\end{itemize}

This distinction is extremely important.
\subsection{Existing Stable Scheduler Infrastructure}
\label{sec:org603682e}

Before enabling fairshare, the cluster already supported:

\begin{center}
\begin{tabular}{ll}
Feature & Status\\
\hline
Multinode scheduling & Working\\
MPI execution & Working\\
QoS enforcement & Working\\
Resource-aware scheduling & Working\\
CPU affinity & Working\\
Persistent accounting & Working\\
Slurm database accounting & Working\\
Scheduler policy enforcement & Working\\
\end{tabular}
\end{center}

This provided the required foundation for multifactor scheduling.
\subsection{IMPORTANT Scheduler Concept}
\label{sec:orge2c5b5e}

Fairshare requires:

\begin{verbatim}
multifactor priority scheduling
\end{verbatim}

because scheduler decisions must now consider multiple factors
simultaneously.

Without multifactor scheduling:

\begin{verbatim}
fairshare cannot function correctly
\end{verbatim}
\subsection{Enable Multifactor Priority Scheduler}
\label{sec:orgd18da30}

Edit Slurm configuration on master node:

\begin{verbatim}
sudo vim /etc/slurm/slurm.conf
\end{verbatim}

Add:

\begin{verbatim}
PriorityType=priority/multifactor

PriorityWeightFairshare=10000
PriorityWeightAge=1000
PriorityWeightJobSize=100
PriorityWeightPartition=1000
\end{verbatim}

Recommended placement:

Near existing scheduler configuration:

\begin{verbatim}
SchedulerType=sched/backfill
\end{verbatim}
\subsection{IMPORTANT PriorityType Concept}
\label{sec:orgc231050}

\begin{verbatim}
PriorityType=priority/multifactor
\end{verbatim}

enables:

\begin{verbatim}
advanced scheduler priority calculations
\end{verbatim}

The scheduler now computes priorities using:

\begin{itemize}
\item fairshare
\item waiting age
\item job size
\item partition priority
\item scheduler weights
\end{itemize}

This is the standard production HPC scheduling model.
\subsection{Understanding PriorityWeightFairshare}
\label{sec:org818f20d}

\begin{verbatim}
PriorityWeightFairshare=10000
\end{verbatim}

This is the MOST important fairshare configuration line.

Purpose:

Controls how strongly historical resource usage influences scheduler
priority.

Higher value:

\begin{verbatim}
stronger fairshare effect
\end{verbatim}

Meaning:

Users with lower historical usage gain significantly better priority.
\subsection{Understanding PriorityWeightAge}
\label{sec:org51f197e}

\begin{verbatim}
PriorityWeightAge=1000
\end{verbatim}

Purpose:

Older waiting jobs gradually gain scheduler priority.

Prevents:

\begin{verbatim}
scheduler starvation
\end{verbatim}

Meaning:

Jobs waiting for long periods slowly move upward in priority.
\subsection{Understanding PriorityWeightJobSize}
\label{sec:org2ded258}

\begin{verbatim}
PriorityWeightJobSize=100
\end{verbatim}

Purpose:

Influence scheduling based on requested job size.

Depending on cluster policy:

\begin{itemize}
\item larger jobs may receive preference
\item smaller jobs may backfill easier
\end{itemize}

This helps improve cluster utilization.
\subsection{Understanding PriorityWeightPartition}
\label{sec:org46c95c2}

\begin{verbatim}
PriorityWeightPartition=1000
\end{verbatim}

Purpose:

Allow different partitions to receive different scheduler priorities.

Useful for future partitions such as:

\begin{itemize}
\item debug
\item teaching
\item long-running
\item premium
\item GPU partitions
\end{itemize}
\subsection{Configure Fairshare Decay Settings}
\label{sec:org23ccc7e}

Still inside:

\begin{verbatim}
/etc/slurm/slurm.conf
\end{verbatim}

Add:

\begin{verbatim}
PriorityDecayHalfLife=7-0
PriorityUsageResetPeriod=NONE
PriorityFavorSmall=NO
FairShareDampeningFactor=1
\end{verbatim}
\subsection{Understanding PriorityDecayHalfLife}
\label{sec:org0b828a8}

\begin{verbatim}
PriorityDecayHalfLife=7-0
\end{verbatim}

Meaning:

Historical resource usage gradually decays over:

\begin{verbatim}
7 days
\end{verbatim}

Purpose:

Allows users to recover scheduler priority over time.

Without decay:

\begin{verbatim}
historical usage penalties become permanent
\end{verbatim}

which is undesirable.

Production clusters commonly use:

\begin{itemize}
\item days
\item weeks
\item months
\end{itemize}

depending on scheduling policy.
\subsection{Understanding PriorityUsageResetPeriod}
\label{sec:orgd283b65}

\begin{verbatim}
PriorityUsageResetPeriod=NONE
\end{verbatim}

Meaning:

Usage history never fully resets.

Instead:

\begin{verbatim}
usage gradually decays naturally
\end{verbatim}

This creates smoother long-term fairness behavior.
\subsection{Understanding PriorityFavorSmall}
\label{sec:org883c831}

\begin{verbatim}
PriorityFavorSmall=NO
\end{verbatim}

Purpose:

Prevent scheduler from aggressively favoring very small jobs.

This keeps scheduling behavior more balanced.
\subsection{Understanding FairShareDampeningFactor}
\label{sec:orgee263d1}

\begin{verbatim}
FairShareDampeningFactor=1
\end{verbatim}

Purpose:

Controls strength of fairshare effects.

Lower values:

\begin{itemize}
\item weaker balancing
\end{itemize}

Higher values:

\begin{itemize}
\item stronger balancing
\end{itemize}

Value:

\begin{verbatim}
1
\end{verbatim}

provides moderate balanced behavior.
\subsection{Distribute Updated Configuration}
\label{sec:org8d425de}

Copy updated configuration from master node:

\begin{verbatim}
scp /etc/slurm/slurm.conf login:/tmp/
scp /etc/slurm/slurm.conf compute01:/tmp/
scp /etc/slurm/slurm.conf compute02:/tmp/
scp /etc/slurm/slurm.conf compute03:/tmp/
\end{verbatim}

Purpose:

Distribute updated scheduler configuration to all nodes.
\subsection{Install Configuration}
\label{sec:org3789fb9}

Move configuration into place on login node:

\begin{verbatim}
sudo mv /tmp/slurm.conf /etc/slurm/
\end{verbatim}

Move configuration into place on compute nodes:

\begin{verbatim}
ssh compute01 sudo -S mv /tmp/slurm.conf /etc/slurm/
ssh compute02 sudo -S mv /tmp/slurm.conf /etc/slurm/
ssh compute03 sudo -S mv /tmp/slurm.conf /etc/slurm/
\end{verbatim}

Purpose:

Replace old scheduler configuration with updated multifactor scheduling
configuration.
\subsection{Restart Slurm Services}
\label{sec:org99eec35}

Restart scheduler controller on master node:

\begin{verbatim}
sudo systemctl restart slurmctld
\end{verbatim}

Observed temporarily:

\begin{verbatim}
compute[01-03] unk
\end{verbatim}

This is normal during scheduler restart.

Restart compute daemons:

\begin{verbatim}
ssh compute01 sudo -S systemctl restart slurmd
ssh compute02 sudo -S systemctl restart slurmd
ssh compute03 sudo -S systemctl restart slurmd
\end{verbatim}

After restart:

\begin{verbatim}
sinfo
\end{verbatim}

Observed:

\begin{verbatim}
compute[01-03] idle
\end{verbatim}

This validated:

\begin{itemize}
\item successful scheduler reload
\item healthy compute nodes
\item stable scheduler state
\end{itemize}
\subsection{IMPORTANT Issue Initially Observed}
\label{sec:org1e292f0}

Immediately after configuration:

\begin{verbatim}
sprio
\end{verbatim}

returned:

\begin{verbatim}
You are not running a supported priority plugin
\end{verbatim}

This revealed:

\begin{verbatim}
PriorityType=priority/multifactor
\end{verbatim}

was not yet active.

Cause:

Scheduler restart had not yet fully applied the new configuration.

After:

\begin{itemize}
\item configuration distribution
\item slurmctld restart
\item slurmd restart
\end{itemize}

the issue disappeared successfully.
\subsection{IMPORTANT sprio Concept}
\label{sec:org76af162}

\begin{verbatim}
sprio
\end{verbatim}

is one of the MOST important Slurm administrator commands.

Purpose:

Displays:

\begin{itemize}
\item scheduler priority calculations
\item fairshare contribution
\item job age contribution
\item job size contribution
\item partition priority contribution
\end{itemize}

This command explains:

\begin{verbatim}
WHY jobs receive current scheduling priority
\end{verbatim}
\subsection{IMPORTANT Observation About sprio}
\label{sec:org74cb1bb}

Initially:

\begin{verbatim}
sprio
\end{verbatim}

showed no jobs.

This was NOT a failure.

Reason:

\begin{verbatim}
sprio only displays pending jobs
\end{verbatim}

NOT:

\begin{itemize}
\item completed jobs
\item running jobs
\end{itemize}

Since jobs executed quickly, there were no pending jobs to analyze.

This behavior is completely normal.
\subsection{Verify Fairshare Accounting}
\label{sec:orgbfd5511}

Run:

\begin{verbatim}
sshare
\end{verbatim}

Observed output:

\begin{verbatim}
RawUsage = 387
\end{verbatim}

Purpose:

Validates:

\begin{itemize}
\item historical usage tracking
\item accounting integration
\item fairshare accounting functionality
\end{itemize}

This proved Slurm was successfully recording:

\begin{itemize}
\item CPU usage
\item historical resource consumption
\item scheduling history
\end{itemize}
\subsection{Understanding sshare Output}
\label{sec:org7f80096}

\begin{verbatim}
sshare
\end{verbatim}

displays:

\begin{itemize}
\item scheduler shares
\item historical usage
\item fairshare values
\item normalized shares
\end{itemize}

This is one of the most important scheduler accounting commands.
\subsection{Understanding RawUsage}
\label{sec:org7983ea0}

Example:

\begin{verbatim}
RawUsage = 387
\end{verbatim}

Meaning:

User has consumed historical scheduler resources.

This includes:

\begin{itemize}
\item CPU usage
\item scheduler allocations
\item accumulated job execution time
\end{itemize}

Higher value means:

\begin{verbatim}
heavier historical cluster usage
\end{verbatim}
\subsection{Understanding FairShare Value}
\label{sec:orgab2847a}

Example:

\begin{verbatim}
FairShare = 1.000000
\end{verbatim}

Meaning:

User currently has full scheduling entitlement.

Initially this remained:

\begin{verbatim}
1.000000
\end{verbatim}

because only one active user existed.

Fairshare becomes meaningful only when:

\begin{itemize}
\item multiple users compete
\item jobs remain pending
\item scheduler must prioritize workloads
\end{itemize}

This is extremely important.
\subsection{IMPORTANT Fairshare Concept}
\label{sec:org8e0e1f6}

Fairshare is:

\begin{verbatim}
comparative scheduling behavior
\end{verbatim}

NOT:

\begin{verbatim}
absolute scheduling behavior
\end{verbatim}

Meaning:

Fairshare only matters when scheduler must choose between competing jobs.
\subsection{Create Additional User Competition}
\label{sec:org3e36889}

Additional users added to Slurm accounting:

\begin{verbatim}
sudo sacctmgr add user hpcadmin account=general
sudo sacctmgr add user student01 account=general
sudo sacctmgr add user student02 account=general
\end{verbatim}

Verify associations:

\begin{verbatim}
sacctmgr show assoc
\end{verbatim}

Purpose:

Create realistic multi-user scheduler competition.
\subsection{Generate Scheduler Contention}
\label{sec:org3a39594}

Multiple users submitted jobs simultaneously.

Observed queue state:

\begin{verbatim}
squeue
\end{verbatim}

Observed:

\begin{verbatim}
44 debug test student01 PD (Resources)
45 debug test student01 PD (Priority)
46 debug test rishi     PD (Priority)
\end{verbatim}

This was the FIRST real proof that:

\begin{verbatim}
multifactor scheduler priorities were functioning
\end{verbatim}
\subsection{IMPORTANT Understanding Pending Reasons}
\label{sec:org0187889}

\begin{verbatim}
(Resources)
\end{verbatim}

Meaning:

Cluster currently lacks sufficient free CPUs or nodes.

Scheduler would run the job immediately if resources existed.

\begin{verbatim}
(Priority)
\end{verbatim}

Meaning:

Resources may become available, BUT another job currently has higher
scheduler priority.

This is the MOST important fairshare scheduling observation.
\subsection{Understand sprio Output}
\label{sec:org6d69578}

Observed example:

\begin{verbatim}
JOBID PARTITION PRIORITY SITE AGE FAIRSHARE JOBSIZE PARTITION
44    debug      9066     0    0    8000      67      1000
46    debug      3066     0    0    2000      67      1000
\end{verbatim}

This output demonstrates active multifactor scheduling.
\subsection{Understanding PRIORITY Column}
\label{sec:orgc7f3316}

Example:

\begin{verbatim}
9066
\end{verbatim}

Meaning:

Final scheduler priority score.

Higher value means:

\begin{verbatim}
job executes earlier
\end{verbatim}

This is the combined result of:

\begin{itemize}
\item fairshare
\item age
\item partition priority
\item job size
\end{itemize}
\subsection{Understanding FAIRSHARE Column}
\label{sec:orgaeff282}

Example:

\begin{verbatim}
8000
\end{verbatim}

Meaning:

Fairshare contribution to total scheduler priority.

Higher value:

\begin{itemize}
\item lighter historical usage
\item scheduler preference
\item better fairness entitlement
\end{itemize}

Lower value:

\begin{itemize}
\item heavier historical resource usage
\item reduced scheduler preference
\end{itemize}

This is the core of fairshare scheduling.
\subsection{IMPORTANT Fairshare Validation}
\label{sec:orga62b7c8}

Observed priorities:

\begin{center}
\begin{tabular}{rr}
Job & Fairshare Contribution\\
\hline
44 & 8000\\
46 & 2000\\
\end{tabular}
\end{center}

This proved:

\begin{verbatim}
historical resource usage was actively influencing scheduler priority
\end{verbatim}

This is REAL production-style scheduler behavior.
\subsection{Understanding AGE Column}
\label{sec:orgc2d5240}

Initially:

\begin{verbatim}
0
\end{verbatim}

because jobs were still new.

Over time:

\begin{itemize}
\item waiting jobs accumulate age score
\item scheduler gradually boosts their priority
\end{itemize}

Purpose:

Prevent:

\begin{verbatim}
scheduler starvation
\end{verbatim}
\subsection{Understanding JOBSIZE Column}
\label{sec:org7ddc006}

Example:

\begin{verbatim}
67
\end{verbatim}

Meaning:

Scheduler contribution based on requested resources.

Can depend on:

\begin{itemize}
\item CPUs
\item nodes
\item tasks
\item requested resources
\end{itemize}

Different clusters tune this differently.
\subsection{Understanding PARTITION Column}
\label{sec:org1d36189}

Example:

\begin{verbatim}
1000
\end{verbatim}

This came from:

\begin{verbatim}
PriorityWeightPartition=1000
\end{verbatim}

Currently all jobs used the same partition.

In future:

Different partitions can receive:

\begin{itemize}
\item different priorities
\item different scheduling behavior
\item different policies
\end{itemize}
\subsection{IMPORTANT Final Scheduler State}
\label{sec:orge8240d5}

The scheduler now simultaneously considers:

\begin{center}
\begin{tabular}{ll}
Scheduling Factor & Purpose\\
\hline
resources & free CPUs and nodes\\
qos & policy restrictions\\
fairshare & historical usage balancing\\
age & waiting time fairness\\
partition & queue importance\\
job size & workload balancing\\
\end{tabular}
\end{center}

This is significantly closer to real production HPC scheduling behavior.
\subsection{Final Fairshare Validation State}
\label{sec:org45cd140}

After validation, the cluster successfully supported:

\begin{center}
\begin{tabular}{ll}
Feature & Status\\
\hline
Multifactor scheduling & Working\\
Fairshare accounting & Working\\
Historical usage tracking & Working\\
Dynamic scheduler priorities & Working\\
Queue priority balancing & Working\\
QoS integration & Working\\
Resource-aware scheduling & Working\\
Pending priority calculations & Working\\
\end{tabular}
\end{center}
\subsection{Final Conclusion}
\label{sec:org34e22d3}

The scheduler successfully evolved from:

\begin{verbatim}
simple resource-based scheduling
\end{verbatim}

into:

\begin{verbatim}
fairness-aware multifactor HPC scheduling
\end{verbatim}

while maintaining:

\begin{itemize}
\item cluster stability
\item MPI execution
\item multinode scheduling
\item scheduler accounting
\item QoS enforcement
\item realistic HPC scheduling behavior
\end{itemize}

This now resembles the scheduling architecture used in real HPC centers
and production supercomputing environments.
\end{document}
